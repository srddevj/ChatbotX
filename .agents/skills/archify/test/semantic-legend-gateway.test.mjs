...
test("only legends with an exact node-kind meaning publish bridge entries", () => {
  ...
  assert.deepEqual(
    new Set(values(architecture, "data-legend-kind")),
    architectureKinds,
  )
...
test("runtime decoration derives counts from compiled node facts and stays viewer-only", () => {
  ...
  assert.doesNotMatch(
    svg,
    /data-legend-bridge-runtime|data-legend-count=|role="toolbar"/,
  )
  assert.doesNotMatch(
    svg,
    /data-legend-kind="[^"]+"[^>]+(?:role=|aria-pressed=)/,
  )
})
...
test("preview is soft, input-aware, and yields to stronger exploration owners", () => {
  ...
  assert.match(
    html,
    /data-route-picking.*data-story-active.*data-relationship-preview-active/s,
  )
...
test("bridge state is print-safe, reduced-motion-safe, and absent from canonical export", () => {
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-legend-bridge-runtime\]'\)/,
  )
