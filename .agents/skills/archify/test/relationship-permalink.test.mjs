...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-relationship-permalink-"),
)
...
function fixture(mode) {
  return JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "examples", CASES[mode].example),
      "utf8",
    ),
  )
}
...
function relationshipKey(html, id) {
  return (
    html.match(
      new RegExp(`data-edge-key="(\\d+)" data-edge-id="${id}"`),
    )?.[1] ?? null
  )
}
...
test("all typed renderers preserve optional authored relationship ids beside runtime keys", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.match(
      html,
      /data-edge-key="\d+" data-edge-id="shareable-relation"/,
      mode,
    )
...
test("relationship ids stay optional and duplicate ids fail closed in the shared zero-install path", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    duplicate[CASES[mode].collection][1].id =
      duplicate[CASES[mode].collection][0].id
...
test("the viewer restores and copies stable relation links without exposing numeric keys", () => {
  ...
  assert.match(
    html,
    /target\.setAttribute\('data-relationship-id', record\.id\)/,
  )
  assert.match(
    html,
    /button\.setAttribute\('data-relationship-id', relationship\.id\)/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /inspectRelationshipById\(relation, \{ updateUrl: false, toggle: false \}\)/,
  )
  assert.match(
    html,
...
test("runtime overlays drop durable edge ids while canonical SVG keeps authored identity", () => {
  ...
  assert.doesNotMatch(
    canonicalSvg(html),
    /data-relationship-hit-overlay|data-relationship-id=/,
  )
  assert.ok(
    (html.match(/clone\.removeAttribute\('data-edge-id'\)/g) || []).length >= 6,
  )
