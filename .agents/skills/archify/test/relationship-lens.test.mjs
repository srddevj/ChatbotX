...
test("all typed renderers expose named, stable relationships without changing geometry", () => {
  for (const [mode, config] of Object.entries(CASES)) {
    ...
    const keys = new Set(
      Array.from(
        diagram.matchAll(/data-edge-key="(\d+)"/g),
        (match) => match[1],
      ),
    )
    ...
    assert.match(
      diagram,
      /data-node-id="[^"]+" data-node-label="[^"]+" tabindex="0"/,
      mode,
    )
  }
})
...
test("relationship lens groups incoming, outgoing, and self-loop paths and follows neighbors", () => {
  ...
  assert.match(
    html,
    /id="relationship-lens-list" aria-label="Connected relationships"/,
  )
...
test("relationship preview precisely links pointer and keyboard rows to an edge and its endpoints", () => {
  ...
  assert.match(
    html,
    /pinnedRelationship \|\| focusedRelationship \|\| hoveredRelationship/,
  )
  assert.doesNotMatch(
    diagram,
    /data-relationship-preview(?:-active|-node|-source|-target)?=/,
  )
})
...
test("relationship lens is keyboard navigable, mobile-pinned, and excluded from embed and print", () => {
  ...
  assert.match(
    html,
    /\.relationship-lens-row:not\(\[data-preview-active="true"\]\)/,
  )
  ...
  assert.match(
    html,
    /visibleTop = Math\.max\(padding, -containerRect\.top \+ padding\)/,
  )
  assert.match(
    html,
