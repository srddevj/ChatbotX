...
test("all renderers inherit one viewer-only Live/Still Motion Governor", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="btn-motion"[^>]+hidden[^>]+aria-label="Pause motion"/,
      mode,
    )
    ...
    assert.doesNotMatch(
      svgBlock(html),
      /data-motion-(?:capable|owner)|motion-control/,
      mode,
    )
  }
})
...
test("static artifacts stay truly still while trace artifacts opt into ambient motion", () => {
  ...
  assert.match(
    html,
    /if \(!capable\) \{[\s\S]*?btn\.hidden = true;[\s\S]*?capable: false/,
  )
  assert.match(
    html,
...
test("reader pause is persistent, explicit, and reduced-motion aware", () => {
  ...
  assert.match(
    html,
    /Archify\.guidedViews\.isPlaying\(\)[\s\S]*?Archify\.guidedViews\.pause\(\)/,
  )
...
test("strong semantic intent receives the single motion budget", () => {
  ...
  assert.match(
    html,
    /data-relationship-preview-active'\)\) return 'relationship'/,
  )
  ...
  assert.match(
    html,
    /new MutationObserver\(function \(\) \{ publishOwner\(\); \}\)/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /function clearClaim\(preempted\)[\s\S]*?try \{ cleanup\(\); \} catch/,
  )
  assert.match(
    html,
    /function claim\(next, cleanup\)[\s\S]*?clearClaim\(true\)/,
  )
...
test("motion control is mobile-contained, embed-safe, and export-neutral", () => {
  ...
  assert.doesNotMatch(
    svgBlock(html),
    /btn-motion|data-motion=|data-motion-owner/,
  )
})
