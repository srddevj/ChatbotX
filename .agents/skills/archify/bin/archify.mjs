  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--quality") {
      ...
      if (!quality || quality.startsWith("--")) {
        fail("--quality requires standard or showcase.")
      }
      index += 1
      continue
    }
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--repo-root") {
      ...
      if (!repoRoot || repoRoot.startsWith("--")) {
        fail("--repo-root requires a repository path.")
      }
      index += 1
      continue
    }
...
function checkerDiagnostics(checker) {
  ...
  for (const check of checker?.checks || []) {
    if (check.ok || COMPOSITION_CHECKS.has(check.name)) {
      continue
    }
    diagnostics.push(
      diagnostic({
...
function reportCompareFailure({
  ...
  if (json) {
    ...
    console.error(formatDiagnostics(error, receipt.diagnostics))
  }
...
function commandRender(args) {
  ...
  if (unknown.length) {
    fail(`Unknown render option "${unknown[0]}".`)
  }
  const [type, input, output] = repoArgs.rest
  if (!(type && input) || repoArgs.rest.length > 3) {
    fail(usage())
  }
...
async function commandDeliver(args) {
  ...
  if (unknown.length) {
    fail(`Unknown deliver option "${unknown[0]}".`)
  }
...
async function commandPreview(args) {
  ...
  if (unknown.length) {
    fail(`Unknown preview option "${unknown[0]}".`)
  }
  const positional = repoArgs.rest.filter((arg) => !knownOptions.has(arg))
...
async function commandVisualCheck(args) {
  ...
  if (unknown.length) {
    fail(`Unknown visual-check option "${unknown[0]}".`, 1)
  }
  ...
  if (json) {
    ...
    if (result.receipt.error) {
      console.error(result.receipt.error)
    }
  }
...
async function commandDoctor() {
  ...
  const problems = []
  if (nodeFailed) {
    problems.push("Node.js 18 or newer is required")
  }
  if (missingFiles) {
    problems.push(
...
async function commandBrands(args) {
  ...
  if (unknown.length) {
    fail(`Unknown brands option "${unknown[0]}".`)
  }
...
async function commandMigrate(args) {
  ...
  const reportMigrationFailure = ({ status = 1, ...details }) => {
    ...
    if (options.json) {
      ...
      console.error(formatDiagnostics(report.error, report.diagnostics))
    }
    process.exitCode = status
  }
  try {
...
function commandValidate(args) {
  ...
  if (unknown.length) {
    fail(`Unknown validate option "${unknown[0]}".`)
  }
  const json = args.includes("--json")
