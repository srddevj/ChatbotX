...
for (const type of diagramTypes) {
  const schema = JSON.parse(
    fs.readFileSync(path.join(schemasDir, `${type}.schema.json`), "utf8"),
  )
...
if (!validatorCode.includes(ajvUcs2Import)) {
  throw new Error(
    "AJV standalone output no longer contains the expected ucs2length helper",
  )
}
validatorCode = validatorCode.replaceAll(ajvUcs2Import, inlineUcs2Length)
if (validatorCode.includes("require(")) {
  throw new Error(
    "AJV standalone output contains an unexpected runtime dependency",
  )
}
...
if (process.argv.includes("--check")) {
  ...
  if (current !== generated) {
    console.error(
      "generated validators are stale — run npm run generate:validators",
    )
    process.exit(1)
  }
} else {
