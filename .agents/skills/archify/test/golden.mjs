...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    const checked = fs.readFileSync(
      path.join(repoRoot, "examples", golden),
      "utf8",
    )
    const packaged = fs.readFileSync(
      path.join(skillRoot, "examples", golden),
      "utf8",
    )
    check(
      `${mode}: ${golden}`,
...
// ---------------------------------------------------------------------------
console.log(
  "schema enforcement (invalid JSON must fail with a path-prefixed message)",
)

function expectFailure(name, mode, mutate, expectInMessage) {
  const base = JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "examples", GOLDEN.find(([m]) => m === mode)[1]),
      "utf8",
    ),
  )
  mutate(base)
...
// ---------------------------------------------------------------------------
console.log(
  "template freshness (architecture example must carry the current template)",
)
...
const webApp = fs.readFileSync(
  path.join(repoRoot, "examples/web-app.html"),
  "utf8",
)
...
for (const tag of ["style", "script"]) {
  ...
  const w = blocks(webApp, tag).filter(
    (b) => !b.includes("Sample Web App") && isTemplateOwned(b),
  )
  check(
...
const lock = JSON.parse(
  fs.readFileSync(path.join(skillRoot, "package-lock.json"), "utf8"),
)
check(
...
for (const readmeName of ["README.md", "README_EN.md", "README_ZH.md"]) {
  ...
  check(
    ...
    badgeVersions.length > 0 &&
      badgeVersions.every((version) => version === pkg.version),
...
const landingVersions = [
  ...landingPage.matchAll(
    /\bv\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?\b/g,
  ),
].map((match) => match[0])
check(
  "GitHub Pages version labels match package.json",
  landingVersions.length > 0 &&
    landingVersions.every((v) => v === `v${pkg.version}`),
