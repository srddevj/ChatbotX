...
async function waitForState(url, predicate, message, timeoutMs = 12_000) {
...
test("preview: invalid candidates preserve the last verified artifact and repair automatically", {
  timeout: 30_000,
}, async () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-last-good-"),
  )
  ...
  try {
    ...
    const firstArtifact = await (
      await fetch(new URL("/artifact.html", preview.url))
    ).text()
    ...
    assert.equal(
      sha256(output),
      firstSha,
      "deleted input replaced the last verified output",
    )
    ...
    assert.equal(
      sha256(output),
      firstSha,
      "invalid input replaced the last verified output",
    )
    assert.equal(
    ...
    assert.match(
      schemaFailed.failure.message,
      /\/components\/0.*additional properties/i,
    )
    assert.doesNotMatch(
      schemaFailed.failure.message,
      /file:\/\/|\/Users\/|node:internal/,
    )
    assert.equal(
      sha256(output),
      firstSha,
    ...
    assert.equal(
      (await rawRequest(preview.url, { method: "POST" })).status,
      405,
    )
    assert.equal(
      (await rawRequest(preview.url, { pathname: "/../../etc/passwd" })).status,
      404,
    )
    assert.equal(
      (await rawRequest(preview.url, { hostHeader: "example.com" })).status,
      403,
    )
  } finally {
    await preview.stop()
...
test("preview: content digests suppress identical writes and a burst publishes only its stable tail", {
  timeout: 30_000,
}, async () => {
  ...
  try {
    ...
    state = await waitForState(
      preview.url,
      (candidate) =>
        candidate.status === "verified" && candidate.generation === 2,
      "semantically identical source did not settle",
    )
    assert.equal(
      state.revision,
      1,
      "identical artifact bytes triggered a browser revision",
    )
    ...
    state = await waitForState(
      preview.url,
      (candidate) =>
        candidate.status === "verified" && candidate.revision === 2,
...
test("preview: a superseded slow candidate can never become a published revision", {
  timeout: 30_000,
}, async () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-latest-wins-"),
  )
  const input = path.join(tmp, "diagram.json")
  const output = path.join(tmp, "diagram.html")
...
`,
  ...
  try {
    ...
    assert.equal(
      state.revision,
      1,
      "superseded generation was published before the latest one",
    )
    const artifact = await (
      await fetch(new URL("/artifact.html", preview.url))
    ).text()
...
test("preview: each delivery reads the immutable bytes bound to its observed digest", {
  timeout: 30_000,
}, async () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-snapshot-"),
  )
  const input = path.join(tmp, "diagram.json")
  const output = path.join(tmp, "diagram.html")
...
`,
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(readMarker),
      "fake delivery never read its generation input",
    )
    fs.writeFileSync(input, JSON.stringify({ title: "Source A" }))
    ...
    const state = await waitForState(
      preview.url,
      (candidate) =>
        candidate.status === "verified" && candidate.revision === 1,
      "snapshot generation did not verify",
    )
    assert.equal(
      state.generation,
      1,
      "an unobserved A → B → A edit started a second generation",
    )
...
test("preview: commit rechecks the live digest when watcher and poll have not seen a newer save", {
  timeout: 10_000,
}, async () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-commit-race-"),
  )
  const input = path.join(tmp, "diagram.json")
  const output = path.join(tmp, "diagram.html")
...
test("preview: stopping drains an active delivery without publishing it", {
  timeout: 30_000,
}, async () => {
  ...
  fs.writeFileSync(
    input,
    JSON.stringify({ title: "Do not publish after stop" }),
  )
  fs.writeFileSync(
    deliveryCli,
    `
...
`,
  ...
  await preview.stop()
  assert.ok(
    Date.now() - stoppedAt >= 250,
    "preview did not drain the active delivery",
  )
...
test("preview: stopping has a bounded kill path for a delivery that never exits", {
  timeout: 5000,
}, async () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-hung-stop-"),
  )
  const input = path.join(tmp, "diagram.json")
  const output = path.join(tmp, "diagram.html")
...
`,
  ...
  await preview.stop()
  assert.ok(
    Date.now() - stoppedAt < 1000,
    "hung delivery kept preview shutdown open",
  )
  assert.equal(fs.readFileSync(output, "utf8"), prior)
  await assert.rejects(fetch(preview.url))
  assert.deepEqual(
...
test("preview: checker failures keep their actionable detail instead of a generic stage only", {
  timeout: 30_000,
}, async () => {
...
`,
  ...
  try {
    ...
    assert.match(
      ...
      /found 2 <svg> blocks; expected exactly one/,
    )
  } finally {
    await preview.stop()
  }
...
test("preview: all five typed renderers reach a verified first revision", {
  timeout: 60_000,
}, async () => {
  const cases = {
  ...
  for (const [type, example] of Object.entries(cases)) {
    const tmp = fs.mkdtempSync(
      path.join(os.tmpdir(), `archify-preview-${type}-`),
    )
    ...
    try {
      ...
      assert.equal(
        state.lastVerified.checksPassed,
        state.lastVerified.checkCount,
        type,
      )
...
test("preview: polling continues after an asynchronous watcher error", {
  timeout: 30_000,
}, async (t) => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-preview-watch-error-"),
  )
  ...
  try {
    ...
    watcher.emit(
      "error",
      Object.assign(new Error("watch limit reached"), { code: "EMFILE" }),
    )
  ...
  assert.equal(
    closeCount,
    1,
    "shutdown must not close the failed watcher again",
  )
  await assert.rejects(fetch(preview.url))
})
