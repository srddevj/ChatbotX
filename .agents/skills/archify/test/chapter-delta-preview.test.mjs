...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-chapter-delta-preview-"),
)
...
test("all five renderers inherit one viewer-only static Chapter Delta Preview", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /data-chapter-preview|data-chapter-preview-role/,
      mode,
    )
  }
})
...
test("exact stable-ID set math powers truthful counts and the existing handoff", () => {
  ...
  for (const file of PROOF_CASES) {
    const doc = JSON.parse(
      fs.readFileSync(path.join(skillRoot, "examples", file), "utf8"),
    )
    const views = doc.meta.views
    for (let index = 1; index < views.length; index += 1) {
      adjacent += 1
      if (delta(views[index - 1].focus, views[index].focus).stay.length) {
        shared += 1
      }
    }
  }
...
test("pointer and keyboard inspect while touch and native activation still commit directly", () => {
  ...
  assert.match(
    html,
    /chapterList\.addEventListener\('click',[\s\S]*?activateById/,
  )
...
test("latest intent, stronger owners, playback, and lifecycle cleanup remain bounded", () => {
  ...
  assert.match(
    html,
    /data-relationship-preview-active'[\s\S]*?data-intent-trace-active/,
  )
  ...
  assert.match(
    html,
    /beforeprint'[\s\S]*?clearChapterPreview[\s\S]*?settleHandoff/,
  )
})
...
test("mobile, Still, embed, print, and canonical exports keep the preview viewer-only", () => {
  ...
  assert.match(
    html,
    /prefers-reduced-motion: reduce[\s\S]*?svg\[data-chapter-preview\]/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-chapter-preview-role\]'\)/,
  )
