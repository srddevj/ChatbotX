...
test("export menu has grouped, single-column rows and a zoom-safe width", () => {
  const sectionCss =
    template.match(/\.export-menu-section \{[\s\S]*?\}/)?.[0] || ""
  assert.match(
    template,
...
test("mobile menus share one viewport-safe placement and disabled exports remain explicit", () => {
  ...
  assert.match(
    template,
    /\.toolbar \.export-menu button:disabled \{[\s\S]*?cursor: not-allowed;/,
  )
...
test("diagram view dock stays compact on desktop and touch-safe on narrow screens", () => {
  ...
  assert.match(
    template,
    /\.diagram-nav button \{[\s\S]*?min-width: 2rem;[\s\S]*?height: 2rem;/,
  )
  assert.match(
    template,
