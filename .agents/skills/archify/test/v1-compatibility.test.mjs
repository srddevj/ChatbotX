...
function validate(mode, doc) {
  ...
  try {
    ...
    return {
      code: error.status ?? 1,
      stderr: String(error.stderr || error.stdout || ""),
    }
  }
}
...
for (const [mode, filename] of Object.entries(OFFICIAL_V1_EXAMPLES)) {
  test(`official v1 ${mode} baseline remains renderable and valid`, () => {
    const doc = JSON.parse(
      fs.readFileSync(
        path.join(__dirname, "fixtures/v1-baseline", filename),
        "utf8",
      ),
    )
...
test("quality-profile lifecycle keeps the checked-in authored via authoritative", () => {
  ...
  const transition = doc.transitions.find(
    ({ id }) => id === "approval-cancelled",
  )
...
test("legacy v1 architecture auto viewBox accommodates all seven implicit auto legend kinds", () => {
  ...
  const viewBox = svg
    ...
    ?.slice(1)
    .map(Number)
  const baselines = [...svg.matchAll(/data-legend-baseline="([\d.]+)"/g)].map(
    (match) => Number(match[1]),
  )
  assert.deepEqual(
    [...svg.matchAll(/data-legend-semantic-kind="([^"]+)"/g)].map(
      (match) => match[1],
    ),
    types,
  )
  ...
  assert.ok(
    viewBox[0] > 160,
    "auto viewBox should widen for its widest measured legend entry",
  )
...
function narrowExplicitViewBoxDocuments() {
  ...
  return {
    ...
    sequence: {
      ...
      messages: ["emphasis", "return", "security", "dashed", "default"].map(
        (variant, index) => ({
    ...
    dataflow: {
      ...
      flows: ["emphasis", "security", "dashed", "default"].map(
        (variant, index) => ({
          from: `in_${index}`,
          to: `out_${index}`,
          label: variant,
  ...
  for (const [mode, doc] of Object.entries(narrowExplicitViewBoxDocuments())) {
    ...
    if (mode === "lifecycle") {
      assert.match(
        svg,
        />Legend</,
        "a fitting implicit legend should remain visible",
      )
