...
function run(type, doc, command = "render") {
  ...
  return {
    ...result,
    html:
      result.status === 0 && command === "render"
...
test("Dataflow database node facts are interactive while flow variants stay visual-only", () => {
  ...
  assert.deepEqual(attrValues(databaseSvg, "data-legend-semantic-kind"), [
    "database",
    "default",
  ])
...
test("Issue #52 dataflow and lifecycle reproductions publish truthful default legends", () => {
  ...
  assert.deepEqual(attrValues(dataflow, "data-legend-semantic-kind"), [
    "default",
  ])
  assert.doesNotMatch(
    dataflow,
    /policy \/ PII|async batch|primary data|data store/i,
  )
  ...
  assert.deepEqual(attrValues(lifecycleLegend, "data-legend-kind"), [
    "start",
    "active",
    "success",
  ])
...
test("visibility overrides apply after auto/all and empty legends leave no chrome", () => {
  ...
  for (const [type, kinds] of Object.entries(cases)) {
    ...
    const expected = AUTO_KINDS[type]
      ...
      .sort(
        (left, right) =>
          CATALOGS[type].indexOf(left) - CATALOGS[type].indexOf(right),
      )
  ...
  for (const type of Object.keys(FIXTURES)) {
    const entries = Object.fromEntries(
      AUTO_KINDS[type].map((kind) => [kind, { visible: false }]),
    )
  ...
  for (const type of Object.keys(FIXTURES)) {
    ...
    if (["architecture", "workflow", "lifecycle"].includes(type)) {
      assert.match(
        svg,
        new RegExp(
          `data-legend-kind="${kind}"[^>]+data-legend-label="${label}"`,
        ),
        type,
      )
    } else {
...
test("label overrides preserve stable kinds and exact Semantic Legend boundaries", () => {
  ...
  const baselineSvg = canonicalSvg(
    render("architecture", FIXTURES.architecture),
  )
  assert.deepEqual(
    attrValues(svg, "data-node-id"),
    attrValues(baselineSvg, "data-node-id"),
  )
  assert.deepEqual(
    attrValues(svg, "data-node-kind"),
    attrValues(baselineSvg, "data-node-kind"),
  )
  assert.deepEqual(
    attrValues(svg, "data-edge-from"),
    attrValues(baselineSvg, "data-edge-from"),
  )
  ...
  assert.deepEqual(attrValues(svg, "data-legend-kind"), [
    "frontend",
    "database",
  ])
  assert.match(
...
test("strict per-renderer schemas reject malformed legend contracts with path-prefixed errors", () => {
  ...
  const cases = [
    ...
    [
      (type) => ({ entries: { [known[type]]: { label: "" } } }),
      "/meta/legend/entries/",
    ],
    [
      (type) => ({ entries: { [known[type]]: { visible: "yes" } } }),
      "/meta/legend/entries/",
    ],
    [
      (type) => ({ entries: { [known[type]]: { color: "#fff" } } }),
      "/meta/legend/entries/",
    ],
  ...
  for (const type of Object.keys(FIXTURES)) {
    for (const [legendOrFactory, pathPrefix] of cases) {
      const legend =
        typeof legendOrFactory === "function"
          ? legendOrFactory(type)
          : legendOrFactory
      const failure = validateFailure(type, withLegend(type, legend))
      assert.ok(
        failure.diagnostics.some((diagnostic) =>
          diagnostic.subject.path.startsWith(pathPrefix),
        ),
...
test("measured legends fail explicitly instead of wrapping into diagram content", () => {
  ...
  const entries = Object.fromEntries(
    CATALOGS.workflow.map((kind) => [kind, { label }]),
  )
  const failure = validateFailure(
    "workflow",
    withLegend("workflow", { mode: "all", entries }),
  )
  const diagnostic = failure.diagnostics.find(
    (entry) => entry.code === "legend/vertical-overflow",
  )
  ...
  const routedEntries = Object.fromEntries(
    CATALOGS.architecture.map((kind) => [
      kind,
      { label: `Long ${kind} convention` },
    ]),
  )
  const routed = withLegend("architecture", {
    mode: "all",
...
test("explicit Architecture viewBox rejects legend title rectangles that overlap content", () => {
  ...
  assert.equal(diagnostic.subject.path, "/meta/legend")
  assert.ok(
    diagnostic.evidence.requiredTopY < diagnostic.evidence.availableTopY,
  )
})
...
test("a single unfit label fails with a path-specific width diagnostic", () => {
  ...
  const diagnostic = failure.diagnostics.find(
    (entry) => entry.code === "legend/label-too-wide",
  )
  assert.ok(diagnostic, JSON.stringify(failure.diagnostics))
  assert.equal(diagnostic.subject.path, "/meta/legend/entries/frontend/label")
  assert.ok(
    diagnostic.evidence.measuredWidthPx > diagnostic.evidence.availableWidthPx,
  )
})
...
test("measured legend rows share baselines and stay within the viewBox for localized labels", () => {
  ...
  const viewBox = attrValues(svg.match(/<svg\b[^>]*>/)?.[0] || "", "viewBox")[0]
    ...
    .map(Number)
  const tags = [
    ...svg.matchAll(/<g\b[^>]*data-legend-semantic-kind="[^"]+"[^>]*>/g),
  ].map((match) => match[0])
  ...
  for (const row of rows.values()) {
    ...
    for (let index = 1; index < sorted.length; index += 1) {
      assert.ok(
        sorted[index - 1].x + sorted[index - 1].width <= sorted[index].x,
      )
    }
  }
})
