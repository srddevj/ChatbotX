...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-story-director-strip-"),
)
...
test("all five renderers inherit one viewer-only Story Director Strip", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /guided-story-caption|data-story-caption/,
      mode,
    )
  }
})
...
test("captions derive only authored edge labels and existing node facts", () => {
  ...
  assert.doesNotMatch(
    template,
    /inferred relationship|likely transition|calls service/,
  )
})
...
test("Presentation playback removes secondary chrome without hiding Pause or navigation", () => {
  ...
  assert.match(
    template,
    /\.guided-views\[data-playing="true"\] \.guided-view-beat-link/,
  )
  assert.match(
    template,
    /\.guided-views\[data-playing="true"\] \.guided-view-all/,
  )
