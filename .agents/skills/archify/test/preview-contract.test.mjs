...
test("preview contract: the skill keeps live preview explicit, desktop-only, and last-good", () => {
  assert.match(
    delivery,
    /archify\.mjs preview <type> <input>\.json <output>\.html/,
  )
  ...
  assert.match(
    delivery,
    /CI, unattended agents, remote sharing, or mobile use/i,
  )
  assert.match(
    delivery,
    /must never enter the generated artifact or any export/i,
  )
})
