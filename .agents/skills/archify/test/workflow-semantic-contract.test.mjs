...
test("semanticChecks accepts required reachability through intermediate nodes without changing SVG bytes", () => {
  ...
  const baselineResult = compileWorkflow({
    workflow: baseline,
    qualityProfile: "standard",
  })
  const constrainedResult = compileWorkflow({
  ...
  assert.equal(
    baselineResult.ok,
    true,
    JSON.stringify(baselineResult.diagnostics, null, 2),
  )
  assert.equal(
    constrainedResult.ok,
    true,
    JSON.stringify(constrainedResult.diagnostics, null, 2),
  )
...
test("semanticChecks rejects a required path that is not reachable in authored direction", () => {
  ...
  assert.ok(
    result.diagnostics.some(
      (diagnostic) => diagnostic.code === "workflow/required-path",
    ),
    JSON.stringify(result.diagnostics, null, 2),
  )
})
...
test("semanticChecks reports unknown contract node ids before graph analysis", () => {
  ...
  assert.equal(
    result.diagnostics[0].subject.path,
    "/semanticChecks/requiredEdges/0/from",
  )
})
