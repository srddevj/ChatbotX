...
test("cli: doctor identifies an incomplete installation", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [path.join(incompleteBin, "archify.mjs"), "doctor"],
    {
      cwd: incompleteRoot,
      encoding: "utf8",
    },
  ...
  assert.match(
    result.stdout,
    /\[missing\] workflow renderer, schema, and example/,
  )
  assert.match(
    result.stderr,
    /Archify is not ready: \d+ required files? missing\./,
  )
})
...
test("cli: examples renders from an installed skill", () => {
  ...
  for (const output of [
    ...
    assert.equal(
      fs.existsSync(path.join(installedRoot, "examples", output)),
      true,
      output,
    )
  }
...
test("cli: guide detects Chinese and explains the recommendation boundary", () => {
  ...
  assert.equal(result.status, 0, result.stderr)
  assert.match(result.stdout, /推荐: 事件流拓扑 {2}\[dataflow\]/)
...
test("cli: demo defaults to the current directory", () => {
  ...
  assert.equal(result.status, 0, result.stderr)
  assert.equal(
    fs.existsSync(path.join(workingDirectory, "archify-demo.html")),
    true,
  )
})
...
test("cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable", () => {
  ...
  assert.equal(
    fs.existsSync(out.replace(/\.html$/, ".visual-check.json")),
    true,
  )
})
...
test("cli: visual-check keeps automated and perceptual claims separate on input failure", () => {
  const result = run([
    "visual-check",
    path.join(tmp, "missing-browser-evidence.html"),
  ])
...
test("cli: deliver --open launches only the committed absolute artifact as one argument", {
  ...
  const input = path.join(skillRoot, "examples/agent-tool-call.workflow.json")
  const result = run(["deliver", "workflow", input, out, "--open", "--json"], {
    env: fake.env,
  })
  ...
  assert.equal(
    invocation.existed,
    true,
    "the opener must run after the atomic commit",
  )
...
test("cli: opener failure does not invalidate a verified delivery or pollute json stdout", {
  ...
  const result = run(
    ["deliver", "architecture", input, out, "--open", "--json"],
    { env: fake.env },
  )
...
test("cli: deliver preserves the renderer default output contract", () => {
  ...
  const result = run(["deliver", "architecture", input, "--json"], {
...
test("cli: deliver works from an installed skill without node_modules", () => {
  ...
  for (const [label, type, example] of cases) {
    ...
    assert.equal(
      extracted.direct.length,
      1,
      `${label}: expected one delivered SVG`,
    )
    assert.doesNotThrow(
...
test("cli: deliver XML guard parses markup instead of scanning attribute-like text", () => {
  ...
  assert.throws(
    () =>
      parseXml(
        '<svg xmlns="http://www.w3.org/2000/svg" data-node-label></svg>',
      ),
...
test("cli: preview runs from an installed skill without node_modules and exits cleanly", {
  ...
  assert.ok(
    previewUrl,
    `preview URL missing; stdout=${stdout}; stderr=${stderr}`,
  )

  let state
  while (Date.now() - started < 15_000) {
    state = await fetch(new URL("/state", previewUrl)).then((response) =>
      response.json(),
    )
  ...
  assert.deepEqual(
    fs
      ...
      .filter((name) => name.startsWith(".archify-preview-")),
    [],
  )
})
...
test("cli: deliver preserves the previous artifact when the final check fails", () => {
  ...
  fs.writeFileSync(
    ...
    template.replace(
      "</body>",
      '<svg aria-label="accidental second svg"></svg>\n</body>',
    ),
  )

  const input = path.join(installedRoot, "examples/web-app.architecture.json")
  const out = path.join(tmp, "preserved-delivery.html")
  const trustedPriorArtifact =
    "<!doctype html><title>trusted prior artifact</title>\n"
  ...
  assert.ok(
    failure.diagnostics[0].supportedFixes.some((fix) =>
      fix.includes("exactly one diagram SVG"),
    ),
  )
  assert.equal(
    failure.checker.checks.find((entry) => entry.name === "single_svg").ok,
    false,
  )
  ...
  assert.deepEqual(
    fs
      ...
      .filter((name) => name.includes(".archify-delivery-")),
    [],
  )
})
...
test("cli: invalid source output metadata still fails inside the renderer", () => {
  ...
  const trustedPriorArtifact =
    "<!doctype html><title>metadata did not replace me</title>\n"
  fs.writeFileSync(out, trustedPriorArtifact)
...
test("cli: --quality overrides the source profile for render, validate, and deliver", () => {
  ...
  assert.equal(
    JSON.parse(delivered.stdout).validation.compositionProfile,
    "standard",
  )
})
...
test("cli: inspect remains architecture-only while workflow uses validate --layout-json", () => {
  ...
  assert.match(
    result.stderr,
    /inspect is currently supported for architecture diagrams only/,
  )
  assert.equal(result.stdout, "")
})
...
test("render rejects a mistyped option instead of writing a file named after it", () => {
  ...
  fs.copyFileSync(
    path.join(skillRoot, "../examples/archify-repo.architecture.json"),
    spec,
  )

  // Without the guard this wrote a 600KB file literally named `--json` and
  // never wrote out.html, exiting 0.
  const result = run(["render", "architecture", spec, "--json", "out.html"], {
    cwd: dir,
  })
...
test("render rejects an extra positional argument", () => {
  ...
  const result = run(
    ["render", "architecture", spec, "out.html", "extra.html"],
    { cwd: dir },
  )
