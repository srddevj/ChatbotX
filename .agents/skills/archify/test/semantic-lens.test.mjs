...
test("all typed renderers inherit one viewer-only Semantic Lens", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /svg\.querySelectorAll\('\[data-node-id\]\[data-node-kind\]'\)/,
      mode,
    )
    assert.doesNotMatch(
      canonicalSvg(html),
...
test("Semantic Lens derives honest kind counts and compares at most two roles", () => {
  ...
  assert.match(
    html,
    /fromKind === selectedKinds\[0\] && toKind === selectedKinds\[1\]/,
  )
  assert.match(
    html,
    /fromKind === selectedKinds\[1\] && toKind === selectedKinds\[0\]/,
  )
...
test("Semantic Lens preserves Reading Depth, mobile containment, print, embed, and export boundaries", () => {
  ...
  assert.match(
    html,
    /\[data-lens-match\], \[data-lens-selected\], \[data-lens-peer\]/,
  )
  assert.match(
    html,
...
test("Semantic Lens docks away from selected nodes without breaking mobile containment", () => {
  ...
  assert.match(
    html,
    /selectedKinds\.indexOf\(byId\[id\]\.getAttribute\('data-node-kind'\)/,
  )
