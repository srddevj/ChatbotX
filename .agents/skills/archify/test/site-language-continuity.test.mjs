...
import { DIAGRAM_TYPE_LABELS, DIAGRAM_TYPES } from "../../scripts/site-copy.mjs"
...
const chromePath =
  integrationEnabled && process.env.ARCHIFY_CHROME ? findChrome() : null
...
function loadRuntime({
  ...
  return {
    language: window.ArchifySiteLanguage,
    values,
...
test("site language runtime normalizes one entry parameter into one durable preference", () => {
  ...
  assert.doesNotMatch(
    source,
    /navigator\.language|detectBrowserLanguage|select\s*:/,
  )
})
...
test("custom site builders emit every shared site asset and preserve entry, navigation, selection, and refresh state", {
  skip: integrationEnabled
    ? false
    : "Run through the serialized site integration gate.",
}, () => {
    ...
    for (const build of builds) {
      ...
      for (const asset of ["site-language.js", "site-navigation.css"]) {
        ...
        assert.ok(
          ...
          `${build.script}: ${asset} missing from custom output`,
        )
        assert.equal(
          fs.readFileSync(emitted, "utf8"),
          fs.readFileSync(canonicalAsset, "utf8"),
        )
      }
    }
...
test("all site pages consume one language runtime and one navigation contract", () => {
  ...
  for (const relative of pages) {
    ...
    assert.match(
      html,
      /ArchifySiteLanguage\.read\(/,
      `${relative}: shared language read missing`,
    )
    assert.match(
      html,
    ...
    assert.match(
      html,
      /href="guide\.html"/,
      `${relative}: Guide navigation missing`,
    )
    assert.match(
      html,
      /href="gallery\.html"/,
      `${relative}: Proof Lab navigation missing`,
    )
    assert.match(
      html,
      /href="start\.html"/,
      `${relative}: Start navigation missing`,
    )
    assert.match(
      html,
      /class="btn btn-primary nav-cta"/,
      `${relative}: install action missing`,
    )
    assert.doesNotMatch(
      html,
      /(?:^|\s)nav\s*\{/,
...
test("site page identity paths localize with the selected language", () => {
  const pages = [
    {
      paths: ["scripts/guide-template.html", "docs/guide.html"],
      en: "/ guide",
      zh: "/ 场景指南",
    },
    {
    ...
    {
      paths: ["scripts/start-template.html", "docs/start.html"],
      en: "/ start",
      zh: "/ 快速上手",
    },
  ]
...
test("proof gallery type filters localize with the selected language", () => {
  ...
  const template = fs.readFileSync(
    path.join(repoRoot, "scripts/gallery-template.html"),
    "utf8",
  )
...
test("scenario guide type filters use consistent Chinese diagram names", () => {
  const template = fs.readFileSync(
    path.join(repoRoot, "scripts/guide-template.html"),
    "utf8",
  )
...
test("real Chrome preserves language through entry, navigation, selection, refresh, and consistent navigation chrome", {
  skip: chromePath
    ? false
    : "Set ARCHIFY_CHROME to run the real site regression.",
  timeout: 60_000,
}, async () => {
  ...
  try {
    ...
    await evaluate(
      browser,
      sessionId,
      "localStorage.setItem('archify-guide-language', 'zh')",
    )
    await navigate(browser, sessionId, `${baseUrl}/index.html`)
    assert.deepEqual(
      await evaluate(
    ...
    assert.equal(
      await evaluate(browser, sessionId, "document.documentElement.lang"),
      "zh-CN",
    )
    assert.equal(
      await evaluate(
        browser,
        sessionId,
        'document.querySelector(".nav-logo-path").textContent',
      ),
      "/ 验证作品集",
    )
    assert.deepEqual(
    ...
    for (const receipt of desktopReceipts.slice(1)) {
      assert.deepEqual(receipt, desktopReceipts[0])
    }
