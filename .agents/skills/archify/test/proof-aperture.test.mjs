...
test("landing declares a truthful first-fold proof aperture in document order", () => {
  assert.match(
    landing,
    /<section class="hero" data-proof-aperture="first-fold">/,
  )
...
test("narrow viewport preserves a contained fallback without adding a mobile product surface", () => {
  ...
  assert.match(
    mobile,
    /\.hero-actions \.btn\s*\{\s*flex:1;\s*justify-content:center;\s*\}/,
  )
...
test("proof aperture remains one real eager sandboxed artifact with explicit user-selected identities", () => {
  assert.equal(
    (landing.match(/<iframe id="hero-proof-frame"/g) || []).length,
    1,
  )
  ...
  assert.doesNotMatch(
    landing,
    /setInterval\(|scrollIntoView\(|scroll-triggered|proof-carousel/,
  )
})
...
test("proof playback delegates reduced motion to the artifact and keeps deliberate-choice boundaries", () => {
  assert.match(
    landing,
    /renderProof\(tab\.dataset\.proof, \{ deliberate: true \}\)/,
  )
  assert.match(
    landing,
...
test("aperture uses normal flow and preserves reduced-motion boundaries", () => {
  ...
  assert.doesNotMatch(
    hero + proof,
    /position:absolute|transform:|top:-|margin-top:-|height:100vh/,
  )
  assert.match(landing, /@media\s*\(prefers-reduced-motion:\s*reduce\)/)
  assert.match(
    landing,
    /\.fade-up\s*\{\s*opacity:1!important;\s*transform:none!important;/,
  )
  assert.match(
    landing,
    /\.pulse-dot,\.proof-live::before\s*\{\s*animation:none!important;\s*\}/,
  )
})
