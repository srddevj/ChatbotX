...
function spawnBuildZip(outputPath, options = {}) {
  ...
  if (process.platform === "win32") {
    ...
    for (const bash of bashCandidates) {
      if (bash.includes("\\") && !fs.existsSync(bash)) {
        continue
      }
      const result = spawnSync(bash, [script, outputPath], {
        cwd,
        encoding: "utf8",
        ...rest,
      })
...
test("release prevents manifest preannouncement and smokes the exact archive before upload", () => {
  ...
  const annotatedTagGate = workflowStep(
    workflow,
    "Stable release tag must be annotated",
  )
  ...
  const freshness = workflowStep(
    workflow,
    "Committed zip must match the build (same gate as CI)",
  )
  const upload = workflowStep(
    workflow,
    "Create GitHub Release with the zip attached",
  )
  const followUp = workflowStep(
    workflow,
    "Record stable notifier publication follow-up",
  )
  ...
  assert.match(
    annotatedTagGate,
    /steps\.release-kind\.outputs\.prerelease == 'false'/,
  )
  assert.match(
    annotatedTagGate,
  ...
  assert.match(
    publicationOrder,
    /compareSemver\(published\.version, releasing\) >= 0/,
  )
  ...
  assert.match(
    smoke,
    /node scripts\/package-smoke\.mjs "\$package_root\/archify"/,
  )
...
test("an exact tag fetch restores an annotated object after a SHA-only checkout", () => {
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-release-tag-fetch-"),
  )
  ...
  try {
    ...
    assert.equal(
      runGit(source, ["config", "user.name", "Archify Test"]).status,
      0,
    )
    assert.equal(
    ...
    assert.equal(
      runGit(source, ["commit", "--quiet", "-m", "release fixture"]).status,
      0,
    )
    assert.equal(
      runGit(source, ["tag", "-a", "v1.0.0", "-m", "Release v1.0.0"]).status,
      0,
    )
    ...
    assert.equal(
      runGit(checkout, ["remote", "add", "origin", source]).status,
      0,
    )
    assert.equal(
      runGit(checkout, [
    ...
    assert.equal(
      runGit(checkout, ["cat-file", "-t", "refs/tags/v1.0.0"]).stdout.trim(),
      "commit",
    )
    ...
    assert.equal(
      runGit(checkout, ["rev-parse", "refs/tags/v1.0.0^{}"]).stdout.trim(),
      commit,
    )
  } finally {
...
test("CI binds a public notifier manifest to the Release asset, tagged archive, and tag tree build", () => {
  ...
  assert.match(
    job,
    /refs\/tags\/v\$\{manifest_version\}:refs\/tags\/v\$\{manifest_version\}/,
  )
  assert.match(
    job,
    /git show "v\$\{manifest_version\}:archify\.zip" > "\$tagged_archive"/,
  )
...
test("release tags with a SemVer prerelease are marked prerelease and never become latest", () => {
  ...
  const classifier = workflowStep(
    workflow,
    "Classify stable and prerelease tags",
  )
  const upload = workflowStep(
    workflow,
    "Create GitHub Release with the zip attached",
  ...
  assert.match(
    upload,
    /prerelease: \$\{\{ steps\.release-kind\.outputs\.prerelease \}\}/,
  )
  assert.match(
    upload,
    /make_latest: \$\{\{ steps\.release-kind\.outputs\.make_latest \}\}/,
  )
})
...
test("package smoke rejects every dependency or repository-only artifact", () => {
  ...
  for (const { relative, kind } of forbidden) {
    const fixture = fs.mkdtempSync(
      path.join(os.tmpdir(), "archify-package-gate-"),
    )
    try {
      ...
      const result = spawnSync(process.execPath, [packageSmoke, fixture], {
        encoding: "utf8",
      })
...
test("package smoke rejects a missing or modified distribution license", () => {
  const packageSmoke = path.join(repoRoot, "scripts", "package-smoke.mjs")
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-package-license-gate-"),
  )
  try {
    ...
    let result = spawnSync(process.execPath, [packageSmoke, staged], {
      encoding: "utf8",
    })
    assert.notEqual(result.status, 0, "missing LICENSE must fail package smoke")
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /packaged skill is missing LICENSE/,
    )
    ...
    assert.notEqual(
      result.status,
      0,
      "modified upstream notice must fail package smoke",
    )
    assert.match(
      `${result.stdout}\n${result.stderr}`,
    ...
    assert.notEqual(
      result.status,
      0,
      "truncated LICENSE must fail package smoke",
    )
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /must byte-match the repository LICENSE/,
    )
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true })
  }
...
test("package smoke rejects missing, modified, or incomplete third-party notices", () => {
  const packageSmoke = path.join(repoRoot, "scripts", "package-smoke.mjs")
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-package-notices-gate-"),
  )
  try {
    const staged = path.join(fixture, "archify")
    stageCleanSkill({ repoRoot, destination: staged })
    ...
    assert.notEqual(
      result.status,
      0,
      "modified notices must fail package smoke",
    )
    assert.match(
      `${result.stdout}\n${result.stderr}`,
    ...
    assert.notEqual(
      result.status,
      0,
      "incomplete notices must fail package smoke",
    )
    assert.match(
      `${result.stdout}\n${result.stderr}`,
    ...
    fs.copyFileSync(
      path.join(repoRoot, "LICENSE"),
      path.join(comparisonRoot, "LICENSE"),
    )
    ...
    fs.writeFileSync(
      path.join(comparisonRoot, "THIRD_PARTY_NOTICES.md"),
      synchronizedIncomplete,
    )
    ...
    assert.notEqual(
      result.status,
      0,
      "byte-identical incomplete notices must fail package smoke",
    )
    assert.match(
      `${result.stdout}\n${result.stderr}`,
...
test("package smoke increments an arbitrary-precision SemVer patch without Number coercion", () => {
  const scratch = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-package-bigint-version-"),
  )
...
canonicalZipTest(
  "package smoke rejects every dependency metadata field in a built package",
  () => {
    const fixture = fs.mkdtempSync(
      path.join(os.tmpdir(), "archify-built-package-gate-"),
    )
    try {
      ...
      const unzip = spawnSync("unzip", ["-q", archive, "-d", extracted], {
        encoding: "utf8",
      })
...
canonicalZipTest("built archives contain the embedded notifier runtime", () => {
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-notifier-package-gate-"),
  )
  try {
    const archive = path.join(fixture, "archify.zip")
...
canonicalZipTest(
  ...
  () => {
    const fixture = fs.mkdtempSync(
      path.join(os.tmpdir(), "archify-package-unmerged-"),
    )
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(skill, "scripts", "check-update.mjs"),
        "export {};\n",
      )
      fs.writeFileSync(
        path.join(skill, "scripts", "update-contract.mjs"),
        "export {};\n",
      )
      ...
      const theirs = git(["hash-object", "-w", "--stdin"], {
        input: "theirs\n",
      })
      for (const result of [base, ours, theirs]) {
      ...
      assert.equal(
        git(["update-index", "--index-info"], { input: indexInfo }).status,
        0,
      )
      ...
      const build = spawnSync(
        "bash",
        [path.join(scripts, "build-zip.sh"), archive],
        {
          cwd: fixture,
          encoding: "utf8",
        },
...
test("archive build rejects non-canonical Node versions before publishing output", {
  ...
  const outputRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-package-node-version-"),
  )
  try {
    const archive = path.join(outputRoot, "archify.zip")
...
canonicalZipTest(
  ...
  () => {
    const outputRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "archify-package-reproducible-"),
    )
    ...
    try {
      ...
      assert.ok(
        fs
          ...
          .equals(fs.readFileSync(path.join(repoRoot, "archify.zip"))),
...
test("CI tests the declared Node floor plus every maintained current lane", () => {
  ...
  for (const version of [18, 20, 22, 24]) {
    assert.ok(
      ...
      `test matrix must cover Node ${version}`,
    )
  }
  ...
  assert.match(
    packageSmokeJob,
    /os:\s*\[ubuntu-latest, macos-latest, windows-latest\]/,
  )
