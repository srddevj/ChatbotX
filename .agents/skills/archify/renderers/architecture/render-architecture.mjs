...
for (const [index, conn] of asArray(arch.connections).entries()) {
  if (!componentSteps.has(conn.from)) {
    componentSteps.set(conn.from, index)
  }
  if (!componentSteps.has(conn.to)) {
    componentSteps.set(conn.to, index + 1)
  }
}
for (const [index, c] of asArray(arch.components).entries()) {
  if (!componentSteps.has(c.id)) {
    componentSteps.set(c.id, index)
  }
}
...
function validateArchitecture() {
  ...
  for (const boundary of boundaries) {
    ...
    for (const component of components.values()) {
      if (!rectsOverlap(boundary.title, component)) {
        continue
      }
      problems.push(
  ...
  for (const rect of labelRects) {
    ...
    if (enforcesBoundaryTitleComposition) {
      for (const boundary of boundaries) {
        if (!rectsOverlap(boundary.title, rect)) {
          continue
        }
        problems.push(
...
function routeClearsComponents(conn, points, clearance = 2) {
  ...
  for (const component of components.values()) {
    if (endpointIds.has(component.id)) {
      continue
    }
...
function alignFacingPorts(conn, from, to, start, end, fromSide, toSide, ports) {
  ...
  if (!(fromSpread || toSpread) && hasExplicitSides) {
    return { start, end }
  }
...
function pathFor(conn) {
  if (pathCache.has(conn)) {
    return pathCache.get(conn)
  }
  const from = components.get(conn.from)
  const to = components.get(conn.to)
