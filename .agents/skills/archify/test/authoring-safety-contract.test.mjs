...
const schemaReadme = fs.readFileSync(
  path.join(skillRoot, "schemas", "README.md"),
  "utf8",
)
...
test("semantic relationship labels are preserved and deletion is not a geometry repair", () => {
  for (const [name, source] of [
    ...
    assert.match(
      source,
      /move the label[\s\S]*adjust the route or spacing[\s\S]*shorten/i,
      name,
    )
    assert.match(
      source,
    ...
    assert.match(
      source,
      /Omit only wording[\s\S]*fully implied by both endpoints/i,
      name,
    )
    assert.match(source, /Preserve every meaningful label/i, name)
    assert.match(
      source,
      /deleting it is not\s+a (?:geometry|spacing) repair/i,
      name,
    )
  }
})
...
test("deployment ownership stays explicit, fact-backed, and cannot be removed to pass", () => {
  ...
  assert.match(
    skill,
    /production deployment topology.*ownership.*fail-closed deployment review/i,
  )
...
test("visual-check stays a pending sidecar receipt instead of a polish claim", () => {
  ...
  assert.match(
    skill,
    /automated browser evidence[\s\S]*perceptual visual review/i,
  )
  ...
  assert.match(
    deliveryContract,
    /1440×900[\s\S]*1600×1000[\s\S]*1920×1080[\s\S]*2048×1320/,
  )
  assert.match(deliveryContract, /visualReview: "pending"/)
  assert.match(
    deliveryContract,
    /never changes.*delivered|without (?:rerendering or )?modifying/i,
  )
})
