...
test("all five renderers inherit native Route Journey controls outside canonical SVG", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="route-journey-prev"[^>]+aria-label="Previous route position"/i,
      mode,
    )
    assert.match(
      html,
    ...
    assert.match(
      html,
      /id="route-journey-next"[^>]+aria-label="Next route position"/i,
      mode,
    )
    assert.match(
      html,
...
test("a position owns its exact ordered incoming edge while the full route remains authored truth", () => {
  ...
  assert.match(
    template,
    /edge\.setAttribute\('data-route-journey-current', ''\)/,
  )
  assert.match(
    template,
  ...
  assert.doesNotMatch(
    template,
    /renderJourneyPulse\([\s\S]{0,120}querySelector\(.*data-edge-from/,
  )
})
...
test("route chips provide one roving tab stop, native activation, and manual ownership", () => {
  ...
  assert.match(
    template,
    /path\.addEventListener\('keydown'[\s\S]*?event\.key === 'ArrowRight'/,
  )
...
test("playback is explicit, finite, resumable, and never leaks position into the route URL", () => {
  ...
  assert.match(
    template,
    /generation !== journeyGeneration \|\| !journeyPlaying/,
  )
...
test("motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit", () => {
  ...
  assert.match(
    template,
    /window\.addEventListener\('beforeprint'[\s\S]*?pauseJourney/,
  )
  assert.match(
    template,
  ...
  assert.match(
    template,
    /\.route-probe\[data-route-dock="top"\] \{\s*top: 1rem;\s*bottom: auto;/,
  )
  assert.match(template, /function updateDocking\(\) \{\s*if \(panel\.hidden\)/)
  assert.doesNotMatch(
    template,
    /if \(panel\.hidden \|\| window\.innerWidth > 720\)/,
  )
  assert.match(
    template,
...
test("standalone export strips every journey attribute and transient pulse", () => {
  ...
  assert.match(
    template,
    /clone\.querySelectorAll\('\[data-route-journey-overlay\]'\)/,
  )
