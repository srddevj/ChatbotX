...
test("architecture compare classifies authored facts separately from geometry and presentation", () => {
  ...
  const checkout = receipt.changes.components.find(
    (change) => change.id === "checkout",
  )
  ...
  const queue = receipt.changes.components.find(
    (change) => change.id === "queue",
  )
  ...
  assert.deepEqual(authorization.changedFields, [
    "/from",
    "/fromSide",
    "/toSide",
    "/via",
  ...
  assert.deepEqual(
    receipt.changes.connections.find((change) => change.status === "added")
      .classifications,
    ["topology"],
  )

  const headWithBoundary = read(headFixture)
  headWithBoundary.boundaries.push({
    kind: "region",
    label: "Fraud edge",
    wraps: ["fraud"],
  })
  const boundaryReceipt = compareArchitecture(
    read(baseFixture),
    headWithBoundary,
  )
  assert.deepEqual(
    boundaryReceipt.changes.boundaries.find(
      (change) => change.label === "Fraud edge",
    ).classifications,
    ["scope"],
  )
})
...
test("canonical architecture ignores formatting, entity order, and set-like order", () => {
  ...
  assert.equal(
    canonicalArchitectureJson(reordered),
    canonicalArchitectureJson(original),
  )
})
...
test("exact identity fails closed instead of guessing relationships or unrelated systems", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ArchitectureDeltaError &&
      error.code === "delta/no-shared-component-id",
  )
})
...
test("evidence-only component changes keep an enabled exact review contract", () => {
  const base = read(baseFixture)
  const head = read(baseFixture)
  base.components[0].sources = [
    { path: "src/entry.js", line: 1, label: "baseline" },
  ]
  head.components[0].sources = [
    { path: "src/entry.js", line: 2, label: "head" },
  ]
  const receipt = compareArchitecture(base, head)
  ...
  const runtime = fs.readFileSync(
    path.join(skillRoot, "delta/architecture-delta.mjs"),
    "utf8",
  )
  assert.match(
    runtime,
    /statuses: \['added', 'changed', 'evidence-changed', 'removed', 'moved'\]/,
  )
})

test("mixed semantic and geometry component changes retain both exact forms", () => {
  const head = read(headFixture)
  head.components.find((component) => component.id === "queue").sublabel =
    "durable queue v2"
...
test("mixed semantic and geometry relationship changes retain both exact routes", () => {
  const head = read(headFixture)
  head.connections.find(
    (connection) => connection.id === "publish-order",
  ...
  assert.equal(result.status, 0, result.stderr)
  const receipt = JSON.parse(result.stdout)
  const publishOrder = receipt.changes.connections.find(
    (change) => change.id === "publish-order",
  )
...
test("baseline boundary title masks stay below current components and carry delta identity", () => {
  const documentAt = (pos, pad) => ({
    ...
    connections: [],
    boundaries: [
      { kind: "region", label: "Boundary label", wraps: ["node"], pad },
    ],
  })
  ...
  assert.ok(
    phantomMask && currentNodeRect,
    "expected the phantom mask and current component rect",
  )
  const rect = (tag) =>
    Object.fromEntries(
  ...
  assert.equal(
    overlaps,
    true,
    `expected overlap: ${JSON.stringify({ maskBox, nodeBox })}`,
  )
  assert.ok(
...
test("same-label node id changes remain one removal plus one addition", () => {
  ...
  head.boundaries.forEach((boundary) => {
    boundary.wraps = boundary.wraps.map((id) =>
      id === "cache" ? "session-store" : id,
    )
  })
  head.connections.find((connection) => connection.id === "session-read").to =
    "session-store"
  ...
  assert.equal(
    receipt.changes.components.find((change) => change.id === "session-store")
      .status,
    "added",
  )
  assert.equal(
    receipt.changes.components.filter(
      (change) =>
        change.headLabel === "Session Cache" ||
        change.baseLabel === "Session Cache",
    ).length,
    2,
  )
})
...
test("repository mismatch fails and verified matching revisions remain evidence-bounded", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ArchitectureDeltaError &&
      error.code === "delta/repository-mismatch",
  )
...
test("compare CLI writes a deterministic three-state artifact and complete sidecar receipt", () => {
  ...
  assert.equal(
    (firstHtml.match(/<section class="canvas" data-view=/g) || []).length,
    3,
  )
  ...
  assert.match(
    firstHtml,
    /data-node-id="queue"[^>]+data-delta-state="moved-from"/,
  )
  ...
  assert.match(
    firstHtml,
    /data-delta-boundary-state="added".*fill:#34d399!important/,
  )
  assert.match(
    firstHtml,
    /delta-boundary-marker\[data-delta-state\]\{color:var\(--delta\)\}/,
  )
  ...
  assert.match(
    firstHtml,
    /document\.querySelectorAll\('#archify-compare-receipt'\)\.length !== 1/,
  )
  assert.match(
    firstHtml,
    /targetsMatch\(reviewSources\[index\], row, matches\)/,
  )
  ...
  assert.doesNotMatch(
    deltaShell,
    /\b(?:SAFE|LOW RISK|MERGEABLE|NO IMPACT|VERIFIED PR)\b/i,
  )
  ...
  assert.equal(
    ...
    JSON.parse(repeat.stdout).artifact.sha256,
  )
...
test("checked-in Checkout compare artifact is reproducible from its authoritative inputs", () => {
  ...
  assert.equal(
    ...
    fs.readFileSync(checkedArtifact, "utf8"),
  )
...
test("artifact validation fails closed on missing, duplicate, or self-blessed review identity", () => {
  ...
  const fraudTag = deltaSection.match(
    /<g\s+[^>]*\bdata-node-id="fraud"[^>]*>/,
  )?.[0]
  assert.ok(fraudTag)
  ...
  assert.throws(
    ...
    (error) =>
      ...
      error.details.failures.includes(
        "ambiguous Delta identity component:fraud",
      ),
  )
  ...
  assert.throws(
    ...
    (error) =>
      ...
      error.code === "delta/artifact-invalid" &&
      error.details.failures.includes(
        "ambiguous Delta target signature relationship:fraud-check",
      ),
  )
  ...
  const storedSignature = duplicateRowTag?.match(
    /data-change-target-signature="([^"]+)"/,
  )?.[1]
  assert.ok(duplicateRowTag && storedSignature)
  const selfBlessedSignature = [
  ...
  assert.throws(
    ...
    (error) =>
      ...
      error.code === "delta/artifact-invalid" &&
      error.details.failures.includes(
        "missing Delta target state relationship:fraud-check",
      ),
  )

  const receiptNode = html.match(
    /<script id="archify-compare-receipt"[\s\S]*?<\/script>/,
  )?.[0]
  assert.ok(receiptNode)
  const duplicateReceipt = html.replace(
    receiptNode,
  ...
  assert.throws(
    ...
    (error) =>
      ...
      error.code === "delta/artifact-invalid" &&
      error.details.failures.includes(
        "expected exactly one embedded compare receipt",
      ),
  )
  ...
  assert.throws(
    ...
    (error) =>
      ...
      error.code === "delta/artifact-invalid" &&
      error.details.failures.includes(
        "expected exactly one root SVG in the Delta canvas",
      ),
  )
})
...
test("formatting-only input changes raw proof but not semantic hash or artifact bytes", () => {
  ...
  assert.equal(
    originalReceipt.base.semanticSha256,
    changedReceipt.base.semanticSha256,
  )
  assert.equal(
    fs.readFileSync(originalOut, "utf8"),
    fs.readFileSync(reorderedOut, "utf8"),
  )
...
test("compare validates raw snapshots before canonicalization can discard invalid fields", () => {
  ...
  assert.equal(
    fs.existsSync(path.join(tmp, "invalid-raw-base.receipt.json")),
    false,
  )
  const receipt = JSON.parse(result.stdout)
  assert.equal(receipt.ok, false)
  ...
  assert.equal(
    receipt.diagnostics[0].evidence.additionalProperty,
    "unknown_top_level_fact",
  )
})
