...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-viewer-chrome-layout-"),
)
...
test("the public CLI gives all typed renderers one final Viewer contract", () => {
  ...
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      directChildSvg,
      `${mode} keeps the SVG as a direct child`,
    )
    assert.doesNotMatch(
      html,
      /class="diagram-stage"/,
      `${mode} does not re-nest the exported SVG`,
    )
    assert.doesNotMatch(
      canonicalSvg(html),
...
test("Dock Safe Rail keeps typed renderers clear across themes, Presentation, and low-height desktops", {
  skip: chromePath
    ? false
    : "Set ARCHIFY_CHROME to run the real browser regression.",
}, async () => {
  ...
  try {
    for (const entry of matrix) {
      ...
      const sessionId = await load(
        browser,
        render(entry.mode, CASES[entry.mode]),
        {
          width: entry.width,
          height: entry.height,
          query,
      ...
      assert.ok(
        receipt.minimumProjectedNodeTextPx >= MIN_PROJECTED_NODE_TEXT_PX,
        message,
      )
    }
  } finally {
...
test("an artifact with no Legend still receives the desktop stage rail", {
  ...
  try {
    ...
    assert.ok(
      receipt.navBottom <= receipt.containerBottom + 0.5,
...
test("Dock Safe Rail resolves a forced Legend collision across the shared diagram viewer", {
  ...
  try {
    for (const [mode, example] of Object.entries(CASES)) {
      ...
      assert.equal(
        receipt.legendDockIntersectionArea,
        0,
        `${mode}: ${JSON.stringify(receipt)}`,
      )
...
test("Maka remains collision-free at the reported Retina-equivalent viewport", {
  ...
  try {
    ...
    assert.ok(
      receipt.scrollWidth <= receipt.innerWidth,
      JSON.stringify(receipt),
    )
  } finally {
    await browser.close()
...
test("manual zoom and pan reschedules Legend and Dock collision measurement", {
  ...
  try {
    ...
    assert.equal(receipt.legendDockIntersectionArea, 0, JSON.stringify(receipt))
    assert.equal(
      receipt.semanticDockIntersectionArea,
      0,
      JSON.stringify(receipt),
    )
    assert.ok(receipt.reserve > 0, JSON.stringify(receipt))
  ...
  try {
    for (const scenario of scenarios) {
      ...
      assert.deepEqual(
        result.hits,
        [],
        `${scenario.name}: ${JSON.stringify(result.hits)}`,
      )
      if (scenario.clearsClip) {
        assert.equal(
          result.clipPath,
          "",
          `${scenario.name} retains runtime clip-path`,
        )
      }
    }
  } finally {
...
test("zoom keeps the desktop rail stable and reports protected stage geometry", {
  ...
  try {
    ...
    assert.equal(
      zoomed.dockStageIntersectionArea,
      0,
      JSON.stringify({ baseline, zoomed }),
    )
    assert.equal(
      zoomed.receiptStageIntersectionArea,
      0,
      JSON.stringify({ baseline, zoomed }),
    )
  } finally {
...
test("Reset followed immediately by zoom and pan retains a collision-free desktop rail", {
  ...
  try {
    ...
    assert.ok(
      receipt.reserve <= baseline.reserve + 1,
      JSON.stringify({ baseline, receipt }),
    )
    assert.equal(
      receipt.semanticDockIntersectionArea,
      0,
...
test("localized multiline Legends remain clear across required viewports, themes, and presets", {
  ...
  try {
    ...
    for (const entry of cases) {
      ...
      assert.equal(
        receipt.dockStageIntersectionArea,
        0,
        JSON.stringify({ ...entry, receipt }),
      )
...
test("Semantic Lens and Radar protect the final Legend and Dock rectangles", {
  ...
  try {
    ...
    receipt = await finalGeometry(browser, sessionId)
    assert.equal(
      receipt.legendRadarIntersectionArea,
      0,
      JSON.stringify(receipt),
    )
...
test("mobile, embed, and print keep zero reserve while hidden Legends retain the stage rail", {
  ...
  try {
    ...
    assert.equal(
      receipt.legendDockIntersectionArea,
      0,
      `mobile: ${JSON.stringify(receipt)}`,
    )

    sessionId = await load(
      browser,
      render("architecture", CASES.architecture),
      { query: "?embed=1" },
    )
