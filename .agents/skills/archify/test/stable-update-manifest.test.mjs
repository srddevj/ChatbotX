...
const checker = path.join(
  repoRoot,
  "scripts",
  "check-stable-update-manifest.mjs",
)
...
function annotatedTaggerTime(root, tag) {
  ...
  return new Date(Number(result.stdout.trim()) * 1000)
    .toISOString()
    .replace(".000Z", "Z")
}
...
test("stable release gate binds manifest tag, tree, and final archive digest", () => {
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-stable-manifest-"),
  )
  try {
    writeJson(path.join(fixture, "archify", "package.json"), {
      version: "3.0.0",
    })
    fs.writeFileSync(
      path.join(fixture, "archify", "SKILL.md"),
      "stable fixture\n",
    )
    ...
    const historicalRelease = runCheck(fixture, archive, [
      "--source-ref",
      "v3.0.0",
    ])
    assert.equal(historicalRelease.status, 0, historicalRelease.stderr)
    const wrongHistoricalRef = runCheck(fixture, archive, [
      "--source-ref",
      "v2.9.0",
    ])
    assert.notEqual(wrongHistoricalRef.status, 0)
    assert.match(
      wrongHistoricalRef.stderr,
      /--source-ref must be HEAD or the exact release tag/,
    )
    writeJson(path.join(fixture, "archify", "package.json"), {
      version: "3.0.0",
    })
    ...
    writeJson(manifestPath, {
      ...manifest,
      publishedAt: new Date(Date.parse(publishedAt) + 1000)
        .toISOString()
        .replace(".000Z", "Z"),
    })
    ...
    assert.doesNotMatch(
      nullManifest.stderr,
      /TypeError|check-stable-update-manifest\.mjs:\d+/,
    )
    writeJson(manifestPath, manifest)
