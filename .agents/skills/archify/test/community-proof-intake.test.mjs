...
test("showcase intake requires reproducible proof, redaction, and explicit publication permission", () => {
  ...
  assert.match(
    template,
    /repository, documentation, gallery, and project website/i,
  )
  assert.match(template, /required:\s*true/g)

  const submissionUrl =
    "https://github.com/tt-a1i/archify/issues/new?template=showcase.yml"
...
test("bug intake captures a minimal deterministic reproduction before visual diagnosis", () => {
  ...
  assert.ok(
    template.indexOf("id: validation_receipt") <
...
test("contributor and pull-request guides keep proof changes reproducible and stability-first", () => {
  ...
  assert.match(
    contributing,
    /automated or browser evidence separately from perceptual review/i,
  )
  assert.match(
    contributing,
    /non-visual pull request must write `Not applicable`/i,
  )
  ...
  assert.doesNotMatch(
    contributing,
    /must reach `passed` before final review or merge/i,
  )
  assert.doesNotMatch(
    pullRequest,
    /must reach `passed` before final review or merge/i,
