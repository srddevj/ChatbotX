...
test("clean staging rejects a packaged notice that diverges from the repository notice", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      canonicalNotices.replace(
        "Simple Icons 16.28.0",
        "Simple Icons 16.28.0 modified",
      ),
    )
...
test("clean staging preserves index modes and strips repository-only package metadata", () => {
  ...
  try {
    ...
    assert.equal(
      fs.statSync(path.join(destination, "bin", "executable.mjs")).mode & 0o777,
      0o755,
    )
    assert.equal(fs.existsSync(path.join(destination, "test")), false)
    assert.equal(
      fs.readFileSync(
        path.join(destination, "runtime", "test", "required.dat"),
        "utf8",
      ),
      "runtime fixture\n",
    ...
    assert.equal(
      fs.existsSync(path.join(destination, "package-lock.json")),
      false,
    )
...
test("clean staging snapshots unstaged tracked bytes before a source ancestor can be swapped", (t) => {
  ...
  try {
    ...
    assert.equal(
      swapped,
      true,
      "the deterministic ancestor-swap attack must run",
    )
    assert.equal(
...
test("clean staging rejects a source ancestor swapped during preflight traversal", (t) => {
  ...
  try {
    ...
    assert.equal(
      swapped,
      true,
      "the deterministic mid-preflight ancestor swap must run",
    )
    assert.equal(fs.existsSync(destination), false)
  } finally {
...
test("clean staging reports the Git spawn error when Git cannot start", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /unable to enumerate tracked Archify files: .*ENOENT/,
    )
    assert.doesNotMatch(
      result.stderr,
      /tracked Archify paths must be valid UTF-8/,
    )
    assert.equal(fs.existsSync(destination), false)
  } finally {
    fs.rmSync(root, { recursive: true, force: true })
