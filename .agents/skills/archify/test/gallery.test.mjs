...
test("generated proof gallery matches its sources, receipts, and checked-in artifacts", () => {
  ...
  assert.deepEqual(
    Object.fromEntries(
      ["architecture", "workflow", "sequence", "dataflow", "lifecycle"].map(
        (type) => [
          type,
  ...
  const workflow = manifest.entries.find(
    (entry) => entry.id === "agent-tool-call",
  )
  assert.equal(workflow.view, "happy-path")
  assert.equal(workflow.viewCount, 3)
  assert.deepEqual(workflow.viewIds, [
    "happy-path",
    "safety-gate",
    "evidence-loop",
  ])
  ...
  const deployment = manifest.entries.find(
    (entry) => entry.id === "deployment-ownership",
  )
  ...
  for (const entry of manifest.entries) {
    const artifact = path.join(
      generatedRoot,
      entry.artifact.replace(/^gallery\//, "gallery/"),
    )
    const source = path.join(
      generatedRoot,
      entry.input.replace(/^gallery\//, "gallery/"),
    )
    ...
    assert.equal(
      ...
      `${entry.id}: artifact digest drift`,
    )
    assert.equal(
      sha256(source),
      entry.sourceSha256,
      `${entry.id}: source digest drift`,
    )
    ...
    assert.equal(
      entry.viewCount,
      3,
      `${entry.id}: expected a three-step reader story`,
    )
    assert.equal(
      entry.guidedPlayback,
      true,
      `${entry.id}: guided playback missing`,
    )
  }
  ...
  assert.match(
    html,
    /data-src-base="gallery\/artifacts\/agent-tool-call\.workflow\.html"/,
  )
  assert.match(
    html,
    /agent-tool-call\.workflow\.html\?present=1&amp;play=1#view=happy-path/,
  )
  assert.match(
    html,
    /event-stream\.dataflow\.html\?present=1&amp;play=1#view=order-transit/,
  )
  ...
  assert.match(
    fs.readFileSync(
      path.join(generatedRoot, "assets/site-navigation.css"),
      "utf8",
    ),
  ...
  for (const relative of [
    ...
    assert.ok(
      ...
      `${relative}: checked-in gallery output missing`,
    )
    assert.equal(
