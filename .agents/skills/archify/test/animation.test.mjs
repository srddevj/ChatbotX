...
test("webm renders an explicit time-varying canvas scene instead of replaying one cached SVG bitmap", () => {
  ...
  const recordBlock =
    html.match(
      /function recordWebm\(options\) \{[\s\S]*?\n {6}var menu =/,
    )?.[0] || ""
...
test("editorial preset reaches every visual surface and all five typed renderers", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /svg\[data-preset="editorial"\] \.story-trail-flow/,
      mode,
    )
  }
})
...
test("all five renderers add one geometry-neutral semantic sigil per primary node", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    const sigils = (svg) =>
      [
        ...svg.matchAll(
          /<g aria-hidden="true" data-semantic-sigil="[^"]+"[\s\S]*?<\/g>/g,
        ),
      ].map((match) => match[0])
    ...
    assert.match(
      staticHtml,
      /svg \.s-database\s+\{ color: var\(--database-stroke\); \}/,
      mode,
    )
  }
})
...
test("semantic SVG identity is deterministic for unchanged input", () => {
  ...
  const hooks = (svg) =>
    [...svg.matchAll(/(?:id="node-|data-edge-from=")[^>]+/g)].map(
      (match) => match[0],
    )
