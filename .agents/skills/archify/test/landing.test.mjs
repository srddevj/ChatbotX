...
test("landing metadata describes the full technical-diagram product and trusted hero promise", () => {
  ...
  assert.match(
    landing,
    /An agent skill for Cursor, Claude Code, Codex CLI, and OpenCode/,
  )
  assert.equal(
    (
      landing.match(
  ...
  assert.match(
    landing,
    /From plain English<br>to architecture <em>you can trust\.<\/em>/,
  )
})
...
test("landing proof switcher is bilingual and keyboard navigable", () => {
  ...
  assert.doesNotMatch(
    landing,
    /proofFrame\.contentWindow|proofFrame\.contentDocument/,
  )
...
test("landing makes Route Journey and core exploration shortcuts discoverable in both languages", () => {
  assert.match(
    landing,
    /Route Journey keeps the complete authored path visible/,
  )
  assert.match(landing, /Route Journey 始终保留完整作者路径/)
  assert.match(
    landing,
    /one finite, reader-controlled pass over each exact incoming relationship/,
  )
  ...
  assert.match(
    landing,
    /data-i18n="kbd-guide">Diagram guide<\/span><kbd>\?<\/kbd>/,
  )
  assert.match(landing, /'kbd-guide':'图表指南'/)
  assert.match(
    landing,
    /data-i18n="kbd-find">Find node \/ route endpoint<\/span><kbd>\/<\/kbd>/,
  )
  ...
  assert.match(
    landing,
    /data-i18n="kbd-lens">Compare semantic kinds<\/span><kbd>L<\/kbd>/,
  )
