...
function writeValidDevelopmentFixture(root, overrides = {}) {
  ...
  const files = {
    ...
    "archify/assets/template.html":
      '<meta name="generator" content="archify 2.13.0-dev.0">',
    "CHANGELOG.md": [
      "# Changelog",
      "",
      "## [Unreleased]",
    ...
    "scripts/gallery-template.html":
      "development · 开发版 · [[ARCHIFY_VERSION]]",
...
function writeValidStableFixture(root, overrides = {}) {
  ...
  const files = {
    ...
    "archify/SKILL.md": '---\nmetadata:\n  version: "2.13"\n---\n',
    "archify/assets/template.html":
      '<meta name="generator" content="archify 2.13.0">',
    "CHANGELOG.md": [
      "# Changelog",
      "",
      "## [Unreleased]",
...
test("an empty Unreleased section accepts a coherent stable release identity", () => {
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-release-identity-"),
  )
  try {
...
test("the published update manifest must track the newest stable changelog release", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /stable\.json must describe the newest published stable v2\.12\.0/,
    )
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true })
  }
...
test("real Unreleased changes cannot reuse a stable published package identity", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /Unreleased changes require a prerelease package version/,
    )
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true })
  }
...
test("package, lockfile, Skill metadata, escaped Shields badge, and public docs share one development identity", () => {
  ...
  try {
    writeFile(
      fixture,
      "archify/package.json",
      JSON.stringify({ version: "2.13.0-dev.0" }),
    )
    writeFile(
      fixture,
    ...
    assert.match(
      result.stderr,
      /README\.md must advertise development identity v2\.13\.0-dev\.0/,
    )
    assert.match(
      result.stderr,
...
test("landing rejects every stale N/N contract count even when 9/9 is also present", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /every N\/N proof receipt must be exactly 9\/9; found 7\/7/,
    )
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true })
  }
...
test("roadmap current identity follows the package release state", () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      "ROADMAP.md":
        "The current development line is `v2.12.0`; it is not a stable release.",
    })
