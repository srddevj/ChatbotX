...
test("artifact SVG extraction follows HTML quoting and preserves SVG document boundaries", () => {
  ...
  assert.equal(
    ...
    1,
    "template SVG is markup while script text is not",
  )
  assert.equal(
    extracted.embedded.length,
    1,
    "srcdoc contributes one top-level SVG document",
  )
  ...
  `)
  assert.equal(
    inheritedNamespace.direct.length,
    1,
    "nested SVG remains inside its XML document",
  )
...
test("tracked browsable HTML embeds well-formed XML SVG", () => {
  ...
  assert.ok(
    artifacts.includes(checkoutArtifact),
    "expected the tracked Checkout compare artifact",
  )
  let checkoutSvgs
  ...
  for (const relative of artifacts) {
    ...
    if (relative === checkoutArtifact) {
      checkoutSvgs = extracted
    }
  ...
  assert.equal(
    checkoutSvgs?.direct.length,
    1,
    "Checkout must contain one comparison SVG",
  )
  assert.equal(
...
test("legacy example URLs redirect to the current canonical artifacts", () => {
  for (const [legacy, canonical] of [
    ...
    assert.equal(
      fs.existsSync(path.join(repoRoot, "examples", canonical)),
      true,
    )
  }
})
