...
test("Share Card uses contain-only canonical geometry with fixed safe areas", () => {
  ...
  assert.match(
    html,
    /throw exportError\('viewer\.export\.error\.contextUnavailable'/,
  )
  assert.match(
    html,
    /throw exportError\('viewer\.export\.error\.toBlobUnavailable'/,
  )
...
test("Copy Share Card reuses one canonical card blob and writes only PNG to the clipboard", () => {
  ...
  assert.match(
    html,
    /Copy Share Card[\s\S]*?<small class="hint">PNG to clipboard<\/small>/,
  )
  assert.match(
    html,
  ...
  const copyBlock =
    html.match(/function runCopyShareCard\(\) \{[\s\S]*?\n {6}\}/)?.[0] || ""
...
test("Copy Share Card fails closed when image clipboard writing is unavailable", () => {
  ...
  assert.match(
    html,
    /it\.dataset\.action === 'copy-share-card'[\s\S]*?!canCopyImage\(\)/,
  )
  assert.match(
    html,
    /function runCopyShareCard\(\)[\s\S]*?if \(!canCopyImage\(\)\)/,
  )
...
test("ordinary Copy PNG keeps its existing full-diagram raster path", () => {
  ...
  assert.match(
    html,
    /function runCopy\(\)[\s\S]*?var blobPromise = rasterize\('png'\);/,
  )
...
test("Share Card stays viewer-only and reuses export cleanup instead of source state", () => {
  ...
  assert.doesNotMatch(
    svgBlock(html),
    /data-last-export-|data-format="share-card"/,
  )
})
