...
export function segmentRectClearance(segment, rect) {
  ...
  if (
    !isFinitePoint(...start, ...end, rect.x, rect.y, rect.width, rect.height)
  ) {
    return null
  }
...
export function segmentRectIntersectionLength(segment, rect) {
  ...
  for (const [direction, distance] of bounds) {
    if (Math.abs(direction) <= 0.000_000_1) {
...
export function collectLabelRouteClearance({
  ...
  for (const [fallbackIndex, label] of asArray(labels).entries()) {
    ...
    if (seenLabels.has(labelIdentity)) {
      continue
    }
    ...
    for (const route of routes) {
      ...
      if (!nearest || nearest.clearance + 0.0001 >= threshold) {
        continue
      }
      hits.push({
        label,
...
function endpointSideIssue(points, endpoint, side) {
  ...
  if (Math.abs(across) <= 0.0001 && along * expectedSign > 0.0001) {
    return null
  }
  return {
    endpoint,
...
export function cleanEndpointSideProblems({
  ...
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    ...
    if (!shouldCheckRelation(relation, relationIndex)) {
      continue
    }
...
export function cleanFlowProblems({
  ...
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    ...
    for (const obstacle of obstacleList) {
      if (!obstacle || endpointIds.has(obstacle.id)) {
        continue
      }
      if (
    ...
    for (
      ...
      for (
        ...
        for (
          ...
          if (!overlap || overlap.length + 0.0001 < minOverlapPx) {
            continue
          }
    ...
    for (const [frameIndex, frame] of asArray(frames).entries()) {
      for (const border of frameBorderSegments(frame)) {
        ...
        for (
          ...
          if (!overlap || overlap.length <= 0.0001) {
            continue
          }
...
export function cleanBorderRunProblems({
  ...
  if (!qualityProfileForGate(profile, profileIsAuthoritative)) {
    return []
  }
  const routedRelations = collectEligibleRoutedRelations({
    relations,
...
export function routeBudgetMetrics({
  ...
  for (const routed of asArray(routedRelations)) {
    ...
    for (let index = 0; index < points.length - 1; index += 1) {
      ...
      if (length < segmentPx) {
        ...
        if (position === "interior") {
          ...
          shortEndpointSegmentCount += 1
        }
      }
      if (length < microSegmentPx) {
        microSegmentCount += 1
      }
    }
    const direct =
    ...
    if (direct > 0.0001) {
      ...
      if (routeStretch > stretch + 0.0001) {
        routesOverSuggestedStretch += 1
      }
    }
  }
...
function collinearForward(a, b, c) {
  if (Math.abs(crossProduct(a, b, c)) > 0.0001) {
    return false
  }
  return (
...
export function automaticPortSpread(
  ...
  for (const relation of asArray(relations)) {
    if (!relation || (relation.route && relation.route !== "auto")) {
      continue
    }
    if (
      relation.via ||
