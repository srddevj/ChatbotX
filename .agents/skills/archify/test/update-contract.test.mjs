...
test("SemVer precedence follows the complete prerelease ordering vector", () => {
  ...
  assert.throws(
    () => compareSemver("1.0.0-alpha.01", "1.0.0"),
    UpdateContractError,
  )
})
...
test("local release identity is exact and channel-consistent", () => {
  ...
  assert.throws(
    () => validateLocalRelease(localRelease({ extra: true })),
    UpdateContractError,
  )
  assert.throws(
    () =>
...
test("publication timestamps are canonical UTC seconds with real calendar dates", () => {
  assert.equal(
    validateCanonicalUtcTimestamp("2024-02-29T23:59:59Z"),
  ...
  for (const value of [
    ...
    assert.throws(
      () => validateCanonicalUtcTimestamp(value),
      UpdateContractError,
      value,
    )
  }
...
test("stable manifests enforce one exact schema and canonical release identity", () => {
  assert.deepEqual(
    validateStableUpdateManifest(stableManifest()),
    stableManifest(),
  )
  const invalid = [
  ...
  for (const value of invalid) {
    assert.throws(
      () => validateStableUpdateManifest(value),
      UpdateContractError,
    )
  }
})
