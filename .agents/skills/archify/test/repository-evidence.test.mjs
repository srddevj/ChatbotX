...
test("repository evidence accepts canonical HTTPS and common SSH remotes", () => {
  ...
  for (const remote of [
    ...
    assert.equal(
      result.status,
      0,
      `${remote}: ${result.stderr || result.stdout}`,
    )
  }
})

async function waitForState(url, predicate, timeoutMs = 12_000) {
...
test("repository evidence is revision-verified, receipt-backed, searchable, and export-clean", () => {
  ...
  assert.doesNotMatch(
    svg,
    /src\/router\.js|github\.com\/example\/evidence-repo|source-evidence/,
  )
})
...
test("live preview forwards repo-root and publishes only verified evidence", {
  timeout: 20_000,
}, async () => {
  const data = fixture()
