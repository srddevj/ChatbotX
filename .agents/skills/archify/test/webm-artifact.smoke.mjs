...
assert.ok(
  ffmpeg,
  "ffmpeg is required for the WebM artifact smoke test (or set ARCHIFY_FFMPEG)",
)
...
try {
  cdp = await connectCdp(
    await devtoolsEndpoint(port, chromeProcess, () => chromeStderr),
  )
  ...
  async function navigateReady(file, condition, label) {
    ...
    assert.equal(
      ready,
      true,
      `${label} did not expose its browser export surface`,
    )
  }
  ...
  async function verifyResolvedLegendContract(outputs) {
    ...
    for (const theme of ["dark", "light"]) {
      const dataflow = await inspectKinds(
        outputs.dataflow,
        ["database", "default"],
        theme,
      )
    ...
    assert.deepEqual(runtime.exportedKinds, [
      "frontend",
      "database",
      "external",
    ])
    ...
    assert.deepEqual(embed, {
      roles: 0,
      runtime: 0,
      kinds: ["frontend", "database", "external"],
    })
  ...
  async function verifySemanticPassportDismissal(file) {
    ...
    assert.equal(
      result?.ok,
      true,
      result?.error || "Semantic Passport smoke failed",
    )
    ...
    assert.deepEqual(result.afterNode, {
      hidden: false,
      active: result.neighborId,
    })
    console.log(
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    await waitForSelected(
      ...
      "architecture-delta initial playback",
    )
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta manual lifecycle navigator",
    )
    await evaluate(
      cdp,
      sessionId,
    ...
    await waitForSelected(
      "relationship:fraud-check",
      "architecture-delta previous control",
    )
    ...
    assert.deepEqual(previousPause, {
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta focus lifecycle navigator",
    )
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta hidden lifecycle navigator",
    )
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta print lifecycle navigator",
    )
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta reduced-motion navigator",
    )
    await evaluate(
      cdp,
      sessionId,
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta tamper navigator",
    )
    ...
    await navigateReady(
      file,
      readyCondition,
      "architecture-delta finite navigator",
    )
    await evaluate(
      cdp,
      sessionId,
  ...
  async function captureShareCard(file, label) {
    ...
    assert.ok(
      colors.size >= 24,
      `${label} Share Card has only ${colors.size} sampled colors`,
    )
    assert.ok(
  ...
  async function captureCopiedShareCard(file, label) {
    ...
    assert.ok(
      copiedPayload.size > 20_000,
      `${label} copied Share Card is unexpectedly small`,
    )
  ...
  async function captureRouteShareCard(
    file,
    label,
    sourceId,
    targetId,
    ...
    for (let offset = 0; offset < sampledPixels.length; offset += 3) {
      sampledColors.add(
        sampledPixels.subarray(offset, offset + 3).toString("hex"),
      )
    }
    assert.ok(
    ...
    assert.deepEqual(
      routePayload.matchedEdgeKeys,
      routePayload.expectedEdgeKeys,
    )
    ...
    assert.match(
      routePayload.toBlobError,
      /canvas\.toBlob returned no data for Share Card/,
    )
    assert.match(
      routePayload.missingToBlobError,
      /canvas\.toBlob unavailable for Share Card/,
    )
    assert.match(
      routePayload.missingContextError,
      /2D canvas context unavailable for Share Card/,
    )
    assert.ok(routePayload.imageDecodeError)
    assert.match(
      routePayload.unknownVariantError,
      /Unknown Share Card variant: unknown/,
    )
    ...
    assert.ok(
      routePayload.headerMetrics.every(
        (metric) => metric.width <= metric.maxWidth + 0.5,
      ),
      `${label} title/subtitle overflowed the Share Card header`,
    )
    assert.deepEqual(routePayload.svgReceipt, {
      format: "svg",
      variant: null,
      canonical: "true",
    })
    assert.deepEqual(routePayload.pngReceipt, {
      format: "png",
      variant: null,
      canonical: "true",
    })
    ...
    if (options.expectedSourceLabel !== undefined) {
      assert.equal(
        routePayload.snapshot.source.label,
        options.expectedSourceLabel,
      )
    }
    if (options.expectedTargetLabel !== undefined) {
      assert.equal(
        routePayload.snapshot.target.label,
        options.expectedTargetLabel,
      )
    }
    console.log(
  ...
  async function verifyDynamicReducedMotionRoute(
    file,
    label,
    sourceId,
    targetId,
    ...
    for (let attempt = 0; attempt < 50; attempt += 1) {
      ...
      if (reduced.matches && reduced.motion === "still" && !reduced.playing) {
        break
      }
      await delay(20)
    }
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    ...
    for (const result of matrix.results) {
      assert.equal(
        result.type,
        "image/png",
        `${label} ${result.preset}/${result.theme} MIME`,
      )
      assert.equal(
        result.width,
        1200,
        `${label} ${result.preset}/${result.theme} width`,
      )
      assert.equal(
        result.height,
        630,
        `${label} ${result.preset}/${result.theme} height`,
      )
      assert.ok(
        result.size > 20_000,
    ...
    console.log(
      `ok ${label} Route visual matrix: Classic/Flow/Blueprint/Editorial x dark/light`,
    )
  }

  async function captureReachShareCard(
    file,
    label,
    originId,
    direction,
    ...
    assert.match(
      reachPayload.downloads[0],
      new RegExp(`-${direction}-reach-share-card\\.png$`),
    )
  ...
  await captureRouteShareCard(
    routeOutputs.workflow,
    "workflow-route",
    "user",
    "approval",
  )
  await captureRouteShareCard(
    routeOutputs.sequence,
    "sequence-route",
    "web",
    "db",
  )
  await captureRouteShareCard(
    routeOutputs.dataflow,
    "dataflow-route",
    "web",
    "dashboard",
  )
  await captureRouteShareCard(
    routeOutputs.lifecycle,
    "lifecycle-route",
    "executing",
    "cancelled",
  )
  ...
  await captureRouteVisualMatrix(
    routeOutputs.architecture,
    "architecture-route",
    "users",
    "api",
  ...
  assert.ok(
    payload.size > 10_000,
    `WebM is unexpectedly small (${payload.size} bytes)`,
  )
