...
test("all typed renderers inherit one geometry-neutral Intent Trace", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /data-intent-trace|intent-trace-flow/,
      mode,
    )
  }
})
...
test("Intent Trace keeps incoming and outgoing motion on authored source-to-target geometry", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    const incomingRule =
      html.match(
        /\.intent-trace-flow\[data-direction="in"\]\s*\{[\s\S]*?\}/,
      )?.[0] || ""
    ...
    assert.match(
      html,
      /@keyframes archify-intent-trace-flow[\s\S]+stroke-dashoffset: -1/,
      mode,
    )
  }
})
...
test("Intent Trace normalizes motion, respects reduced motion, and exports cleanly", () => {
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-intent-trace-overlay\]'\)/,
  )
  assert.match(
    html,
