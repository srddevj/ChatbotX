...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-story-follow-camera-"),
)
...
test("adaptive dwell, Still, reduced motion, hidden pages, and print keep camera motion bounded", () => {
  ...
  assert.match(
    template,
    /else if \(!shouldPlay && svg\.hasAttribute\('data-story-playing'\)\)/,
  )
...
test("pause, settle, overview, and manual camera takeover cancel Story Follow state", () => {
  ...
  assert.match(
    template,
    /function pausePlayback\(options\)[\s\S]*?clearStoryFollow\(\)/,
  )
  assert.match(
    template,
    /function settleStoryBeats\(\)[\s\S]*?clearStoryFollow\(\)/,
  )
  assert.match(
    template,
    /function clearStoryTrail\(\)[\s\S]*?clearStoryFollow\(\)/,
  )
  assert.match(
    template,
