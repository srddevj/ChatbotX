...
test("all five renderers inherit one active-reach-only Reach Share Card item", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /function open\(focusLast\)[\s\S]*?syncReachShareItem\(\);/,
      mode,
    )
    assert.doesNotMatch(
      html,
      /id="reach-share-card"|class="reach-share-card"/,
      mode,
    )
...
test("Reach variant decorates only a finite canonical clone with static authored identity", () => {
  ...
  assert.match(
    applyBlock,
    /nodeId === snapshot\.origin\.id \? depth !== 0 : depth < 1/,
  )
  assert.match(
    applyBlock,
  ...
  assert.doesNotMatch(
    applyBlock,
    /setAttribute\('data-reach-(?:active|match|origin|depth)/,
  )
...
test("Reach styling preserves context, direction, and Blueprint restraint without motion", () => {
  ...
  assert.match(
    html,
    /data-share-reach=\\?"upstream\\?"[\s\S]*?--database-stroke/,
  )
  assert.match(
    html,
    /data-share-reach=\\?"downstream\\?"[\s\S]*?--backend-stroke/,
  )
  assert.match(
    html,
    /data-preset=\\?"blueprint\\?"\]\[data-share-reach\][\s\S]*?filter: none/,
  )
...
test("Reach Share Card reuses the 1200x630 seam and publishes a truthful scoped receipt", () => {
  ...
  assert.match(
    html,
    /options\.variant !== 'route' && options\.variant !== 'reach'/,
  )
...
test("Skill, product docs, and READMEs keep the optional truthful boundary explicit", () => {
  ...
  const png = fs.readFileSync(
    path.join(repoRoot, "docs/assets/mco-runtime-reach-share-card.png"),
  )
