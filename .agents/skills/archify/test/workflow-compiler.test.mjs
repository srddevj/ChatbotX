...
function assertRectInsideViewBox(rect, viewBox, message) {
  ...
  assert.ok(
    rect.x >= minX,
    `${message}: left edge ${rect.x} is outside ${minX}`,
  )
...
function assertRectInsideRect(rect, container, message) {
  assert.ok(
    rect.x >= container.x,
    `${message}: left edge ${rect.x} is outside ${container.x}`,
  )
  assert.ok(
    rect.y >= container.y,
    `${message}: top edge ${rect.y} is outside ${container.y}`,
  )
  assert.ok(
...
function asciiGroupLabelTextRect(svg, label) {
  ...
  const attributes = svg.match(
    new RegExp(`<text\\b([^>]*)>${escapedLabel}</text>`),
  )?.[1]
...
function orthogonalSegmentIntersectsRect(start, end, rect) {
  ...
  throw new Error(
    `expected an orthogonal route segment: ${JSON.stringify([start, end])}`,
  )
}
...
function compileSuccessfully(workflow, qualityProfile) {
  const request =
    qualityProfile === undefined ? { workflow } : { workflow, qualityProfile }
  ...
  assert.match(
    result.svg,
    /^\s*<svg\b/,
    "compiler should return one canonical renderer SVG",
  )
...
function assertReadableAdjacentResult(
  result,
  { label, widths = [92, 92] } = {},
) {
  ...
  assert.ok(
    ...
    "same-lane nodes must not overlap",
  )
  ...
  assert.equal(
    ...
    "edge must leave the source right side",
  )
  ...
  assert.ok(
    directClearance >= 28,
    `direct clearance ${directClearance}px must be at least 28px`,
  )
...
test("fixed-v1 compiler preserves the exact 700x400 compatibility geometry", () => {
  const workflow = readJson(
    path.join(__dirname, "fixtures", "v1-workflow-700x400.workflow.json"),
  )
...
test("fixed-v1 reports one causal column-capacity diagnostic for issue #126", () => {
  ...
  assert.ok(
    diagnostic.supportedFixes.some((fix) => /schema_version 2/.test(fix)),
  )
...
test("fixed-v1 verifies the real coordinate migration before advertising it", () => {
  ...
  assert.ok(
    result.diagnostics[0].supportedFixes.includes(
    ...
    for (let fromCol = 0; fromCol < 5; fromCol += 1) {
      for (const label of [undefined, "liga"]) {
        const result = compileSuccessfully(
          adjacentWorkflow({ fromCol, label }),
          qualityProfile,
        )
      ...
      for (const label of [undefined, "liga"]) {
        for (const frames of [false, true]) {
          for (const width of [900, 1080, 1400, 1600]) {
            ...
            assertReadableAdjacentResult(result, { label })
            if (frames) {
              assert.match(result.svg, /data-composition-frame-kind="group"/)
            }
            cases += 1
          }
        }
...
test("readable-v2 phase and group frames derive from solved ranks without moving the core geometry", () => {
  for (const qualityProfile of ["standard", "showcase"]) {
    for (let fromCol = 0; fromCol < 5; fromCol += 1) {
      ...
      assert.deepEqual(
        edgePoints(framed.svg, "ab"),
        edgePoints(plain.svg, "ab"),
      )
...
test("readable-v2 never routes through a single-rank group label", () => {
  ...
  const attributes = rendered.svg.match(
    new RegExp(`<text\\b([^>]*)>${escapedLabel}</text>`),
  )?.[1]
...
test("readable-v2 shifts top-side routes beyond lane header text deterministically", () => {
  ...
  for (const label of ["Responsibility owner", "R"]) {
    ...
    assert.deepEqual(
      second,
      first,
      `${label}: repeated compilation must be deterministic`,
    )
  ...
  for (const qualityProfile of ["standard", "showcase"]) {
    for (let fromCol = 0; fromCol < 5; fromCol += 1) {
      ...
      for (const width of [900, 1080, 1400, 1600]) {
        ...
        assert.deepEqual(
          result.receipt.requiredViewBox,
          intrinsic.receipt.requiredViewBox,
        )
...
test("capacity-only viewBox failures preserve the intrinsic routes, labels, and requirement", () => {
  ...
  assert.deepEqual(
    capacity.evidence.requiredViewBox,
    intrinsic.receipt.requiredViewBox,
  )
  ...
  assert.deepEqual(
    sufficient.receipt.requiredViewBox,
    intrinsic.receipt.requiredViewBox,
  )
...
test("explicit viewBox capacity names the rank and label constraints that require more width", () => {
  ...
  assert.ok(
    diagnostic.evidence.contributors.includes("rank 1→2 direct clearance"),
  )
...
test("explicit viewBox capacity names the rank and authored node widths that require more width", () => {
  ...
  assert.ok(
    contributors.some(
      (contributor) =>
        /rank 1→2/i.test(contributor) && /clearance/i.test(contributor),
    ),
  ...
  for (const nodeId of ["a", "b"]) {
    assert.ok(
      contributors.some(
        (contributor) =>
          new RegExp(`node ${nodeId}\\b`, "i").test(contributor) &&
          /width/i.test(contributor),
      ),
...
test("readable-v2 compares long-label gutter growth with a legal automatic channel", () => {
  ...
  assert.ok(
    automatic.receipt.requiredViewBox[0] <
      forcedStraight.receipt.requiredViewBox[0],
  ...
  const leftRankDocument = adjacentWorkflow({
    fromCol: 0,
    label: "X".repeat(100),
  })
...
test("readable-v2 contains a long single-rank group label in its frame and intrinsic canvas", () => {
  ...
  assert.ok(
    group.width >= 120 * 5.6 + 20,
    `group width ${group.width} must contain its label`,
  )
...
test("readable-v2 keeps a first-rank group label mask clear of its lane header mask", () => {
  const workflow = {
    ...
    nodes: [
      { id: "member", lane: "main", col: 0, type: "backend", label: "Member" },
    ],
    edges: [],
  }
...
test("readable-v2 measures multi-row legends into intrinsic and explicit viewBox capacity", () => {
  ...
  assert.ok(
    intrinsic.receipt.requiredViewBox[1] > hidden.receipt.requiredViewBox[1],
  )
  ...
  const tooShort = compileWorkflow({
    workflow: tooShortDocument,
    qualityProfile: "showcase",
  })
  assert.equal(tooShort.ok, false)
  assert.ok(
    tooShort.diagnostics.some(
      ({ code }) => code === "workflow/viewbox-capacity",
    ),
  )
  ...
  for (const width of [900, 1400]) {
    ...
    assert.deepEqual(
      explicit.receipt.requiredViewBox,
      expectedGeometry.requiredViewBox,
    )
...
test("explicit labelAt participates in readable-v2 viewBox containment", () => {
  ...
  assert.ok(
    diagnostic.evidence.contributors.some((contributor) =>
      /edge ab label/i.test(contributor),
    ),
...
test("readable-v2 SVG and receipt bytes are deterministic across repeated and reordered input", () => {
  ...
  for (let iteration = 0; iteration < 3; iteration += 1) {
    ...
    assert.equal(
      JSON.stringify(repeated.receipt),
  ...
  assert.equal(
    JSON.stringify(reorderedResult.receipt),
    JSON.stringify(first.receipt),
  )
})
