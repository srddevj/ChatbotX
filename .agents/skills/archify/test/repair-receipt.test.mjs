...
test("repair receipt: validate and deliver share exact Clean Flow evidence while delivery preserves the trusted artifact", () => {
  ...
  assert.ok(
    validateRepair.supportedFixes.some((fix) => fix.includes("route/via")),
  )
...
test("repair receipt: public validate reports borderline desktop readability with a supported fix", () => {
  ...
  assert.ok(
    repair.evidence.projectedFontPx < repair.evidence.minimumProjectedFontPx,
  )
  assert.ok(
    repair.supportedFixes.some((fix) =>
      fix.includes("reduce the viewBox width"),
    ),
  )
})
