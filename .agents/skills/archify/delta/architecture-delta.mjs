...
function stableIndex(
  ...
  ;(items || []).forEach((item, itemIndex) => {
    if (!item?.id) {
      missing.push(`/${collection}/${itemIndex}/id`)
...
function boundaryIndex(boundaries, side) {
  ...
  for (const boundary of boundaries || []) {
    ...
    if (index.has(key)) {
      ambiguous.push(`${boundary.kind}:${boundary.label}`)
    } else {
      index.set(key, boundary)
    }
  }
  if (ambiguous.length) {
    fail(
...
function normalizedField(item, field) {
  ...
  if (field === "wraps" && Array.isArray(value)) {
    return sorted(value)
  }
  return value
}
...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (
    baseRepository &&
    headRepository &&
    baseRepository.url !== headRepository.url
  ) {
    fail(
...
function addState(tag, change, side, forcedState) {
  const append = (attributes) =>
    tag.endsWith("/>")
      ...
      : tag.replace(/>$/, `${attributes}>`)
  if (!(change || forcedState)) {
    return append(' data-delta-state="same"')
  }
  let state = forcedState || change.status
  if (change?.status === "added" && side === "base") {
    state = "same"
  }
  if (change?.status === "removed" && side === "head") {
    state = "same"
  }
...
function forceElementState(markup, state, classifications = []) {
  ...
  if (markup.includes("data-node-id=")) {
    result = addNodeMarker(result, state)
  }
  return result
}
...
function expectedReviewTargetSignature(row) {
  ...
  if (row.kindKey === "component") {
    ...
    for (const form of forms) {
      ...
      if (form.label) {
        descriptors.push(`g:${form.state}:${classifications}`)
      }
    }
  }
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (!targets.length) {
      failures.push(`missing Delta identity ${row.key}`)
    }
  ...
  if (
    /\b(?:SAFE|LOW RISK|MERGEABLE|NO IMPACT|VERIFIED PR)\b/i.test(deltaShell)
  ) {
