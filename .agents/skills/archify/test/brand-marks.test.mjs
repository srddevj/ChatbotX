...
test("third-party notices cover every recorded individual mark license", () => {
  ...
  assert.equal(
    THIRD_PARTY_NOTICE_DISCLOSURE_COUNT,
    34,
    "notice contract changed without review",
  )
  assert.deepEqual(validateThirdPartyNotices(notices), {
    ok: true,
    missing: [],
  })
  assert.equal(
    licensedMarks.length,
    8,
    "pinned Simple Icons license inventory changed",
  )
  for (const mark of licensedMarks) {
    assert.match(
      notices,
      new RegExp(
        `\\| ${mark.title.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")} \\|`,
      ),
    )
    assert.ok(
      notices.includes(mark.provenance.source),
      `${mark.id} source must be disclosed`,
    )
    assert.ok(
...
function nodeBlock(html, id) {
  ...
  return html.slice(
    start,
    candidates.length ? Math.min(...candidates) : html.length,
  )
}
...
test("generated catalog exposes a substantial, unique, provenance-backed preset library", () => {
  ...
  assert.equal(
    new Set(BRAND_MARKS.map((mark) => mark.id)).size,
    BRAND_MARKS.length,
  )
...
test("all five renderers keep the semantic sigil and add one export-safe brand badge", () => {
  for (const type of Object.keys(cases)) {
    ...
    assert.match(
      html,
      /data-brand-mark="openai"[^>]+data-brand-status="preset"/i,
      type,
    )
    ...
    const semantic = block.match(
      /data-semantic-sigil[^>]+translate\(([-\d.]+) ([-\d.]+)\)/,
    )
    ...
    const [brandX, brandY, brandWidth, brandHeight, brandRadius] = brand
      .slice(1, 6)
      .map(Number)
    ...
    assert.equal(
      brandY,
      semanticY,
      `${type}: brand and semantic marks share a top rail`,
    )
  }
})
  ...
  for (const type of ["architecture", "sequence", "dataflow", "lifecycle"]) {
    const input = writeFixture(
      ...
      (_diagram, node) => {
        ...
        if (type === "dataflow" || type === "lifecycle") {
          node.width = 48
        }
      },
    )
    const { result, html } = renderSync(
      type,
      input,
      `narrow-brand-rail-${type}`,
    )
...
test("branded lifecycle states move the semantic stamp left and keep the brand at upper right", () => {
  ...
  const semanticX = Number(
    block.match(/data-semantic-sigil[^>]+translate\(([-\d.]+)/)?.[1],
  )
  const brandX = Number(
    block.match(/data-brand-mark[^>]+translate\(([-\d.]+)/)?.[1],
  )
  assert.ok(
    Number.isFinite(semanticX) && Number.isFinite(brandX) && semanticX < brandX,
    block,
  )
...
test("unknown URL strings fail closed until an exact captured digest is authored", () => {
  const input = writeFixture(
    "architecture",
    "unpinned-link",
    "https://brand.example.invalid/",
  )
  const result = spawnSync(
    process.execPath,
    [cli, "validate", "architecture", input, "--json"],
    {
      cwd: skillRoot,
      encoding: "utf8",
    },
  ...
  assert.ok(
    receipt.diagnostics.some((entry) => entry.code === "brand/unpinned-url"),
  )
  assert.ok(
...
test("capture command returns a digest-pinned brand object that renders reproducibly", async () => {
  ...
  try {
    ...
    assert.match(
      rendered.html,
      new RegExp(`data-brand-sha256="${receipt.brand.sha256}"`),
    )
    assert.ok(
...
test("a pinned brand fails closed when the remote icon digest changes", async () => {
  ...
  try {
    ...
    const receipt = JSON.parse(result.stdout)
    assert.equal(
      receipt.diagnostics.filter(
        (entry) => entry.code === "brand/digest-mismatch",
      ).length,
      1,
    )
  } finally {
...
test("a pinned brand keeps identical artifact metadata when the remote page title changes", async () => {
  ...
  try {
    ...
    const input = writeFixture(
      "architecture",
      "stable-title",
      JSON.parse(capture.stdout).brand,
    )
    ...
    const second = await renderAsync(
      "architecture",
      input,
      "stable-title-second",
      { ARCHIFY_BRAND_ALLOW_PRIVATE: "1" },
...
test("each prepare call rechecks pinned remote bytes instead of trusting a process-wide cache", async () => {
  ...
  try {
    ...
    if (priorAllowPrivate === undefined) {
      delete process.env.ARCHIFY_BRAND_ALLOW_PRIVATE
    } else {
      process.env.ARCHIFY_BRAND_ALLOW_PRIVATE = priorAllowPrivate
    }
    await new Promise((resolve) => server.close(resolve))
  }
})
...
test("capture blocks IPv4-mapped IPv6 loopback and metadata destinations before connecting", async () => {
  for (const url of [
    ...
    assert.notEqual(
      capture.status,
      0,
      `${url}: ${capture.stderr || capture.stdout}`,
    )
...
test("capture rejects remote SVG even when the document appears passive", async () => {
  ...
  try {
    ...
    assert.match(
      capture.stderr,
      /unsupported brand image type image\/svg\+xml/i,
    )
  } finally {
    await new Promise((resolve) => server.close(resolve))
  }
  ...
  const server = http.createServer((request, response) => {
    ...
    response.end(
      `<!doctype html><title>Fallback mark</title>${Array.from(
        { length: 6 },
        (_, index) =>
          `<link rel="icon" type="image/svg+xml" href="/mark-${index}.svg">`,
      ).join("")}`,
    )
  })
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve))
  ...
  try {
    ...
    const capture = await runCliAsync(
      [
        "brands",
        "capture",
        `http://127.0.0.1:${address.port}/mark.png`,
        "--json",
      ],
      { ARCHIFY_BRAND_ALLOW_PRIVATE: "1" },
    )
...
test("viewer exposes brand identity to Passport and Finder while keeping source beacons clear", () => {
  ...
  assert.match(
    template,
    /brandOffset = node\.hasAttribute\('data-node-brand'\) \? 24 : 0/,
  )
  assert.match(
    template,
