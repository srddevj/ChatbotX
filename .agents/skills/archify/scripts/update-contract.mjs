...
export function validateCanonicalUtcTimestamp(value) {
  if (typeof value !== "string" || !UTC_SECONDS.test(value)) {
    throw new UpdateContractError(
      "publication time must use YYYY-MM-DDTHH:mm:ssZ",
    )
  }
  ...
  if (
    ...
    throw new UpdateContractError(
      "publication time is not a real UTC calendar instant",
    )
  }
  return value
}
...
export function validateLocalRelease(value) {
  ...
  if (value.channel !== expectedChannel) {
    throw new UpdateContractError(
      "local release channel does not match its version",
    )
  }
  return {
...
export function validateReleaseNotesUrl(value, version) {
  ...
  if (value !== expected) {
    throw new UpdateContractError(
      "release notes URL is outside the exact trusted release path",
    )
  }
  return value
}
