...
const benchmark = path.join(
  repoRoot,
  "benchmarks/ordinary-model-floor/benchmark.mjs",
)
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-ordinary-model-floor-"),
)
...
function renameCandidateIds(candidate, mapping) {
  ...
  for (const activation of candidate.activations || []) {
    activation.participant =
      mapping.get(activation.participant) || activation.participant
  }
...
test("benchmark verifies one first-pass architecture candidate through semantic, renderer, and visual-review gates", () => {
  ...
  assert.deepEqual(receipt.gates.validation.composition, {
    errors: 0,
    warnings: 0,
  })
...
test("benchmark rejects a renderer-valid candidate that changes required technical roles or relationship labels", () => {
  ...
  source.components.find((component) => component.id === "cache").type =
    "frontend"
...
test("benchmark applies the same semantic and delivery seam to workflow, sequence, data-flow, and lifecycle candidates", () => {
  const cases = [
    {
      ...
      relationships: [
        { from: "router", to: "approval", label: "needs approval?" },
      ],
    },
    {
      type: "sequence",
    ...
    },
    {
      ...
      relationships: [
        { from: "approval", to: "cancelled", variant: "security" },
      ],
    },
  ]
  ...
  for (const item of cases) {
    ...
    assert.equal(
      result.status,
      0,
      `${item.type}: ${result.stderr || result.stdout}`,
    )
    const receipt = JSON.parse(result.stdout)
...
test("benchmark records a timeout without a candidate as a complete first-pass failure", () => {
  ...
  assert.deepEqual(failureReceipt.operational, {
    status: "failed",
    reason: "timeout",
  })
...
test("benchmark semantic requirements bind by accepted technical labels instead of forcing model-authored internal IDs", () => {
  ...
  source.components.find((component) => component.id === "users").label =
    "Browser Users"
  ...
  source.connections.find(
    (connection) =>
      connection.from === "service-api-v1" &&
      connection.to === "redis-cache-v1",
...
test("checked-in cases accept equivalent ordinary-model vocabulary without weakening required topology", () => {
  ...
  const fixtures = [
    {
      ...
      mutate(candidate) {
        candidate.components.find(
          (node) => node.id === "browser-clients",
        ).label = "Browser Clients"
        candidate.components.find((node) => node.id === "cdn-edge").label =
          "CDN Edge"
        candidate.components.find((node) => node.id === "app-api").label =
          "App API"
      },
    },
    {
      ...
      mutate(candidate) {
        candidate.nodes.find((node) => node.id === "task-planner").label =
          "Intent Planner"
        Object.assign(
          candidate.nodes.find((node) => node.id === "risk-router"),
          { label: "Route Decision", type: "security" },
        )
        candidate.nodes.find((node) => node.id === "consent-check").label =
          "Approval"
        candidate.nodes.find((node) => node.id === "tool-runner").label =
          "Tool Dispatch"
        candidate.nodes.find((node) => node.id === "request-blocked").label =
          "Held"
        candidate.nodes.find((node) => node.id === "service-provider").label =
          "Remote API"
    ...
    },
    {
      ...
      mutate(candidate) {
        ...
        candidate.participants.find(
          (node) => node.id === "dashboard-api",
        ).label = "Dashboard API"
        candidate.participants.find((node) => node.id === "token-guard").label =
          "JWT Guard"
        candidate.messages.find(
          (message) => message.id === "verify-jwt",
        ).label = "verify Bearer JWT"
        candidate.messages.find(
          (message) => message.id === "cache-read",
        ).label = "GET dashboard key"
        candidate.messages.find(
          (message) => message.id === "profile-query",
        ).label = "SELECT profile + metrics"
        candidate.messages.find(
          (message) => message.id === "cache-write",
        ).label = "SETEX profile 300"
      },
    },
    {
      type: "dataflow",
      ...
      mutate(candidate) {
        candidate.nodes.find((node) => node.id === "edge-collector").label =
          "Edge Ingestion"
        candidate.nodes.find((node) => node.id === "consent-policy").label =
          "Consent Policy"
        candidate.nodes.find((node) => node.id === "identity-vault").label =
          "Identity Vault"
        candidate.nodes.find((node) => node.id === "facts-warehouse").label =
          "Analytics Warehouse"
        candidate.nodes.find((node) => node.id === "metric-dashboards").label =
          "Analytics UI"
        candidate.flows.find((flow) => flow.id === "consent-enrichment").label =
          "identity context"
        candidate.flows.find((flow) => flow.id === "accepted-events").label =
          "telemetry"
        candidate.flows.find((flow) => flow.id === "identity-map").label =
          "encrypted identity"
        candidate.flows.find((flow) => flow.id === "metrics-query").label =
          "metrics query"
      },
    },
    {
      ...
      mutate(candidate) {
        candidate.states.find((node) => node.id === "approval-wait").label =
          "Approval Pending"
        candidate.states.find((node) => node.id === "input-wait").label =
          "Pending Input"
  ...
  for (const fixture of fixtures) {
    const source = JSON.parse(
      fs.readFileSync(
        path.join(skillRoot, "examples", fixture.example),
        "utf8",
      ),
    )
    const candidate = renameCandidateIds(source, fixture.mapping)
    fixture.mutate(candidate)
    const candidateFile = writeJson(
      `calibrated-${fixture.type}.json`,
      candidate,
    )
    ...
    assert.equal(
      result.status,
      0,
      `${fixture.type}: ${result.stderr || result.stdout}`,
    )
    assert.equal(
      JSON.parse(result.stdout).gates.semantic.ok,
      true,
      fixture.type,
    )
    ...
    if (fixture.type === "workflow") {
      ...
      wrongRoleCandidate.nodes.find((node) => node.id === "risk-router").type =
        "external"
      const wrongRoleFile = writeJson(
        "calibrated-workflow-wrong-role.json",
        wrongRoleCandidate,
      )
      const wrongRole = run([
        "verify",
      ...
      assert.deepEqual(
        JSON.parse(wrongRole.stdout).gates.semantic.mismatchedNodes,
        [
          {
            id: "router",
            field: "type",
      ...
      const brokenCandidate = writeJson(
        "calibrated-workflow-missing-route.json",
        candidate,
      )
      const broken = run([
        "verify",
      ...
      assert.equal(
        JSON.parse(broken.stdout).gates.semantic.missingRelationships.length,
        1,
      )
    }
  }
...
test("checked-in benchmark suite covers all five diagram types without presenting reference fixtures as model evidence", () => {
  const manifest = path.join(
    repoRoot,
    "benchmarks/ordinary-model-floor/manifest.json",
  )
...
test("checked-in prompts permit bundled CLI repair while retaining external validation authority", () => {
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(suiteRoot, "manifest.json"), "utf8"),
  )
...
test("suite integrity rejects a long prompt that omits the attempt-1 file contract", () => {
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(sourceSuite, "manifest.json"), "utf8"),
  )
  manifest.cases = manifest.cases.map((entry, index) => ({
    ...entry,
    case: path.resolve(sourceSuite, entry.case),
    prompt:
      index === 0 ? incompletePrompt : path.resolve(sourceSuite, entry.prompt),
  ...
  const receipt = JSON.parse(result.stdout)
  assert.equal(
    receipt.cases.find((item) => item.caseId === "web-runtime-architecture")
      .promptOk,
    false,
  )
})
...
test("benchmark fails closed with machine-readable errors for malformed JSON and mismatched run identity", () => {
  ...
  assert.deepEqual(JSON.parse(mismatch.stdout), {
    ...
    error: {
      ...
      message:
        'run case_id "different-case" does not match benchmark case "identity-case"',
    },
  })
...
test("packaged skill puts a bounded ordinary-model path before progressive feature references", () => {
  ...
  const progressiveReferences = skill.indexOf(
...
test("dated three-model evidence retains every frozen attempt-1 candidate and truthful gate result", () => {
  ...
  assert.equal(
    evidence.generation.repositoryCommit,
    "66414c7d2366d16a70c9e7282836e416b7917d51",
  )
  assert.equal(
...
test("post-fix evidence keeps the complete matrix and reports the no-uplift comparison truthfully", () => {
  ...
  assert.equal(
    evidence.generation.repositoryCommit,
    "2dce766ab19ff5871020828eb85d770745f1069d",
  )
  assert.equal(
    evidence.generation.packageSha256,
  ...
  const minimaxLifecycle = evidence.runs.find(
    (entry) =>
      entry.model === "codewiz-anthropic/minimax-m3" &&
...
test("quality-first evidence preserves the complete matrix and the measured lifecycle gain without overstating uplift", () => {
  ...
  assert.equal(
    evidence.generation.repositoryCommit,
    "7eef4db36a97d04da74a9cb1d1bc3f735058c074",
  )
  assert.equal(
    evidence.generation.packageSha256,
  ...
  assert.equal(
    evidence.comparison.qualityFirst.firstPassUsableByCase[
      "agent-run-lifecycle"
    ],
    1,
  )
  ...
  const deepseekWorkflow = evidence.runs.find(
    (entry) =>
      entry.model === "seal/deepseek-v4-flash" &&
