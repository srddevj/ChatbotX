...
test("all five renderers inherit one resolved-only Route Share Card Export item", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      html,
      /id="route-probe-share"|class="route-probe-share"/,
      mode,
    )
...
test("Route Share Card menu lifecycle excludes hidden state from keyboard navigation", () => {
  ...
  assert.match(
    html,
    /function open\(focusLast\)[\s\S]*?syncRouteShareItem\(\);/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /function clear\(options\)[\s\S]*?Archify\.exportMenu\.syncRouteShare\(\)/,
  )
  assert.match(
    html,
...
test("Route Share Card snapshots copy exact resolved node and relationship identity without rerouting", () => {
  ...
  const snapshotBlock =
    html.match(/function exportSnapshot\(\) \{[\s\S]*?\n {6}\}/)?.[0] || ""
  const geometryBlock =
    html.match(
      /function hasDrawableGeometry\(element\) \{[\s\S]*?\n {4}\}/,
    )?.[0] || ""
  ...
  assert.match(
    snapshotBlock,
    /drawableFragments = fragments\.filter\(hasDrawableGeometry\)/,
  )
...
test("Route Share Card snapshot fails closed when the resolved DOM becomes stale or conflicting", () => {
  ...
  assert.match(
    snapshotBlock,
    /activeEdges\.length !== activeNodeIds\.length - 1/,
  )
...
test("Route variant decorates only a finite canonical clone with dedicated static attributes", () => {
  ...
  const applyBlock =
    html.match(
      /function applyRouteSnapshot\(clone, snapshot\) \{[\s\S]*?\n {6}\}/,
    )?.[0] || ""
  assert.match(
    applyBlock,
    /snapshot\.edges\.length !== snapshot\.nodeIds\.length - 1/,
  )
  ...
  assert.match(
    applyBlock,
    /drawableMatches = matchedEdges\.filter\(hasDrawableGeometry\)/,
  )
  ...
  assert.match(
    html,
    /Number\.isFinite\(vb\.width\)[\s\S]*?vb\.width > 0 && vb\.height > 0/,
  )
  assert.ok(
...
test("Route Share Card reuses one 1200x630 variant seam and publishes a truthful receipt", () => {
  ...
  assert.match(
    html,
    /clearExportReceipt\(\);[\s\S]*?var snapshot = Archify\.routeProbe/,
  )
  assert.match(
    html,
    /function runExport\(format\)[\s\S]*?clearExportReceipt\(\);/,
  )
  assert.match(
    html,
...
test("skill and READMEs describe the optional Export variant and show one real card without changing the hero", () => {
  ...
  const png = fs.readFileSync(
    path.join(repoRoot, "docs/assets/archify-route-share-card.png"),
  )
