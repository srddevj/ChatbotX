...
function executeStartPage(html) {
  ...
  ids["start-data"] = new FakeElement({
    id: "start-data",
  ...
  const agents = ["cursor", "codex", "claude-code", "opencode"].map(
    (agent) =>
      new FakeElement({
        textContent: agent === "codex" ? "Codex" : agent,
        dataset: { agent },
      }),
  )
  ...
  const document = {
    ...
    querySelector(selector) {
      ...
      return match
        ? agents.find((element) => element.dataset.agent === match[1])
        : null
    },
    querySelectorAll(selector) {
      ...
      if (selector === "[data-agent]") {
        return agents
      }
      if (selector === "[data-input]") {
        return inputs
      }
      if (selector === "[data-copy-source]") {
        return copySources
      }
      if (selector === "[data-en][data-zh]") {
        return []
      }
      return []
    },
  ...
  const window = {
    location: {
      href: "https://example.test/start.html",
      search: "",
      pathname: "/start.html",
    },
...
test("start page: checked-in HTML is reproducible from canonical scenario recipes", () => {
  ...
  try {
    execFileSync(process.execPath, [
      path.join(repoRoot, "scripts/build-start.mjs"),
      generated,
    ])
    assert.equal(
  ...
  for (const [type, id] of selected) {
    ...
    for (const language of ["en", "zh"]) {
      ...
      assert.equal(
        ...
        recipe.start[language].descriptionPrompt,
      )
      assert.ok(
        prompts.repositoryPrompt
          .toLowerCase()
          .includes(recipe[language].prompt.toLowerCase()),
      )
    }
  }
})
...
test("start page: input mode drives rendered prompt, copy, keyboard, and URL without changing event schema", async () => {
  ...
  const [viewEvent, promptEvent, starterEvent] =
    page.window.ArchifyStartMetrics.snapshot()
  ...
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(
        html,
        /source=artifact/,
        `${type}: removed artifact CTA URL`,
      )
      assert.match(
        html,
        /id="btn-diagram-guide"/,
        `${type}: diagram guide remains available`,
      )
