...
for (const [index, transition] of asArray(lifecycle.transitions).entries()) {
  if (!stateSteps.has(transition.from)) {
    stateSteps.set(transition.from, index)
  }
  if (!stateSteps.has(transition.to)) {
    stateSteps.set(transition.to, index + 1)
  }
}
for (const [index, state] of asArray(lifecycle.states).entries()) {
  if (!stateSteps.has(state.id)) {
    stateSteps.set(state.id, index)
  }
}
