...
function artifact(name = "diagram.html") {
  ...
  fs.writeFileSync(
    file,
    "<!doctype html><html><body>checked artifact</body></html>",
  )
  return file
}
...
function fakeBrowser({
  ...
  return {
    ...
    async inspect({ width, height, theme, screenshotPath }) {
      ...
      const overflow = overflowAt?.({ width, height, theme })
      const unreadable = unreadableAt?.({ width, height, theme })
      const chromeCollision = chromeCollisionAt?.({ width, height, theme })
      const stageCollision = stageCollisionAt?.({ width, height, theme })
      const dockStageGap =
        stageGapAt?.({ width, height, theme }) ?? (stageCollision ? -12 : 10)
...
test("visual-check disables the Chrome sandbox only for root or an explicit environment opt-in", () => {
  ...
  const ordinary = chromeVisualBrowserArgs(profileRoot, {
    env: {},
    getuid: () => 1001,
  })
  ...
  const root = chromeVisualBrowserArgs(profileRoot, {
    env: {},
    getuid: () => 0,
  })
...
test("visual-check records four containment viewports and four endpoint theme captures", async () => {
  ...
  assert.equal(
    result.receipt.containment.viewports.length,
    VISUAL_CHECK_VIEWPORTS.length,
  )
  assert.equal(
  ...
  assert.deepEqual(
    result.receipt.captures.screenshots.map(({ width, height, theme }) => [
      width,
      height,
      theme,
    ]),
  ...
  assert.equal(
    sha256(input),
    before,
    "visual-check mutated the delivered artifact",
  )
  ...
  for (const screenshot of outputs.screenshots) {
    assert.match(
      contactSheet,
      new RegExp(
        path.basename(screenshot.path).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
      ),
    )
    assert.doesNotMatch(
      contactSheet,
...
test("visual-check returns 1 when the navigation dock enters the SVG stage", async () => {
  ...
  assert.ok(
    diagnostic?.supportedFixes.some((fix) => fix.includes("dockStageGap")),
  )
  assert.equal(
...
test("visual-check returns 1 and removes misleading capture sidecars on screenshot failure", async () => {
  ...
  for (const screenshot of outputs.screenshots) {
    fs.writeFileSync(screenshot.path, png)
  }
...
test("visual-check returns 2 with a truthful skipped receipt when Chrome is unavailable", async () => {
  ...
  assert.ok(
    result.receipt.diagnostics[0]?.supportedFixes.some((fix) =>
      fix.includes("ARCHIFY_CHROME"),
    ),
  )
  assert.equal(fs.existsSync(sidecarPaths(input).receipt), true)
})
