...
test("guide page: checked-in HTML is reproducible from the shared recipe source", () => {
  ...
  try {
    execFileSync(process.execPath, [
      path.join(repoRoot, "scripts/build-guide.mjs"),
      generated,
    ])
    assert.equal(
...
test("guide page: ships bilingual recipes and syntactically valid interaction code", () => {
  ...
  const releaseIdentity = packageVersion.includes("-")
    ? "development"
    : "stable"
  ...
  assert.match(
    html,
    /archify guide &quot;your scenario&quot;|archify guide "your scenario"/,
  )
  ...
  assert.ok(
    data.every(
      (recipe) => recipe.en.prompt && recipe.zh.prompt && recipe.proof,
    ),
  )
