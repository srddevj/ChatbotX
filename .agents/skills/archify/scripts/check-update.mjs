...
const OPERATION_NAME =
  /^(reserved|pending|committed|fenced|cancelled)-(\d{1,20})$/
const MAX_OPERATION_GENERATION = 10n ** 20n - 1n
const CHECK_TTL_MS = 72 * 60 * 60 * 1000
const MAX_CACHE_HORIZON_MS = Math.ceil(CHECK_TTL_MS * 1.2)
const FIRST_FAILURE_DELAY_MS = 6 * 60 * 60 * 1000
const LATER_FAILURE_DELAY_MS = 24 * 60 * 60 * 1000
const DEFAULT_TIMEOUT_MS = 1000
const MAX_RESPONSE_BYTES = 32 * 1024
const MAX_LOCAL_RELEASE_BYTES = 4 * 1024
const MAX_CACHE_STATE_BYTES = 64 * 1024
const MAX_CLAIM_OWNER_BYTES = 1024
const LONGEST_ISO_TIMESTAMP = "+275760-09-13T00:00:00.000Z"
const LOCK_STALE_MS = 30_000
const ACK_LOCK_WAIT_MS = 1200
const LOCK_RETRY_DELAY_MS = 20
const DIGEST_PATTERN = /^sha256:[a-f0-9]{64}$/
const scriptDirectory = path.dirname(fileURLToPath(import.meta.url))
const defaultReleasePath = path.resolve(
  scriptDirectory,
  "..",
  "skill-release.json",
)
...
function validateCachedCandidate(value) {
  if (
    !(
      ...
      hasOnlyKeys(
        value,
        new Set(["version", "targetDigest", "severity", "releaseNotes"]),
      )
    )
  ) {
    return null
  ...
  try {
    if (!isStableCoreVersion(value.version)) {
      return null
    }
    if (
      !(
...
function stateAfterAcknowledgement(state, targetDigest) {
  const alreadyAcknowledged =
    state.notification.acknowledgedDigests.includes(targetDigest)
  return {
    ...state,
    notification: {
      offeredDigests: state.notification.offeredDigests.filter(
        (digest) => digest !== targetDigest,
      ),
...
function stateWithFailureCheck(
  state,
  nextCheckAt,
  consecutiveFailures,
...
function encodeRecoverableState(state) {
  ...
  if (
    !encodeBoundedState(stateWithFailureCheck(state, LONGEST_ISO_TIMESTAMP, 2))
  ) {
    return null
  }
  let acknowledgementState = state
  for (const targetDigest of state.notification.offeredDigests) {
    acknowledgementState = stateAfterAcknowledgement(
    ...
    if (
      !(
        ...
        encodeBoundedState(
          stateWithFailureCheck(acknowledgementState, LONGEST_ISO_TIMESTAMP, 2),
        )
      )
    ) {
      return null
...
function normalizeState(value, installedVersion) {
  ...
  if (
    ...
    (installedVersion !== null &&
      value.installedVersion !== installedVersion) ||
    (value.installedVersion !== null &&
      typeof value.installedVersion !== "string") ||
    !isPlainObject(value.check) ||
    !(
      value.check.nextCheckAt === null ||
      typeof value.check.nextCheckAt === "string"
    ) ||
...
function isSameFile(left, right) {
  ...
  if (left.birthtimeNs === 0n || right.birthtimeNs === 0n) {
    return false
  }
...
async function readJsonFile(target, maxBytes, expectedMetadata = null) {
  ...
  try {
    ...
    let size = 0
    while (size <= maxBytes) {
      const chunk = Buffer.allocUnsafe(Math.min(8 * 1024, maxBytes + 1 - size))
...
async function canonicalizeTrustedDirectoryPrefix(target) {
  ...
  const trustedDirectories = [
    ...new Set(
      [os.homedir(), os.tmpdir()].map((directory) => path.resolve(directory)),
    ),
  ].sort((left, right) => right.length - left.length)
  for (const trustedDirectory of trustedDirectories) {
    if (!isWithinDirectory(trustedDirectory, absoluteTarget)) {
      continue
    }
    try {
      ...
      return path.resolve(
        canonicalDirectory,
        path.relative(trustedDirectory, absoluteTarget),
      )
    } catch {
...
function invalidateCacheToken(token, message, cause) {
  token.trusted = false
  if (cause instanceof CachePathChangedError) {
    return cause
  }
...
async function verifyDirectorySnapshots(token, snapshots, description) {
  if (!token.trusted) {
    throw new CachePathChangedError("cache directory is no longer trusted")
  }
  try {
...
async function verifyCacheToken(token) {
  await verifyDirectorySnapshots(
    token,
    token.ancestorSnapshots,
    "cache directory",
  )
}
...
function resolveCacheTarget(token, target) {
  ...
  if (
    ...
    !isWithinDirectory(token.directory, resolved)
  ) {
    throw invalidateCacheToken(
      token,
      "cache mutation escaped its prepared directory",
    )
  }
  return resolved
}
...
async function stableCacheEntry(
  ...
  try {
    ...
    throw invalidateCacheToken(
      token,
      "cache mutation produced an untrusted object",
      error,
    )
  }
}
...
async function stablePreparedDirectory(token, target) {
  try {
    ...
    throw invalidateCacheToken(
      token,
      "prepared directory could not be verified",
      error,
    )
  }
}
...
async function assertCacheTargetAbsent(token, target) {
  ...
  try {
    ...
    throw invalidateCacheToken(
      token,
      "cache mutation target could not be verified",
      error,
    )
  }
...
async function guardedCacheMutation(
  ...
  try {
    ...
    if (error instanceof CacheOperationRaceError) {
      throw error
    }
    throw invalidateCacheToken(
      token,
      "cache mutation could not be verified",
      error,
    )
  }
}

async function cacheMkdirWithToken(
  token,
  target,
  options,
  ...
  const resolved = path.resolve(target)
  const parents =
    parentSnapshots ?? (await captureMutationParentSnapshots(token, [resolved]))
    ...
    mutate: async () => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            "cache write did not preserve the opened file identity and size",
          )
        }
...
async function cacheRename(
  ...
  return guardedCacheMutation(token, parents, {
    verifyBefore: () =>
      stableCacheEntry(token, resolvedSource, validate, sourceMetadata, true),
...
async function cacheRm(cacheDirectory, target, options) {
  ...
  return guardedCacheMutation(token, parents, {
    verifyBefore: () =>
      stableCacheEntry(token, resolved, assertSafeCacheEntry, targetMetadata),
        ...
        return [
          {
            ...parsed,
            name: entry.name,
            directory: path.join(cacheDirectory, entry.name),
          },
        ]
      })
    } catch (error) {
...
function corruptClaimStatus(
  directory,
  directoryMetadata,
  ownerMetadata = directoryMetadata,
) {
...
async function retireActiveClaim(cacheDirectory, claim) {
  ...
  await cacheMkdir(cacheDirectory, retirement, { mode: 0o700 }).catch(
    (error) => {
  ...
  try {
    ...
    if (
      !retirementMetadata.isDirectory() ||
      retirementMetadata.isSymbolicLink()
    ) {
      return false
    }
  } catch (error) {
  ...
  try {
    ...
    if (["ENOENT", "EEXIST", "ENOTEMPTY", "EPERM"].includes(error?.code)) {
      return false
    }
    throw error
  }
...
async function prepareOperationClaim(cacheDirectory, operation) {
  const directory = path.join(
    cacheDirectory,
    operationName("claim", operation.generation),
  )
...
async function discardPreparedClaim(cacheDirectory, operation) {
  if (!operation.preparedClaimDirectory) {
    return
  }
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    try {
      ...
      if (raced.state === "absent") {
        if (["EEXIST", "ENOTEMPTY", "EPERM"].includes(promotionError?.code)) {
          continue
        }
        await discardPreparedClaim(cacheDirectory, operation)
    ...
    return (
      await Promise.all(
        pending.map((operation) =>
          operationIsActive(cacheDirectory, operation, token),
        ),
      )
    ).some(Boolean)
  })
...
async function reserveOperation(cacheDirectory) {
  ...
  const occupied = new Set(
    operations.map((operation) => operation.generation.toString()),
  )
  let generation = 1n
  while (true) {
    while (occupied.has(generation.toString())) {
      generation += 1n
    }
    ...
    const reservation = path.join(
      cacheDirectory,
      operationName("reserved", generation),
    )
    try {
    ...
    const directory = path.join(
      cacheDirectory,
      operationName("pending", generation),
    )
    try {
      await cacheMkdir(cacheDirectory, directory, { mode: 0o700 })
      const operation = {
      ...
      try {
        ...
        if (await operationWasFenced(cacheDirectory, operation)) {
          const fencedError = new OperationFencedError(
            "operation was fenced while reserving",
          )
...
async function transitionOperation(cacheDirectory, operation, kind) {
  const destination = path.join(
    cacheDirectory,
    operationName(kind, operation.generation),
  )
  await cacheRename(
    cacheDirectory,
    operation.directory,
...
async function acquireOperation(cacheDirectory) {
  cacheDirectory = await prepareCacheDirectory(cacheDirectory)
  if ((await inspectActiveClaim(cacheDirectory)).state === "busy") {
    return null
  }
  if (await hasActivePendingOperation(cacheDirectory)) {
    return null
  }
  let operation
  try {
    ...
    if (error instanceof OperationFencedError) {
      return null
    }
    throw error
  }
  ...
  for (const candidate of lower) {
    try {
      await cacheRename(
        ...
        path.join(
          cacheDirectory,
          operationName("fenced", candidate.generation),
        ),
        assertSafeDirectory,
      )
    } catch (error) {
...
function digestForEventKey(eventKey) {
  ...
  if (typeof eventKey !== "string" || !eventKey.startsWith(prefix)) {
    return null
  }
...
function resultForCandidate(localRelease, state) {
  ...
  if (
    state.notification.acknowledgedDigests.includes(
      state.candidate.targetDigest,
    )
  ) {
...
function defaultCacheDirectory() {
  ...
  return path.join(
    xdgCache && path.isAbsolute(xdgCache)
      ? xdgCache
...
async function resultAfterLosingOperation(
  cacheDirectory,
  operation,
  localRelease,
) {
...
async function waitUntilRetry(deadline, monotonicNow) {
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, Math.min(LOCK_RETRY_DELAY_MS, remaining)),
  )
  return true
}
...
export async function checkForUpdate({
  ...
  if (typeof fetchImpl !== "function") {
    return silent("runtime-unavailable")
  }
  let localRelease
  try {
    localRelease = validateLocalRelease(
      await readJsonFile(releasePath, MAX_LOCAL_RELEASE_BYTES),
    )
  } catch {
  ...
  if (!operation) {
    try {
      ...
      if (
        cached.candidate &&
        cached.notification.offeredDigests.includes(
          cached.candidate.targetDigest,
        )
      ) {
        return resultForCandidate(localRelease, cached)
      }
  ...
  try {
    ...
    try {
      ...
      const reason =
        error instanceof UpdateContractError
          ? "invalid-manifest"
          : "check-failed"
      try {
        ...
        (current) => {
          const next = {
            ...
            notification: {
              ...
              acknowledgedDigests: [
                ...current.notification.acknowledgedDigests,
              ],
            },
          }
          ...
          if (
            ...
            !next.notification.offeredDigests.includes(
              next.candidate.targetDigest,
            )
          ) {
...
export async function acknowledgeUpdate({
  ...
  while (true) {
    ...
    if (!operation) {
      if (!(await waitUntilRetry(deadline, monotonicNow))) {
        return silent("check-in-progress")
      }
      continue
    }
    try {
      const committed = await commitOperation(
        ...
        (state) => {
          const wasOffered =
            state.notification.offeredDigests.includes(targetDigest)
...
async function runCli() {
  if (process.env.ARCHIFY_UPDATE_CHECK_DISABLED === "1") {
