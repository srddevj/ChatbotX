...
test("all typed renderers inherit one viewer-only Semantic Radar", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="overview-map-surface" tabindex="0" role="group"/,
      mode,
    )
    assert.match(
      html,
      /id="overview-map-expand"[^>]+aria-label="Open full semantic radar"/,
      mode,
    )
    assert.match(
      html,
      /id="overview-map-feedback" role="status" aria-live="polite" hidden/,
      mode,
    )
    assert.match(
      html,
    ...
    assert.equal(
      ...
      1,
      `${mode} keeps one static canonical SVG`,
    )
    assert.doesNotMatch(
      canonicalSvg(html),
      /overview-map|Semantic radar|data-radar-node-id/,
      mode,
    )
  }
})
...
test("Semantic Radar derives semantic node bounds and focuses stable IDs", () => {
  ...
  assert.match(
    html,
    /rect\.setAttribute\('aria-label', viewerText\('viewer\.radar\.focus'/,
  )
...
test("Semantic Radar keeps redundant accessible navigation and clean exports", () => {
  ...
  assert.match(
    html,
    /node && \(event\.key === 'Enter' \|\| event\.key === ' '\)/,
  )
  ...
  assert.doesNotMatch(
    canonicalSvg(html),
    /overview-map-node|overview-map-viewport/,
  )
})
...
test("Semantic Radar stays above the measured MAP control strip", {
  ...
  try {
    ...
    assert.ok(
      controlGap >= 15,
      JSON.stringify({ ...rects, controlGap }, null, 2),
    )
    assert.ok(
...
test("Semantic Radar avoids an expanded mobile Passport without hiding a collision", {
  ...
  try {
    ...
    assert.equal(
      expanded.surfaceVisible,
      true,
      JSON.stringify(expanded, null, 2),
    )
    assert.equal(
      expanded.passportYielded,
      "true",
      JSON.stringify(expanded, null, 2),
    )
    assert.equal(
      expanded.passportVisible,
      false,
      JSON.stringify(expanded, null, 2),
    )
    assert.ok(
...
test("Semantic Radar automatically avoids a tall Semantic Passport", {
  ...
  try {
    ...
    await dragMouse(
      ...
      {
        x:
          dragGeometry.head.left +
          48 +
          dragGeometry.requested.left -
          dragGeometry.radar.left,
        y:
