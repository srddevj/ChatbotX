...
import {
  ...
  renderSemanticSigil,
  textUnits,
} from "../renderers/shared/utils.mjs"
...
test("rectsOverlap: non-finite geometry is not an overlap", () => {
  ...
  assert.equal(
    rectsOverlap(rect(0, 0, 10, 10), rect(20, 0, Number.NaN, 10)),
    false,
  )
  assert.equal(
    rectsOverlap(rect(0, 0, 10, 10), rect(5, 5, 10, Number.POSITIVE_INFINITY)),
    false,
  )
})
...
test("segmentIntersectsRect: detects an edge crossing a node box", () => {
  assert.equal(
    segmentIntersectsRect({ start: [0, 5], end: [20, 5] }, rect(8, 0, 4, 10)),
    true,
  )
  assert.equal(
    segmentIntersectsRect({ start: [0, 20], end: [20, 20] }, rect(8, 0, 4, 10)),
    false,
  )
})
...
test("segmentRectClearance measures horizontal, vertical, and reversed diagonal segments", () => {
  ...
  assert.equal(
    segmentRectClearance({ start: [0, 0], end: [8, 8] }, box),
    Math.sqrt(8),
  )
  assert.equal(
    segmentRectClearance({ start: [8, 8], end: [0, 0] }, box),
    Math.sqrt(8),
  )
...
test("label-route clearance locks tangent, sub-threshold, boundary, and reversed coordinates", () => {
  ...
  const cases = [
    {
      segment: { start: [0, 10], end: [30, 10] },
      clearance: 0,
      intersection: 10,
    },
    {
      segment: { start: [10, 0], end: [10, 30] },
      clearance: 0,
      intersection: 10,
    },
    {
      segment: { start: [0, 0], end: [30, 30] },
      clearance: 0,
      intersection: Math.sqrt(200),
    },
    {
      segment: { start: [0, 0], end: [10, 10] },
      clearance: 0,
      intersection: 0,
    },
    {
      segment: { start: [0, 8.1], end: [30, 8.1] },
      clearance: 1.9,
      intersection: 0,
    },
    { segment: { start: [0, 8], end: [30, 8] }, clearance: 2, intersection: 0 },
    {
      segment: { start: [0, 6.1], end: [30, 6.1] },
      clearance: 3.9,
      intersection: 0,
    },
    { segment: { start: [0, 6], end: [30, 6] }, clearance: 4, intersection: 0 },
    {
      ...
      clearance: Math.sqrt(125),
      intersection: 0,
    },
  ]
  for (const { segment, clearance, intersection } of cases) {
    assert.ok(
      Math.abs(segmentRectClearance(segment, box) - clearance) < 0.000_001,
    )
    assert.ok(
      Math.abs(segmentRectIntersectionLength(segment, box) - intersection) <
        0.000_001,
    )
    const reversed = { start: segment.end, end: segment.start }
    assert.ok(
      Math.abs(segmentRectClearance(reversed, box) - clearance) < 0.000_001,
    )
    assert.ok(
      Math.abs(segmentRectIntersectionLength(reversed, box) - intersection) <
        0.000_001,
    )
  }
})
...
test("collectLabelRouteClearance exempts only the owning relationship at an exact threshold", () => {
  ...
  assert.deepEqual(
    collectLabelRouteClearance({ labels, routedRelations, threshold: 2 }),
    [],
  )
...
test("cleanFlowProblems reports collection index, ids, segment, clearance, and fix", () => {
  ...
  assert.match(
    problems[0],
    /segment 0 \[20, 10\] -> \[80, 10\] \(2px clearance\)/,
  )
...
test("cleanCrossingProblems reports one deterministic proper X in showcase", () => {
  ...
  assert.match(
    problems[0],
    /\[composition\/proper-crossing\] showcase architecture/,
  )
  assert.match(
    problems[0],
...
test("ambiguous corridor gate reports unrelated collinear overlap with exact identities", () => {
  ...
  assert.match(
    problems[0],
    /\[composition\/ambiguous-corridor\] showcase workflow/,
  )
  assert.match(
    problems[0],
...
test("route rhythm is a showcase-only generation gate with actionable relationship identity", () => {
  ...
  assert.deepEqual(
    cleanRouteRhythmProblems({ ...args, profile: "standard" }),
    [],
  )
...
test("isFinitePoint rejects NaN/undefined/Infinity", () => {
  ...
  assert.equal(isFinitePoint(1, Number.NaN), false)
  assert.equal(isFinitePoint(1, undefined), false)
  assert.equal(isFinitePoint(1, Number.POSITIVE_INFINITY), false)
})
