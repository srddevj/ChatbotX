...
for (const autocrlf of ["true", "input", "false"]) {
  test(`Git checkout preserves committed text and binary bytes with core.autocrlf=${autocrlf}`, () => {
    const fixture = fs.mkdtempSync(
      path.join(os.tmpdir(), "archify-checkout-eol-"),
    )
    ...
    const runGit = (cwd, args) => {
      ...
      assert.equal(
        result.status,
        0,
        `git ${args.join(" ")}: ${result.error || result.stderr}`,
      )
    ...
    try {
      ...
      const files = new Map([
        [
          ".gitattributes",
          fs.readFileSync(path.join(repoRoot, ".gitattributes")),
        ],
        ...
        [
          "docs/assets/proof.png",
          Buffer.from([0x89, 0x50, 0x4e, 0x47, 13, 10, 0, 0xff]),
        ],
      ])
      ...
      assert.equal(
        runGit(checkout, ["status", "--porcelain"]).toString().trim(),
        "",
      )
      assert.equal(
        runGit(checkout, [
