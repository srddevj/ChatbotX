...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(
      guided.html,
      /addEventListener\('hashchange', syncViewFromHash\)/,
    )
    ...
    assert.match(
      guided.html,
      /URLSearchParams\(location\.search\)\.get\('play'\) === '1'/,
    )
    ...
    assert.match(
      guided.html,
      /id="share-chapter-cue" hidden role="status" aria-live="polite"/,
    )
    assert.match(
      guided.html,
      /data-share-playback="true"\] \.share-chapter-cue:not\(\[hidden\]\)/,
    )
    assert.match(
      guided.html,
    ...
    assert.match(
      guided.html,
      /startShareCueProgress\(progress, remainingChapter\)/,
    )
...
test("guided views schema enforces collection and focus bounds", () => {
  ...
  duplicateFocus.meta.views = [
    { id: "duplicate", label: "Duplicate", focus: ["web", "web"] },
  ]
