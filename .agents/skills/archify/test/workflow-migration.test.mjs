...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-workflow-migration-"),
)
...
function runValidation(source) {
  return spawnSync(
    process.execPath,
    [cli, "validate", "workflow", source, "--json"],
    {
      encoding: "utf8",
    },
  )
...
test("migration geometry helpers construct mapped v2 candidates without mutating the source", () => {
  ...
  assert.equal(
    mapX(260),
    274,
    "the migration module must preserve the mapper re-export",
  )
...
test("document migration treats a legacy explicit viewBox as capacity and expands it monotonically", () => {
  ...
  assert.deepEqual(
    source,
    sourceBefore,
    "the document migration must not mutate its input",
  )
  assert.deepEqual(migration.document.meta.viewBox, [768, 404])
  assert.deepEqual(
    migration.document.meta.viewBox,
    migration.newRequiredViewBox,
  )
  ...
  assert.ok(
    migration.preExistingDiagnostics.some(({ message }) =>
...
test("CLI atomically commits a capacity-expanded migration and preserves source bytes", () => {
  ...
  const destination = path.join(
    tmp,
    "height-capacity-destination.workflow.json",
  )
  const sourceBytes = Buffer.from(
    `${JSON.stringify(heightConstrainedWorkflow(), null, 2)}\n`,
  )
  ...
  assert.deepEqual(
    ...
    sourceBytes,
    "the CLI must preserve source bytes",
  )
...
test("column-capacity diagnostics do not advertise migration across a quality-profile divergence", () => {
  ...
  const capacity = standard.diagnostics.find(
...
test("document migration uses the authored profile and returns an independently compilable document", () => {
  ...
  assert.equal(
    standalone.ok,
    true,
    JSON.stringify(standalone.diagnostics, null, 2),
  )
})
...
test("CLI migration ignores ambient standard for an authored showcase workflow", () => {
  ...
  const destination = path.join(
    tmp,
    "authored-showcase-destination.workflow.json",
  )
  const sourceBytes = Buffer.from(
    `${JSON.stringify(profileDivergenceWorkflow(), null, 2)}\n`,
  )
  fs.writeFileSync(source, sourceBytes)
...
test("CLI migration ignores ambient showcase for an authored standard workflow", () => {
  ...
  const destination = path.join(
    tmp,
    "authored-standard-destination.workflow.json",
  )
  const document = profileDivergenceWorkflow()
  document.meta.quality_profile = "standard"
...
test("CLI migration makes the blocking new-schema diagnostic primary over pre-existing findings", () => {
  ...
  const destination = path.join(
    tmp,
    "blocking-diagnostic-destination.workflow.json",
  )
  fs.writeFileSync(
    source,
    `${JSON.stringify(profileDivergenceWorkflow(), null, 2)}\n`,
  ...
  assert.equal(report.ok, false)
  assert.equal(
    report.preExistingDiagnostics[0].code,
    "workflow/column-capacity",
  )
  ...
  assert.ok(
    report.diagnostics.some(({ code }) => code === "workflow/column-capacity"),
  )
})
...
test("workflow migration never shrinks an already spacious explicit viewBox", () => {
  ...
  assert.equal(
    parseJsonOutput(result).destination.sha256,
...
test("workflow migration reports the measured legacy requirement separately from authored capacity", () => {
  const source = {
    ...
    lanes: [{ id: "main", label: "Main" }],
    nodes: [
      { id: "only", lane: "main", col: 0, type: "backend", label: "Only" },
    ],
    edges: [],
  }
...
test("workflow migration staging never aliases a destination named like its verification artifact", () => {
  ...
  assert.equal(result.status, 0, result.stderr || result.stdout)
  assert.equal(
    JSON.parse(fs.readFileSync(destination, "utf8")).schema_version,
    2,
  )
  assert.equal(
    parseJsonOutput(result).destination.path,
...
test("workflow migration cleanup failure warns without reversing a successful commit", () => {
  ...
  const destination = path.join(
    tmp,
    "cleanup-warning-destination.workflow.json",
  )
  const importModule = path.join(
    __dirname,
    "fixtures",
  ...
  assert.match(
    result.stderr,
    /Warning: could not remove workflow migration staging directory/,
  )
...
for (const { example, expectedEdge } of [
  {
    ...
    expectedEdge: ["alert", "page"],
  },
  {
    example: "release-delivery.workflow.json",
    expectedEdge: ["pull_request", "build"],
  },
]) {
  test(`workflow migration preserves causal route diagnostics for packaged ${example}`, () => {
    ...
    assert.deepEqual(
      [diagnostic.subject?.from, diagnostic.subject?.to],
      expectedEdge,
    )
...
test("workflow migration rejects the source path as its destination without changing the file", () => {
  ...
  assert.ok(
    failure.diagnostics[0].supportedFixes.some((fix) =>
      /different.*destination/i.test(fix),
    ),
  )
})
...
test("failed workflow migration emits diagnostics and never writes its destination", () => {
  ...
  fs.writeFileSync(
    source,
    `${JSON.stringify(explicitPinConflictWorkflow(), null, 2)}\n`,
  )
  assert.equal(fs.existsSync(destination), false)
