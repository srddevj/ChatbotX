...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: guided paths expose a viewer-only Story Trail`, () => {
    ...
    assert.match(
      html,
      /id="guided-view-trail" hidden role="group" aria-label="Story trail"/,
    )
    ...
    assert.match(
      html,
      /firstEdge\.parentNode\.insertBefore\(overlay, firstEdge\)/,
    )
    ...
    assert.doesNotMatch(
      generatedSvg,
      /data-story-(?:active|playing|beat|step|overlay|carrier)/,
    )
  })
}
