...
test("all five renderers inherit one finite running-to-settled ambient contract", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      html,
      /animation: archify-edge-flow [^;]+ infinite/,
      mode,
    )
    assert.doesNotMatch(
      html,
      /animation: archify-node-pulse [^;]+ infinite/,
      mode,
    )
  }
})
...
test("settled flow restores authored security and async dash semantics", () => {
  ...
  assert.match(
    template,
    /@keyframes archify-edge-flow\s*\{[\s\S]*?stroke-dasharray:\s*10 8/,
  )
  assert.match(
    template,
    /100%\s*\{\s*stroke-dashoffset:\s*0;\s*opacity:\s*1;\s*\}/,
  )
  const runningRule =
    template.match(
...
test("ambient ownership is generation-bounded and cannot replay after settle", () => {
  ...
  assert.match(
    template,
    /if \(paused \|\| owner \|\| html\.hasAttribute\('data-embed'\)/,
  )
  assert.doesNotMatch(
    template,
    /setInterval\([^)]*ambient|addEventListener\('scroll'[^)]*ambient/,
  )
})
...
test("only the WebM canvas scene opts into a repeatable finite motion timeline", () => {
  ...
  assert.doesNotMatch(
    template,
    /serializeSvg\(1, \{ autoTheme: true, motion: true \}\)/,
  )
})
