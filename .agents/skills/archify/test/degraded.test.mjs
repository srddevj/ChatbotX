...
function render(mode, doc) {
  ...
  try {
    execFileSync(
      "node",
      [
        path.join(installedRoot, `renderers/${mode}/render-${mode}.mjs`),
        input,
        out,
      ],
...
function assertFriendlyFailure(mode, doc, label) {
  ...
  assert.doesNotMatch(
    html,
    /NaN|undefined/,
    `${label}: wrote NaN/undefined into HTML`,
  )
}
...
for (const [mode, fields] of Object.entries(ARRAY_FIELDS)) {
  for (const field of fields) {
    test(`${mode}: ${field} as a string fails friendly`, () => {
      ...
      if (!(field in doc)) {
        return // optional field absent in this example
      }
      doc[field] = "oops"
...
test("workflow: node missing col never writes NaN", () => {
  const doc = JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "examples", EXAMPLES.workflow),
      "utf8",
    ),
  )
...
test("lifecycle: state missing col never writes NaN", () => {
  const doc = JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "examples", EXAMPLES.lifecycle),
      "utf8",
    ),
  )
  ...
  for (const mode of ["workflow", "dataflow", "lifecycle"]) {
    ...
    for (let seed = 1; seed <= 8; seed += 1) {
      ...
      assert.equal(
        code,
        0,
        `${mode} seed ${seed}: valid shuffle should render (exit 0)`,
      )
      assert.doesNotMatch(
        html,
        /NaN|undefined>/,
        `${mode} seed ${seed}: NaN in output`,
      )
    }
  }
})
...
test("installed skill rejects unknown fields without node_modules", () => {
  ...
  assert.match(
    stderr,
    /\/nodes\/0 \(id\/label: "user"\) must NOT have additional properties/,
  )
  assert.match(stderr, /"additionalProperty":"colour"/)
  assert.doesNotMatch(
    stderr,
    /ajv is not installed|skipping JSON-schema validation/,
  )
})
