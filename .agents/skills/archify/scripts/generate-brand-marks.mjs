...
const simpleBySlug = new Map(
  Object.values(simpleIcons)
    .filter(
      (icon) => icon && typeof icon === "object" && icon.slug && icon.path,
    )
...
if (
  ...
  !Array.isArray(catalog.marks) ||
  catalog.marks.length === 0
) {
...
const generated = catalog.marks
  .map((entry, index) => {
    ...
    const entryDomains = normalizedList(entry.domains).map((domain) =>
      domain.toLowerCase(),
    )
    ...
    for (const domain of entryDomains) {
      if (domains.has(domain) && domains.get(domain) !== entry.id) {
        fail(
          `domain ${domain} is shared by ${domains.get(domain)} and ${entry.id}`,
        )
      }
    ...
    if (entry.simpleIcon) {
      ...
      if (!icon) {
        fail(
          `${entry.id} references missing Simple Icons slug ${entry.simpleIcon}`,
        )
      }
      mark = {
        id: entry.id,
      ...
      if (
        ...
        fail(
          `${entry.id} custom mark requires title, path, source, and six-digit hex`,
        )
      }
      mark = {
        id: entry.id,
    ...
    if (!(mark.category && mark.title)) {
      fail(`${entry.id} is missing category or title`)
    }
...
if (process.argv.includes("--check")) {
  ...
  if (current !== source) {
    console.error(
      "generated brand marks are stale — run npm run generate:brand-marks",
    )
    process.exit(1)
  }
} else {
  ...
  console.log(
    `generated ${path.relative(root, outputPath)} (${generated.length} marks)`,
  )
}
