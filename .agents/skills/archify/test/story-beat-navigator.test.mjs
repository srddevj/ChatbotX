...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-story-beat-navigator-"),
)
...
test("all five renderers inherit native inspectable Story Beat controls without changing canonical SVG", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      generatedSvg,
      /data-story-(?:active|playing|beat|step|overlay|pulse)/,
    )
  }
})
...
test("adjacent stable IDs classify start, forward, reverse, group, and multiple without inferred cross-links", () => {
  ...
  assert.match(
    template,
    /storySteps\.forEach\(function \(step\) \{\s*step\.edges\.forEach/,
  )
...
test("one generation-owned scheduler resumes remaining dwell and finite exact-edge signals never loop", () => {
  ...
  assert.match(
    template,
    /afterHandoff\(function \(\)[\s\S]*scheduleStoryPlayback\(\)/,
  )
  ...
  assert.match(
    template,
    /addEventListener\('animationend'[\s\S]*clearStoryPulse/,
  )
})
...
test("target size, reduced motion, print, embed, and export keep Story Beats viewer-only", () => {
  assert.match(
    template,
    /\.guided-view-trail \.guided-view-stop \{[\s\S]*min-height: 1\.5rem/,
  )
  assert.match(
    template,
  ...
  assert.match(
    template,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.story-trail-flow/,
  )
  assert.match(
    template,
