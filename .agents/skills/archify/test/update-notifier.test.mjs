...
const childCheckTimeoutMs = 2000
const parentCheckTimeoutMs = 5000
const maxCacheStateBytes = 64 * 1024
...
function cachedStateWithHistory({
  offeredDigests = [],
  acknowledgedDigests = [],
} = {}) {
  return {
...
function candidateStateForDigest(state, digest) {
  return {
    ...state,
    check: {
      nextCheckAt: new Date(baseTime + 72 * 60 * 60 * 1000).toISOString(),
      consecutiveFailures: 0,
    },
    notification: {
...
function writeCompactCommittedState(testFixture, state, generation = 1n) {
  const target = path.join(
    operationPath(testFixture, "committed", generation),
    "state.json",
  )
  fs.mkdirSync(path.dirname(target), { recursive: true })
...
function committedStateFiles(testFixture) {
  return fs
    ...
    .filter(
      (entry) => entry.isDirectory() && /^committed-\d+$/.test(entry.name),
    )
    .map((entry) =>
      path.join(stateDirectory(testFixture), entry.name, "state.json"),
    )
}
...
function fixture(version = "2.15.0") {
  const root = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "archify-update-notifier-")),
  )
...
function statePath(testFixture, version = "2.15.0") {
  ...
  return committed[0]
    ...
    : path.join(
        operationPath(testFixture, "committed", 1n, version),
        "state.json",
      )
}
...
function cachedUpdateState() {
  return {
    ...
    installedVersion: "2.15.0",
    check: {
      nextCheckAt: new Date(baseTime + 24 * 60 * 60 * 1000).toISOString(),
      consecutiveFailures: 0,
    },
    notification: {
...
function runCheckInChild(testFixture) {
  ...
  const diagnostics = [child.error?.stack, child.stderr, child.stdout]
    .filter(Boolean)
    .join("\n")
...
test("SemVer comparison handles stable, prerelease, and downgrade ordering", () => {
  ...
  assert.equal(
    compareSemver("2.16.0-dev.9007199254740993", "2.16.0-dev.9007199254740992"),
    1,
  )
    ...
    options(testFixture, async () =>
      response(
        remoteRelease({
          ...
          releaseNotes:
            "https://github.com/tt-a1i/archify/releases/tag/v2.15.0",
        }),
      ),
    ),
  )
...
test("a changed digest never bypasses same-version or downgrade protection", async () => {
  for (const [version, digest] of [
    ...
    try {
      const result = await checkForUpdate(
        options(testFixture, async () =>
          response(remoteReleaseForVersion(version, digest)),
        ),
      )
      assert.deepEqual(result, { status: "silent", reason: "current" })
    } finally {
...
test("a successful refresh withdraws a previously offered higher candidate", async (t) => {
  ...
  const withdrawn = await checkForUpdate(
    options(testFixture, fetchImpl, {
      now: () => baseTime + 73 * 60 * 60 * 1000,
    }),
  )
  assert.deepEqual(withdrawn, { status: "silent", reason: "current" })
  assert.equal(
    JSON.parse(fs.readFileSync(statePath(testFixture), "utf8")).candidate
      .version,
    "2.15.0",
  )
...
test("a newer immutable candidate is re-offered until the visible notice is acknowledged", async (t) => {
  ...
  assert.equal(
    requests,
    1,
    "fresh cached candidates must not make another request",
  )
...
test("a 64 KiB multi-offer cache only returns an event whose acknowledgement closure can commit", async (t) => {
  ...
  const state = cachedStateWithHistory({
    ...
    acknowledgedDigests: Array.from({ length: 877 }, (_, index) =>
      historyDigest(index),
    ),
  })
  ...
  assert.equal(
    requests,
    1,
    "an unrecoverable cached offer must be rebuilt before exposure",
  )
...
test("a recoverable multi-offer boundary acknowledges every event without pruning history", async (t) => {
  ...
  const currentDigest = `sha256:${"b".repeat(64)}`
  const cachedVersion = `3.16.${"9".repeat(35)}`
  const exactHistory = Array.from({ length: 877 }, (_, index) =>
    historyDigest(index),
  )
  const state = cachedStateWithHistory({
    offeredDigests: [earlierDigest, currentDigest],
  ...
  assert.equal(
    Buffer.byteLength(compactStateSource(state)),
    maxCacheStateBytes - 1,
  )
  writeCompactCommittedState(testFixture, state)
...
test("a near-capacity null schedule does not retry the network on every activation", async (t) => {
  ...
  t.after(() => fs.rmSync(testFixture.root, { recursive: true, force: true }))
  const state = cachedStateWithHistory({
    acknowledgedDigests: Array.from({ length: 883 }, (_, index) =>
      historyDigest(index),
    ),
  })
  ...
  assert.equal(
    requests,
    1,
    "the first check must commit a bounded retry or reusable candidate",
  )
})
...
test("a saturated exact acknowledgement history never returns an unacknowledgeable offer", async (t) => {
  ...
  let index = 0
  while (
    Buffer.byteLength(compactStateSource(projected)) <= maxCacheStateBytes
  ) {
  ...
  assert.equal(
    requests,
    1,
    "capacity rejection should commit a bounded retry delay",
  )
})
...
test("capacity backoff withdraws a stale candidate while preserving its late acknowledgement", async (t) => {
  ...
  let index = 0
  while (
    Buffer.byteLength(compactStateSource(replacement)) <= maxCacheStateBytes
  ) {
    state.notification.acknowledgedDigests.push(historyDigest(index))
    index += 1
...
test("a 64 KiB state can be acknowledged but a 64 KiB plus one state is ignored", async () => {
  ...
  assert.equal(
    Buffer.byteLength(compactStateSource(exactState)),
    maxCacheStateBytes,
  )
...
test("a boundary offer remains acknowledgeable without pruning exact history", async (t) => {
  ...
  while (true) {
    ...
    if (
      Buffer.byteLength(
        compactStateSource(candidateStateForDigest(state, targetDigest)),
      ) > maxCacheStateBytes
    ) {
  ...
  writeCompactCommittedState(testFixture, state)
  const fetchImpl = async () =>
    response(remoteReleaseForVersion("2.16.0", "d".repeat(64)))
...
test("opaque response validators are neither persisted nor replayed after the check TTL", async (t) => {
  ...
  assert.equal(
    Object.hasOwn(
      JSON.parse(fs.readFileSync(statePath(testFixture), "utf8")).check,
      "etag",
    ),
    false,
  )
...
test("failure backoff saturates safely instead of overflowing the cache counter", async (t) => {
  ...
  writeJson(statePath(testFixture), {
    ...
    installedVersion: "2.15.0",
    check: {
      nextCheckAt: new Date(baseTime - 1000).toISOString(),
...
test("a committed cache state symlink is ignored without following its target", (t) => {
  ...
  if (!targetIsFifo) {
    writeJson(linkedTarget, cachedUpdateState())
  }
  fs.symlinkSync(
    linkedTarget,
    cacheStatePath,
...
test("an oversized committed cache state is ignored without an unbounded read", (t) => {
  const testFixture = fixture()
  t.after(() => fs.rmSync(testFixture.root, { recursive: true, force: true }))
  const oversizedState = `${JSON.stringify(cachedUpdateState())}${" ".repeat(256 * 1024)}`
  const cacheStatePath = statePath(testFixture)
  fs.mkdirSync(path.dirname(cacheStatePath), { recursive: true })
...
test("a cache ancestor replaced before a cached read cannot inject a reminder", async (t) => {
  ...
  fsPromises.readdir = async (target, ...args) => {
    if (
      !replaced &&
      path.resolve(target) === path.resolve(stateDirectory(testFixture))
    ) {
      replaced = true
...
test("zero-inode cache metadata uses timestamp fallback to reject a replaced ancestor", async (t) => {
  ...
  fsPromises.lstat = async (target, ...args) => {
    ...
    const isCachePath =
      ...
      resolved.startsWith(
        `${path.resolve(testFixture.cacheDirectory)}${path.sep}`,
      )
...
test("a state write replaced before post-verification is never reported as committed", async (t) => {
  ...
  fsPromises.writeFile = async (target, ...args) => {
    ...
    if (path.basename(target).startsWith(".state.")) {
...
test("a cache trust failure while cancelling cannot return a cached reminder", async (t) => {
  ...
  stale.check.nextCheckAt = new Date(baseTime - 1000).toISOString()
  ...
  fsPromises.rename = async (source, destination, ...args) => {
    ...
    if (
      !refreshed &&
      sourceName.startsWith("claim-") &&
      destinationName === "active-claim"
    ) {
      const result = await originalRename(source, destination, ...args)
...
test("prepared claim cleanup never recursively deletes through a replaced cache ancestor", async (t) => {
  ...
  const discardedClaims = coordinationEntries.filter((name) =>
    name.startsWith("discarded-claim-"),
  )
  ...
  assert.deepEqual(Object.keys(discardedOwner).sort(), [
    "generation",
    "pid",
    "token",
  ])
...
test("a symlink cache root cannot write into its target", async (t) => {
  ...
  const protectedTarget = path.join(
    testFixture.root,
    "simulated-project-config",
  )
  fs.mkdirSync(protectedTarget)
  fs.writeFileSync(
    path.join(protectedTarget, "settings.json"),
    '{"protected":true}\n',
  )
  try {
    fs.symlinkSync(
...
test("a cache ancestor replaced during preparation cannot redirect later writes", async (t) => {
  ...
  const protectedVersion = path.join(
    protectedTarget,
    path.basename(stateDirectory(testFixture)),
  )
  const detachedCache = path.join(testFixture.root, "detached-cache")
  ...
  fs.writeFileSync(
    path.join(protectedVersion, "settings.json"),
    '{"protected":true}\n',
  )
  try {
    fs.symlinkSync(
...
test("a cache ancestor replaced before reservation creation fails closed", async (t) => {
  ...
  fs.writeFileSync(
    path.join(replacementVersion, "settings.json"),
    '{"protected":true}\n',
  )
  try {
    fs.symlinkSync(
  ...
  const reservations = replacementEntries.filter((entry) =>
    entry.startsWith("reserved-"),
  )
  ...
  assert.deepEqual(
    fs.readdirSync(path.join(replacementVersion, reservations[0])),
    [],
  )
})
...
test("a trusted cache prefix switched after validation cannot redirect later writes", async (t) => {
  ...
  assert.ok(
    fs
      .readdirSync(canonicalVersionDirectory)
      .some((entry) => entry.startsWith("committed-")),
  )
})
...
test("concurrent checks use one writer and leave a valid cache", async (t) => {
  ...
  assert.equal(first.status, "update_available")
  assert.doesNotThrow(() =>
    JSON.parse(fs.readFileSync(statePath(testFixture), "utf8")),
  )
})
...
test("an empty precheck snapshot cannot start a second concurrent network request", async (t) => {
  ...
  await fetchStarted
  pause.release()
  assert.deepEqual(await delayedPrecheck, {
    status: "silent",
    reason: "check-in-progress",
  })
  assert.equal(requests, 1)
...
test("two promoters cannot replace and then steal a stale empty active claim", async (t) => {
  ...
  await fetchStarted
  pause.release()
  assert.deepEqual(await delayedRetirement, {
    status: "silent",
    reason: "check-in-progress",
  })
  assert.equal(requests, 1)
...
test("an expired contender cannot fetch after its delayed claim promotion succeeds", async (t) => {
  ...
  pause.release()
  assert.deepEqual(await delayedExpired, {
    status: "silent",
    reason: "check-in-progress",
  })
  assert.equal(requests, 0)
...
test("a stale supersession snapshot cannot let a lower promoter retire a higher claim", async (t) => {
  const testFixture = fixture()
  t.after(() => fs.rmSync(testFixture.root, { recursive: true, force: true }))
  const generationOneClaim = path.basename(
    operationPath(testFixture, "claim", 1n),
  )
...
test("an owner fenced while active ownership is read cannot issue a second GET", {
  ...
  fsPromises.open = async (target, ...args) => {
    ...
    if (
      intercepted ||
      path.resolve(target) !==
        path.join(directory, "active-claim", "owner.json")
    ) {
      return handle
    }
    intercepted = true
  ...
  releaseVerification()
  assert.deepEqual(await suspendedOwner, {
    status: "silent",
...
test("a fenced owner rechecks its active claim immediately before network access", async (t) => {
  ...
  pause.release()
  assert.deepEqual(await delayedOwner, {
    status: "silent",
    reason: "check-in-progress",
  })
  pause.restore()
...
test("a last-good notice remains acknowledgeable after the refresh commits a new candidate", async (t) => {
  ...
  const visibleDuringRefresh = await checkForUpdate(
    options(
      ...
      async () => {
        assert.fail(
          "an overlapping reader must not start another network request",
        )
      },
      {
        now: () => baseTime + 73 * 60 * 60 * 1000,
...
test("an oversized generation name cannot poison future allocation", async (t) => {
  ...
  fs.mkdirSync(stateDirectory(testFixture), { recursive: true })
  fs.mkdirSync(
    path.join(stateDirectory(testFixture), `reserved-${"9".repeat(246)}`),
  )
  let requests = 0
  const fetchImpl = async () => {
    requests += 1
...
test("a lower generation stalled after reservation cannot commit behind a newer generation", async (t) => {
  ...
  pause.restore()
  assert.equal(
    fs.existsSync(operationPath(testFixture, "committed", 2n)),
    false,
  )
...
for (const pausePoint of ["pending owner creation", "state temp creation"]) {
  test(`acknowledgement retries when fenced during ${pausePoint}`, async (t) => {
    ...
    const staleTime = new Date(Date.now() - 60_000)
    fs.utimesSync(
      pausePoint === "pending owner creation"
        ? pending
  ...
  for (const shape of ["file", "symlink", "owner-directory"]) {
    ...
    try {
      ...
      if (shape === "file") {
        ...
        fs.symlinkSync(
          path.join(testFixture.root, "missing-claim-target"),
          activeClaim,
        )
      } else {
      ...
      const staleTime = new Date(Date.now() - 60_000)
      if (shape === "symlink") {
        fs.lutimesSync(activeClaim, staleTime, staleTime)
      } else if (shape === "owner-directory") {
        fs.utimesSync(
          path.join(activeClaim, "owner.json"),
          staleTime,
          staleTime,
        )
      } else {
      ...
      assert.equal(
        requests,
        1,
        `${shape} should be retired after its hard lease`,
      )
    } finally {
      fs.rmSync(testFixture.root, { recursive: true, force: true })
    }
...
test("a delayed corrupt-claim retirement cannot move a successor active claim", async (t) => {
  ...
  const pause = pauseRenameMatchingOnce((source, destination) => {
    if (
      ...
      !path
        ...
        .startsWith("retired-claim-corrupt-")
    ) {
      return false
    }
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(activeClaim, "owner.json"), "utf8"))
      .generation,
    "2",
  )
...
test("repeated corrupt claim contents receive distinct retirement identities", async (t) => {
  ...
  fs.renameSync(
    activeClaim,
    path.join(directory, "test-completed-active-claim"),
  )
  fs.writeFileSync(activeClaim, "same corrupt claim")
  fs.utimesSync(activeClaim, staleTime, staleTime)
  assert.equal(
  ...
  assert.equal(
    fs
      .readdirSync(directory)
      .filter((name) => name.startsWith("retired-claim-corrupt-")).length,
    2,
  )
})
  ...
  fsPromises.lstat = async (target, ...args) => {
    ...
    const isCorruptClaimInstance =
      ...
      (path.basename(resolved) === "active" &&
        path
          .basename(path.dirname(resolved))
          .startsWith("retired-claim-corrupt-"))
    if (!(isCorruptClaimInstance && metadata.isFile())) {
      return metadata
    }
...
test("a forged retirement symlink cannot move an active claim outside its cache partition", async (t) => {
  ...
  fs.symlinkSync(
    outside,
    path.join(stateDirectory(testFixture), "retired-claim-1"),
  )
  const refreshOptions = options(testFixture, fetchImpl, {
    now: () => baseTime + 73 * 60 * 60 * 1000,
  })
  assert.equal(
    (await checkForUpdate(refreshOptions)).status,
    "update_available",
  )
...
test("a corrupt future-dated operation cannot suppress checks indefinitely", async (t) => {
  const testFixture = fixture()
  t.after(() => fs.rmSync(testFixture.root, { recursive: true, force: true }))
  const futureTime = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)
  const pending = writePendingOperation(testFixture, {
    owner: "corrupt-owner",
...
test("a fenced stale owner cannot overwrite a newer committed candidate after resuming", async (t) => {
  ...
  const newer = await checkForUpdate(
    options(testFixture, async () =>
      response(remoteReleaseForVersion("3.0.0", "c".repeat(64))),
    ),
  )
  ...
  assert.deepEqual(await oldCheck, {
    status: "silent",
    reason: "check-in-progress",
  })
  const persisted = JSON.parse(fs.readFileSync(statePath(testFixture), "utf8"))
...
test("different installed versions share no acknowledgement or cache-reset state", async (t) => {
  ...
  t.after(() => fs.rmSync(testFixture.root, { recursive: true, force: true }))
  const olderReleasePath = path.join(
    testFixture.root,
    "older-skill-release.json",
  )
  ...
  const olderInstallOptions = options(testFixture, fetchImpl, {
    releasePath: olderReleasePath,
  })
  ...
  assert.equal(
    requests,
    2,
    "each installed version should retain its own fresh cache",
  )
...
test("an identity mismatch is rejected silently and never cached as a candidate", async (t) => {
  ...
  const result = await checkForUpdate(
    options(testFixture, async () =>
      response(remoteRelease({ skillId: "different-skill" })),
    ),
  )
...
test("release notes must byte-match the exact trusted GitHub URL", async () => {
  ...
  for (const releaseNotes of [
    ...
    try {
      const result = await checkForUpdate(
        options(testFixture, async () =>
          response(remoteRelease({ releaseNotes })),
        ),
      )
      assert.deepEqual(
        result,
        { status: "silent", reason: "invalid-manifest" },
        releaseNotes,
      )
    } finally {
      fs.rmSync(testFixture.root, { recursive: true, force: true })
...
test("malformed release notes are classified as an invalid manifest", async (t) => {
  ...
  const result = await checkForUpdate(
    options(testFixture, async () =>
      response(remoteRelease({ releaseNotes: "not-a-url" })),
    ),
  )
...
test("publishedAt must use canonical UTC seconds and a real calendar date", async () => {
  for (const publishedAt of [
    ...
    try {
      const result = await checkForUpdate(
        options(testFixture, async () =>
          response(remoteRelease({ publishedAt })),
        ),
      )
      assert.deepEqual(
        result,
...
test("stable candidates cannot use prerelease or build metadata", async () => {
  for (const version of ["2.16.0-dev.1", "2.16.0+build.1"]) {
    ...
    try {
      const result = await checkForUpdate(
        options(testFixture, async () =>
          response(remoteReleaseForVersion(version)),
        ),
      )
      assert.deepEqual(
        result,
...
test("a bounded timeout fails silently without retrying", async (t) => {
  ...
  const result = await checkForUpdate(
    options(testFixture, fetchImpl, { timeoutMs: 10 }),
  )
...
test("chunked responses above 32 KiB are rejected instead of fully trusted", async (t) => {
  ...
  const result = await checkForUpdate(
    options(testFixture, async () => response(oversized)),
  )
...
test("corrupt cache is rebuilt without exposing an error to the user", async (t) => {
  ...
  const result = await checkForUpdate(
    options(testFixture, async () =>
      response(remoteRelease({ version: "2.15.0" })),
    ),
  )
...
test("CLI acknowledgement emits the documented one-line success schema", async (t) => {
  ...
  for (const directory of [home, xdg, localData]) {
    fs.mkdirSync(directory, { recursive: true })
  }
  ...
  const acknowledgement = spawnSync(
    process.execPath,
    [checkerPath, "--ack", offered.eventKey],
    {
      cwd: skillRoot,
      encoding: "utf8",
      env: {
...
test("CLI entry detection survives a realpath or symlink alias", (t) => {
  const aliasRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-update-cli-alias-"),
  )
...
test("notifier source has no process execution or remote-origin override surface", () => {
  ...
  assert.doesNotMatch(
    combinedSource,
    /(?:^|[^\w.])(?:spawn|exec|execFile|fork)(?:Sync)?\s*\(/m,
  )
  assert.doesNotMatch(contractSource, /\bfetch\s*\(/)
  const environmentReads = [
    ...combinedSource.matchAll(/process\.env\.([A-Z0-9_]+)/g),
  ]
    .map((match) => match[1])
    .sort()
