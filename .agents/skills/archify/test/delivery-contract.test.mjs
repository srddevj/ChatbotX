...
const delivery = readFileSync(
  path.join(here, "..", "references", "delivery-contract.md"),
  "utf8",
)
...
test("skill requires a bounded and truthful perceptual delivery receipt", () => {
  ...
  assert.match(
    delivery,
    /never report `visual_review: passed` without inspecting/i,
  )
})
...
test("handoff browser evidence mirrors only the automated visual-check outcome", () => {
  ...
  assert.match(
    delivery,
    /`passed`[\s\S]*exit 0[\s\S]*receipt `status: "pass"`/i,
  )
  assert.match(
    delivery,
    /`failed`[\s\S]*exit 1[\s\S]*receipt `status: "fail"`/i,
  )
  assert.match(
    delivery,
    /`skipped`[\s\S]*exit 2[\s\S]*receipt `status: "skipped"`/i,
  )
  assert.match(
    delivery,
    /runtime or capture failures[\s\S]*must not be normalized to `skipped`/i,
  )
  assert.match(
    delivery,
    /remains `skipped` even when[\s\S]*`visual_review: passed`/i,
  )
  assert.match(
    delivery,
    /manual browser record[\s\S]*never changes `browser_evidence`/i,
  )
})
...
test("skill uses atomic verified delivery for the final artifact", () => {
  ...
  assert.match(
    delivery,
    /only replaces the target after.*artifact checks pass/i,
  )
  assert.match(
    delivery,
    /never claim that the deterministic receipt includes visual review/i,
  )
})
