...
test("Semantic Flow clones only exact matched authored geometry and preserves direction", () => {
  ...
  assert.match(
    html,
    /fromKind === selectedKinds\[0\] && toKind !== selectedKinds\[0\] \? 'out'/,
  )
  assert.match(
    html,
    /toKind === selectedKinds\[0\] && fromKind !== selectedKinds\[0\] \? 'in'/,
  )
  assert.match(
    html,
...
test("Semantic Flow has preset identities and motion-safe density boundaries", () => {
  ...
  assert.match(
    html,
    /\.semantic-lens-flow\[data-direction="in"\],[\s\S]+var\(--database-stroke\)/,
  )
  assert.match(
    html,
...
test("Semantic Flow cleanup and exports remain canonical", () => {
  ...
  assert.match(
    html,
    /svg\.querySelectorAll\('\[data-semantic-lens-overlay\]'\)/,
  )
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-semantic-lens-overlay\]'\)/,
  )
