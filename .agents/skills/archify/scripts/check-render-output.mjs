  ...
  for (const match of fragment.matchAll(
    ...
    const containedOwner = arrows.find(
      (arrow) =>
        ...
        (!(groupAttrs["data-edge-id"] && arrow.id) ||
...
function collectRelationshipCrossings(arrows) {
  ...
  for (let leftIndex = 0; leftIndex < relationships.length; leftIndex += 1) {
    ...
    for (
      ...
      if (point) {
        crossings.push({ left, right, point })
      }
    }
  }
  return crossings
...
function textBox(attrs, text) {
  ...
  if (
    !(Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(fontSize))
  ) {
    return null
  }
...
function collectDesktopReadability(svgAttrs, fragment) {
  ...
  for (const match of fragment.matchAll(
    ...
    if (projected >= MIN_PROJECTED_NODE_TEXT_PX) {
      continue
    }
    const candidate = {
      viewBoxWidth,
...
function estimatedTextWidth(text, fontSize) {
  ...
  for (const char of text) {
    units += char.charCodeAt(0) > 255 ? 1.8 : 0.62
  }
    ...
    switch (command.toUpperCase()) {
      ...
      case "Q": {
        while (i + 3 < tokens.length && !isCommand(tokens[i])) {
          ...
          if (![controlX, controlY, endX, endY].every(Number.isFinite)) {
            break
          }
          const control = absolute
            ? [controlX, controlY]
