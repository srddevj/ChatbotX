...
function compactMessage(value) {
  ...
  if (errorLine > 0) {
    text = lines.slice(errorLine).join("\n")
  }
  ...
  if (stackLine > 0) {
    text = relevant.slice(0, stackLine).join("\n")
  }
...
export async function startPreview(options) {
  ...
  function startBoundedChildDrain() {
    if (!child || stopGraceTimer || stopKillTimer) {
      return
    }
  ...
  function beginBuild(digest, epoch) {
    ...
    child.on("close", (code) => {
      ...
      if (stopping) {
        ...
        pendingBuild = false
        if (supersededBy && sourceEpoch === generationEpoch) {
          sourceEpoch += 1
        }
  ...
  function observeSource({ immediate = false } = {}) {
    const digest = sourceDigest(inputPath)
    if (!immediate && digest.hash === queuedHash) {
      return
    }
    sourceEpoch += 1
