...
function checkHtml(
  ...
  try {
    const stdout = execFileSync("node", [checker, htmlPath], {
      encoding: "utf8",
    })
...
test("render output check: showcase rejects a proper X with semantic identities", () => {
  ...
  const check = result.checks.find(
    (item) => item.name === "relationship_crossings",
  )
  assert.equal(check.ok, false)
...
test("render output check: relationship labels cannot hide another shared-source route", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    const check = result.checks.find(
      (item) => item.name === "label_route_clearance",
    )
    if (profile === "standard") {
      assert.equal(code, 0)
...
test("render output check: label-route thresholds include exact 2px and 4px boundaries", () => {
  ...
  assert.equal(
    standardAtTwo.result.composition.metrics.labelRouteClearanceIssues,
    0,
  )
  assert.equal(
    standardAtTwo.result.composition.metrics.minLabelRouteClearance,
    2,
  )

  const standardBelowTwo = checkHtml(
    "label-standard-one-nine",
    body(63.9),
    "standard",
  )
  assert.equal(standardBelowTwo.code, 0)
  assert.equal(
    standardBelowTwo.result.composition.metrics.labelRouteClearanceIssues,
    1,
  )
  ...
  assert.equal(
    showcaseAtTwo.result.composition.metrics.labelRouteClearanceIssues,
    1,
  )
  assert.equal(
  ...
  assert.equal(
    showcaseAtFour.result.composition.metrics.labelRouteClearanceIssues,
    0,
  )
  assert.equal(
    showcaseAtFour.result.composition.metrics.minLabelRouteClearance,
    4,
  )

  const showcaseBelowFour = checkHtml(
    "label-showcase-three-nine",
    body(65.9),
    "showcase",
  )
  assert.notEqual(showcaseBelowFour.code, 0)
  assert.equal(
    showcaseBelowFour.result.composition.metrics.labelRouteClearanceIssues,
    1,
  )
...
test("render output check: unrelated shared corridors warn in standard and fail showcase", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    assert.equal(
      result.composition.issues[0].code,
    ...
    const check = result.checks.find(
      (item) => item.name === "relationship_corridors",
    )
    if (profile === "standard") {
      assert.equal(code, 0)
      assert.equal(check.ok, true)
      ...
      assert.match(
        check.details[0],
        /\[composition\/ambiguous-corridor\] showcase/,
      )
      assert.match(
        check.details[0],
        /relationship id "first".*relationship id "second"/,
      )
      assert.deepEqual(result.composition.summary, { errors: 1, warnings: 0 })
...
test("render output check: container border runs fail both profiles with frame identity", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    assert.notEqual(code, 0)
    const check = result.checks.find(
      (item) => item.name === "container_border_runs",
    )
    assert.equal(check.ok, false)
    assert.match(
      check.details[0],
...
test("render output check: endpoint stubs from 8px pass while cramped interior turns are profile-aware", () => {
  ...
  assert.deepEqual(standard.result.composition.summary, {
    errors: 0,
    warnings: 1,
  })
  assert.equal(
    standard.result.composition.issues[0].code,
    "composition/short-interior-segment",
  )
  assert.equal(
    standard.result.checks.find((item) => item.name === "route_rhythm").ok,
    true,
  )
  ...
  assert.deepEqual(showcase.result.composition.summary, {
    errors: 1,
    warnings: 0,
  })
  const rhythm = showcase.result.checks.find(
    (item) => item.name === "route_rhythm",
  )
