...
test("all five renderers expose one reader-controlled visual style picker", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.match(
      html,
      /id="preset-menu" role="menu" aria-label="Visual style"/,
      mode,
    )
...
test("style try-on is session-only and unavailable to passive embeds", () => {
  ...
  assert.doesNotMatch(
    runtime,
    /localStorage|sessionStorage|history\.|location\.|URLSearchParams/,
  )
...
test("same topology keeps identical canonical SVG geometry across all four styles", () => {
  const normalize = (svg) =>
    svg.replace(
      / data-preset="(?:classic|signal-flow|blueprint|editorial)"/,
      "",
    )
  const variants = ["classic", "signal-flow", "blueprint", "editorial"].map(
    (preset) => normalize(svgBlock(render("architecture", preset))),
  )
