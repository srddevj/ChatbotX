...
test("all typed renderers ship the same geometry-neutral semantic camera", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      svg(html),
      /data-camera-mode|is-camera-moving|AUTO /,
      mode,
    )
  }
})
...
test("semantic camera follows reader intent but yields to manual navigation", () => {
  ...
  assert.match(
    html,
    /reveal\(\[id\], \{ includeNeighbors: true, reason: 'focus' \}\)/,
  )
  assert.match(
    html,
    /reveal\(\[id\], \{ includeNeighbors: true, reason: 'relationship' \}\)/,
  )
  assert.match(
    html,
    /reveal\(\[id\], \{ includeNeighbors: true, reason: 'finder' \}\)/,
  )
  assert.match(html, /function interruptCamera\(reason\)/)
  assert.match(html, /Archify\.guidedViews\.pause\(\)/)
  assert.match(
    html,
    /container\.addEventListener\('pointerdown',[\s\S]+interruptCamera\(\)/,
  )
...
test("semantic camera remains outside canonical SVG export state", () => {
  ...
  assert.doesNotMatch(
    svg(html),
    /style="[^"]*transform|data-view-scale|data-camera-mode/,
  )
})
