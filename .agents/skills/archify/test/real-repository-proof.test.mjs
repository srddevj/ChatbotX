...
const artifactPath = path.join(
  ...
  "docs",
  "cases",
  "mco-runtime.architecture.html",
)
const shareCardPath = path.join(
  repoRoot,
  "docs",
  "assets",
...
function evidencePayload(html) {
  ...
  assert.ok(
    match,
    "checked-in MCO proof is missing verified repository evidence",
  )
...
test("MCO showcase preserves checked-in connection-label geometry", () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-mco-showcase-layout-"),
  )
  try {
...
test("checked-in MCO artifacts are byte-reproducible from the pinned repository revision", {
  ...
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-mco-byte-reproduction-"),
  )
  try {
...
test("MCO public proof is source-backed, valid, and linked from every README", () => {
  ...
  assert.match(
    source.components.find((component) => component.id === "router")
      ?.sublabel || "",
    /\bdoctor\b/,
  )
  assert.match(
    source.components.find((component) => component.id === "adapters")
      ?.sublabel || "",
    /\bdetect\b/,
  )
  assert.match(
  ...
  assert.ok(
    cardCopy.includes(`main @ ${source.meta.repository.revision.slice(0, 7)}`),
  )
  assert.ok(
    cardCopy.includes(
      new URL(source.meta.repository.url).host +
        new URL(source.meta.repository.url).pathname,
    ),
  )
  ...
  assert.equal(
    evidence.repository.shortRevision,
    source.meta.repository.revision.slice(0, 7),
  )
  assert.equal(evidence.referenceCount, references)
  assert.match(checkedInHtml, /Archify\.sourceEvidence\.installBeacons\(\)/)
  execFileSync(process.execPath, [cli, "check", artifactPath], {
    encoding: "utf8",
  })

  const noRootTmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-mco-proof-no-root-"),
  )
  try {
  ...
  const repositorySlug = new URL(source.meta.repository.url).pathname.replace(
    /^\/|\/$/g,
    "",
  )
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      readme.includes(`\`${shortRevision}\``),
      `${filename}: repository revision drifted`,
    )
  }
})
