...
for (const mark of BRAND_MARKS) {
  for (const value of [mark.id, mark.title, ...mark.aliases]) {
    for (const form of lookupForms(value)) {
      if (!MARK_BY_LOOKUP.has(form)) {
        MARK_BY_LOOKUP.set(form, mark)
      }
    }
  }
  for (const domain of mark.domains) {
    MARK_BY_DOMAIN.set(domain, mark)
  }
}
...
export function findBrandMark(value) {
  ...
  if (url) {
    return domainMark(url.hostname)
  }
  for (const form of lookupForms(value)) {
...
function ipv6Private(address) {
  ...
  if (mappedDotted) {
    return ipv4Private(mappedDotted[1])
  }
...
function captureTimeoutMilliseconds() {
  ...
  return Math.max(100, Math.min(30_000, Math.round(configured)))
}
...
async function checkedFetch(input, accept, deadline) {
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    if (Date.now() >= deadline) {
      throw new Error("brand capture timed out")
    }
    ...
    try {
      ...
      const score =
        (type.includes("svg") || /\.svg(?:$|[?#])/i.test(url.href)
          ? 1_000_000
          : 0) +
        (rel.includes("apple-touch-icon") ? 500_000 : 0) +
        area
...
export async function captureBrandReference(value) {
  const url = asUrl(value)
  if (!url) {
    throw new Error("brand capture requires one HTTP(S) URL")
  }
...
function remoteBrand(value, cache, deadline) {
  ...
  if (!cache.has(key)) {
    cache.set(key, captureRemoteBrand(key, deadline))
  }
