...
test("Story deduplicates SVG path and label fragments by stable authored edge key", () => {
  ...
  assert.match(
    template,
    /Object\.prototype\.hasOwnProperty\.call\(positions, key\)/,
  )
  assert.match(
    template,
...
test("only explicit play=1 embeds may show the finite carrier", () => {
  ...
  assert.match(
    template,
    /html\[data-document-hidden="true"\] \.story-carrier-overlay/,
  )
  assert.match(
    template,
...
test("Story Carrier cleanup and export remain owned by Story Trail", () => {
  assert.match(
    template,
    /svg\.querySelectorAll\('\[data-story-carrier-overlay\]'\)/,
  )
  ...
  assert.equal(
    (
      template.match(
        /storyPulseOwnerToken = Archify\.motionGovernor\.claim\('story'/g,
      ) || []
    ).length,
    1,
  )
