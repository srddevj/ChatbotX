  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      ...
      for (const observation of [desktop, darkDesktop]) {
        ...
        assert.ok(
          observation.minimumProjectedNodeTextPx >= MIN_PROJECTED_NODE_TEXT_PX,
        )
        assert.equal(observation.minimumProjectedNodeTextDetail, "boundary")
        assert.equal(
          observation.minimumProjectedNodeText,
          "AWS eu-west-1 / disaster recovery",
        )
        assert.equal(observation.readabilityOk, true)
        assert.equal(
          observation.scrollHeight,
          DESKTOP_READABILITY_VIEWPORT.height,
        )
      }
    }
  } finally {
