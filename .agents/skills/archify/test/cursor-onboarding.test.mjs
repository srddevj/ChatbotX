...
test("Cursor onboarding stays explicit, bilingual, and backed by the same Skill", () => {
  ...
  const englishMirror = fs.readFileSync(
  ...
  const start = fs.readFileSync(
    path.join(repoRoot, "docs", "start.html"),
    "utf8",
  )
  ...
  assert.equal(
    english,
    englishMirror,
    "English README mirrors must stay synchronized",
  )
  ...
  for (const surface of [english, chinese, landing]) {
    assert.ok(surface.includes(cursorCommand))
  }
  ...
  try {
    ...
    for (const [type, fixture] of Object.entries(fixtures)) {
      ...
      assert.equal(
        receipt.ok,
        true,
        `${type}: installed package validation failed`,
      )
