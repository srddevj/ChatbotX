...
test("all typed renderers ship the same geometry-neutral node finder", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="node-finder" hidden role="dialog" aria-modal="false"/,
      mode,
    )
    ...
    assert.doesNotMatch(
      svg(html),
      /node-finder|Archify\.finder|Find a node/,
      mode,
    )
  }
})
...
test("finder presents one focused search control and a structured result list", () => {
  ...
  assert.match(
    html,
    /id="node-finder-input"[^>]+aria-label="Search diagram nodes"/,
  )
  assert.match(html, /\.node-finder-search:focus-within\s*\{/)
  assert.match(
    html,
    /\.node-finder-input:focus-visible\s*\{\s*outline:\s*none;/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /\.node-finder-results\s*\{[\s\S]*?flex:\s*1 1 auto;[\s\S]*?min-height:\s*0;/,
  )
  ...
  assert.match(
    html,
    /\[viewerKindLabel\(item\.type\), item\.id, item\.sublabel, item\.tag\]/,
  )
  assert.doesNotMatch(
    html,
    /\[item\.type, item\.context, item\.sublabel, item\.tag, item\.id\]/,
  )
...
test("finder becomes a contextual Route Probe endpoint picker without changing semantic focus", () => {
  ...
  assert.match(
    html,
    /context\.kind === 'route-source' \|\| context\.kind === 'route-target'/,
  )
...
test("finder is keyboard accessible, mobile-pinned, and subordinate to embed mode", () => {
  ...
  assert.match(
    html,
    /Archify\.exportMenu\.isOpen\(\)\) Archify\.exportMenu\.close\(false\)/,
  )
  ...
  assert.match(
    html,
    /!event\.target\.closest\('\[data-node-finder-trigger\]'\)/,
  )
})
