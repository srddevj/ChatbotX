...
function example(type) {
  return JSON.parse(
    fs.readFileSync(path.join(skillRoot, "examples", EXAMPLES[type]), "utf8"),
  )
}
...
function authoredExample(type, locale) {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (typeof child === "string" && AUTHORED_TEXT_KEYS.has(key)) {
        ...
        value[key] = child.map((item) =>
          typeof item === "string" ? nextAuthoredText() : item,
        )
      } else {
  ...
  if (!document.meta.subtitle) {
    document.meta.subtitle = nextAuthoredText()
  }
...
test("zh-CN localizes renderer-owned output across all five modes without translating authored content", () => {
  ...
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.match(
      result.html,
      /<desc id="archify-diagram-description">\u7531 Archify \u751f\u6210\u7684/,
    )
...
test("explicit en and zh-CN preserve complete authored field inventories across all five modes", () => {
  for (const type of Object.keys(EXAMPLES)) {
    ...
    assert.ok(
      english.authored.length >= 10,
      `${type}: authored inventory is unexpectedly small`,
    )
    if (type === "dataflow") {
      assert.ok(
    ...
    for (const candidate of [english, chinese]) {
      ...
      assert.equal(
        result.status,
        0,
        `${type}/${locale}: ${result.stderr || result.stdout}`,
      )
      assert.match(
        result.html,
      ...
      if (locale === "zh-CN") {
        assert.ok(
          result.html.includes(
            `<title>${candidate.document.meta.title}</title>`,
          ),
          type,
        )
        assert.match(result.html, />导出图表</)
        ...
        assert.ok(
          result.html.includes(
            `<title>${candidate.document.meta.title} Diagram</title>`,
          ),
          type,
        )
    ...
    for (const type of Object.keys(EXAMPLES)) {
      ...
      assert.ok(
        payload.diagnostics.some(
          (entry) => entry.subject?.path === "/meta/locale",
        ),
        `${type}: ${locale}`,
      )
    }
  ...
  try {
    for (const type of Object.keys(EXAMPLES)) {
      ...
      assert.equal(
        result.status,
        0,
        `${type}: ${result.stderr || result.stdout}`,
      )
      ...
      const visual = spawnSync(
        process.execPath,
        [cli, "visual-check", result.output, "--json"],
        {
          cwd: skillRoot,
          encoding: "utf8",
      ...
      assert.ok(
        [0, 1].includes(visual.status),
        `${type}: ${visual.stderr || visual.stdout}`,
      )
      ...
      assert.equal(
        receipt.containment.viewports.every(
          (viewport) => viewport.overflowX === false,
        ),
        true,
...
test("every Viewer message reference resolves through the shared catalog", () => {
  ...
  const references = new Set([
    ...[...template.matchAll(/\{\{i18n:([a-zA-Z0-9_.-]+)\}\}/g)].map(
      (match) => match[1],
    ),
    ...[...template.matchAll(/['"](viewer\.[a-zA-Z0-9_.-]+)['"]/g)].map(
      (match) => match[1],
    ),
  ])
  const unresolved = [...references].filter(
    (key) =>
      !(
        key.endsWith(".") ||
        keys.has(key) ||
        (keys.has(`${key}.one`) && keys.has(`${key}.other`))
      ),
  )
...
test("runtime labels stay localized after composition", () => {
  ...
  assert.equal(
    translateMessage("zh-CN", "viewer.passport.relationship.connectsFrom"),
    "连接自",
  )
  ...
  assert.equal(
    translateMessage("zh-CN", "viewer.finder.result.routeTarget", {
      label: "终点",
      links: zhHops,
    }),
    "选择终点作为路径终点，2 跳",
  ...
  assert.equal(
    translateMessage("en", "viewer.route.overview.status", {
      nodes: enNode,
      hops: enHop,
    }),
...
test("Share Card and export failures use catalog messages instead of fixed English", () => {
  ...
  assert.equal(
    translateMessage("zh-CN", "viewer.export.error.toBlobNull", {
      label: "分享卡片",
    }),
