...
function createReadableLayout(workflow, layoutFeedback = {}) {
  ...
  for (let leftIndex = 0; leftIndex < nodes.length; leftIndex += 1) {
    for (
      ...
      if (!verticalIntervalsOverlap(leftNode, rightNode, 8)) {
        continue
      }
  ...
  for (const edge of asArray(workflow.edges)) {
    ...
    if (preferLabelChannel) {
      channelLabelEdgeKeys.add(stableValueKey(edge))
    }
    constraints.push({
      from: earlier.col,
      to: later.col,
  ...
  for (let iteration = 0; iteration < maxLayoutIterations; iteration += 1) {
    ...
    if (iteration > 0 || !feedbackConstraints.length) {
      break
    }
  ...
  for (const nodeId of unpinnedTopEndpointIds) {
    ...
    if (
      !(node && Number.isInteger(node.col)) ||
      node.col < 0 ||
  ...
  let measuredContentLeftShift = asArray(workflow.edges).reduce(
    (maximum, edge) => {
      if (!channelLabelEdgeKeys.has(stableValueKey(edge))) {
        return maximum
      }
      const fromNode = nodesById.get(edge.from)
      const toNode = nodesById.get(edge.to)
...
function semanticContractDiagnostics(workflow) {
  ...
  for (const [index, relation] of asArray(checks.requiredEdges).entries()) {
    if (authoredEdges.has(`${relation.from}\u0000${relation.to}`)) {
      continue
    }
    diagnostics.push(
      diagnostic(
  ...
  for (const [index, relation] of asArray(checks.requiredPaths).entries()) {
    if (reachable(relation.from, relation.to)) {
      continue
    }
    diagnostics.push(
      diagnostic(
...
function compileWorkflowInternal({
  ...
  function verifiedReducedWidths(from, to, requiredClearance) {
    ...
    if (signedClearance + 0.0001 < requiredClearance) {
      return null
    }
  ...
  function enforceLegacyColumnCapacity() {
    ...
    for (const edge of workflow.edges) {
      ...
      if (!verticalIntervalsOverlap(from, to, 8)) {
        continue
      }
      ...
      if (actualSignedClearance >= requiredDirectClearance) {
        continue
      }
  ...
  function verifiedPinReferenceAlternatives(candidateRefs, reason) {
    ...
    const refs = candidateRefs.filter(({ edge, edgeIndex, field }) => {
      if (edgeIndex < 0 || edge?.[field] === undefined) {
        return false
      }
  ...
  function classifyFailedAutomaticCandidatePins(edge, rawCandidates) {
    ...
    for (const { points } of rawCandidates) {
      ...
      if (reverseHit) {
        throwReadableLabelRoutePinConflict(reverseHit, points)
      }
    }
  }
  ...
  function validateReadablePairwisePinConflicts() {
    ...
    if (labelRouteHit) {
      throwReadableLabelRoutePinConflict(labelRouteHit)
    }
  ...
  function segmentOrientation(start, end) {
    ...
    if (Math.abs(start[1] - end[1]) <= 0.0001) {
      return "horizontal"
    }
    return "diagonal"
  }
  ...
  function endpointSideIsHonored(points, side, endpoint) {
    if (!side || side === "auto" || points.length < 2) {
      return true
    }
    ...
    if (source) {
      if (side === "right") {
        return dx > 0 && Math.abs(dy) <= 0.0001
      }
      if (side === "left") {
        return dx < 0 && Math.abs(dy) <= 0.0001
      }
      if (side === "bottom") {
        return dy > 0 && Math.abs(dx) <= 0.0001
      }
      if (side === "top") {
        return dy < 0 && Math.abs(dx) <= 0.0001
      }
  ...
  function validateWorkflow() {
    ...
    for (const node of nodes.values()) {
      ...
      if (brandRailProblem) {
        problems.push(brandRailProblem)
      }
    ...
    for (let i = 0; i < phaseRanges.length; i += 1) {
      for (let j = i + 1; j < phaseRanges.length; j += 1) {
        ...
        if (later.fromCol > earlier.toCol) {
          break
        }
        problems.push(
  ...
  function firstRouteNodeCollision(edge, points) {
    ...
    for (const node of nodes.values()) {
      ...
      for (
        ...
        if (endpointRole === "source-endpoint" && segmentIndex === 0) {
          continue
        }
        if (
          endpointRole === "target-endpoint" &&
          segmentIndex === lastSegment
        ) {
          continue
        }
  ...
  function legacyAutomaticOneBendSides(edge, from, to) {
    ...
    if (
      !(automaticRoute && automaticFrom && automaticTo) ||
      from.lane === to.lane
    ) {
      return null
    }
  ...
  function readableAutomaticSides(edge, from, to) {
    ...
    if (!automaticRoute || (authoredFrom && authoredTo)) {
      return null
    }
    if (readableSideCache.has(edge)) {
      return readableSideCache.get(edge)
    }

    const preferred = []
    const legacyPreferred = legacyAutomaticOneBendSides(edge, from, to)
    if (legacyPreferred) {
      preferred.push(legacyPreferred)
    }
    preferred.push({
    ...
    for (const candidate of preferred) {
      if (authoredFrom && candidate.fromSide !== authoredFrom) {
        continue
      }
      if (authoredTo && candidate.toSide !== authoredTo) {
        continue
      }
  ...
  function routeClearsEndpointNodes(points, from, to) {
    ...
    for (let index = 0; index <= lastSegment; index += 1) {
      ...
      if (index > 0 && segmentIntersectsRect(segment, from)) {
        return false
      }
    ...
    for (const [otherEdge, routed] of pathCache) {
      ...
      if (candidateLabel) {
        for (let index = 0; index < routed.points.length - 1; index += 1) {
          ...
          if (clearance != null) {
            deficit += Math.max(0, threshold - clearance)
          }
        }
      }
      if (otherLabel) {
  ...
  function routeClearsLegend(edge, points) {
    ...
    for (const rect of legendRects) {
      ...
      if (label && rectsOverlap(label, rect)) {
        return false
      }
    }
    return true
  ...
  function routeClearsSceneLabelObstacles(edge, points) {
    ...
    for (const obstacle of workflowSceneLabelObstacles()) {
      ...
      if (label && rectsOverlap(label, obstacle)) {
        return false
      }
    }
    return true
  ...
  function readableAutomaticVia(edge, from, to, start, end, fromSide, toSide) {
    ...
    if (candidates.length) {
      return candidates[0].points.slice(1, -1)
    }
    ...
    if (outsideRightCandidate) {
      ...
      for (const node of nodes.values()) {
        if (!(labelRect && rectsOverlap(labelRect, node, -2))) {
          continue
        }
  ...
  function routeVia(
    ...
    switch (edge.route || "auto") {
      ...
      case "bottom-channel": {
        const y =
          edge.channelY ?? Math.max(from.y + from.height, to.y + to.height) + 32
        return [
          [start[0], y],
          [end[0], y],
        ]
      ...
      default: {
        ...
        if (from.lane === to.lane) {
          return sameLaneAutoVia(start, end)
        }
  ...
  function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
    ...
    for (const candidateSides of sidePairs) {
      if (authoredFrom && candidateSides.fromSide !== authoredFrom) {
        continue
      }
      if (authoredTo && candidateSides.toSide !== authoredTo) {
        continue
      }
  ...
  function readableControlledRoute(edge, from, to) {
    ...
    const sidePairs = preferredPairs.filter(({ fromSide, toSide }) => {
      if (authoredFrom && fromSide !== authoredFrom) {
        return false
      }
      if (authoredTo && toSide !== authoredTo) {
        return false
      }
  ...
  function pathFor(edge) {
    if (pathCache.has(edge)) {
      return pathCache.get(edge)
    }
    const from = nodes.get(edge.from)
    const to = nodes.get(edge.to)
  ...
  function measuredContentBounds() {
    ...
    for (const [index, edge] of workflow.edges.entries()) {
      ...
      if (label) {
        includeRect(label, `edge ${edge.id || index} label mask`)
      }
    }
    for (const phase of asArray(workflow.phases)) {
      if (
  ...
  for (
    ...
    try {
      ...
      if (
        ...
        const current =
          layoutFeedback.rankGapMinimums?.[key] ?? Number.NEGATIVE_INFINITY
        ...
        Number.isFinite(request.minimum) &&
        request.minimum >
          (layoutFeedback.laneGapMin ?? Number.NEGATIVE_INFINITY) + 0.0001
      ) {
        nextFeedback = {
          ...layoutFeedback,
