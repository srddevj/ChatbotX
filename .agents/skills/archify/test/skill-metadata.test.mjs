...
test("language behavior stays within the bounded locale contract", () => {
  ...
  assert.match(
    skill,
    /product names.*code identifiers.*protocols.*API paths.*environment names/,
  )
  assert.match(
    authoringContract,
    /`meta\.locale` controls only renderer-owned reader surfaces/,
  )
  ...
  assert.match(
    authoringContract,
    /Renderer-owned default legend labels follow `meta\.locale`/,
  )
  assert.match(
    authoringContract,
    /The fallback\s+applies only to renderer-owned surfaces/,
  )
})
...
test("skill keeps the title hierarchy compact by default", () => {
  ...
  assert.match(
    skill,
    /Never invent a subtitle that restates the title, nodes, or cards/,
  )
  assert.match(
    authoringContract,
    /omitted or blank subtitle must not leave an empty visual row/,
  )
})
