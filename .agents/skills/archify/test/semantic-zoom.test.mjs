...
test("all typed renderers emit explicit context and fine reading-depth semantics", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /class="diagram-container" data-detail-level="read"/,
      mode,
    )
  }
})
...
test("semantic zoom is motion-safe and full-fidelity in print and export", () => {
  ...
  assert.match(
    html,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]+svg \[data-detail\]/,
  )
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-detail\], \[data-detail-anchor\]'\)/,
  )
