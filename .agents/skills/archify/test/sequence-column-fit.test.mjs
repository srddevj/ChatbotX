...
function participantBoxes(html) {
  return [
    ...html.matchAll(/<rect x="([\d.]+)" y="72" width="([\d.]+)" height="54"/g),
  ]
    .map(([, x, width]) => ({ x: Number(x), width: Number(width) }))
    .filter(
      (box, index, all) =>
        all.findIndex((other) => other.x === box.x) === index,
    )
...
test("spread column fit uses the viewBox width and stays inside it", () => {
  ...
  assert.ok(
    boxes[0].width > 86,
    "participant boxes widen with the available room",
  )
...
test("a label the fixed box rejects fits the spread box on the same viewBox", () => {
  ...
  assert.ok(
    estimatedLabelW > 86 + 6,
    "the fixture label must actually exceed the fixed box",
  )
  ...
  assert.notEqual(
    fixed.code,
    0,
    "the fixed box still rejects a label it cannot hold",
  )
  assert.ok(
  ...
  assert.ok(
    spread.html.includes(`>${wideLabel}</text>`),
    "the label renders unshortened",
  )
})

test("the sublabel diagnostic reports the width in force, not the historical constant", () => {
  const unrescuable =
    "Payment authorization gateway detail text that stays far too long to shrink"
  ...
  assert.notEqual(
    code,
    0,
    "a sublabel past the legible minimum is still rejected",
  )
  assert.match(
    stderr,
    /participant boxes are 190px for this viewBox width and 5 participants/,
  )
  assert.doesNotMatch(
    stderr,
    /boxes are a fixed/,
    "spread must not quote the fixed layout",
  )
})
...
test("the fast authoring path explains when to opt into spread", () => {
  const schema = JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "schemas/sequence.schema.json"),
      "utf8",
    ),
  )
  ...
  assert.match(
    rendererReadme,
    /try `meta\.column_fit: "spread"` before shortening/,
  )
})
