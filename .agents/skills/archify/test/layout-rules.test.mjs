...
function render(mode, doc) {
  const input = path.join(
    tmp,
    `${mode}-${Math.abs(hash(JSON.stringify(doc)))}.json`,
  )
  const outPath = path.join(
    tmp,
    `${mode}-${Math.abs(hash(JSON.stringify(doc)))}.html`,
  )
...
function validateCli(mode, doc, quality = "showcase") {
  const input = path.join(
    tmp,
    `${mode}-cli-${Math.abs(hash(JSON.stringify(doc)))}.json`,
  )
  fs.writeFileSync(input, JSON.stringify(doc))
  try {
...
function workflowNodeBorderOverlap(points, rect) {
  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    if (
      start[0] === end[0] &&
      (start[0] === rect.x || start[0] === rect.x + rect.width)
    ) {
      const length = axisOverlapLength(
        start[1],
        end[1],
        rect.y,
        rect.y + rect.height,
      ...
      if (length > 0) {
        hits.push({ segment: index, length, start, end })
      }
    }
    if (
      start[1] === end[1] &&
      (start[1] === rect.y || start[1] === rect.y + rect.height)
    ) {
      const length = axisOverlapLength(
        start[0],
        end[0],
...
function assertRelationshipsAvoidAllNodeBorders(
  html,
  relationships,
  diagramNodes,
) {
  for (const [edgeIndex, edge] of relationships.entries()) {
    ...
    for (const node of diagramNodes) {
      const hits = workflowNodeBorderOverlap(
        points,
        workflowNodeRect(html, node.id),
      )
      assert.deepEqual(
        hits,
        [],
...
const CASES = [
  ...
  [
    ...
    (d) => {
      d.nodes[0].sublabel =
        "This supporting sentence is much too long for one workflow node"
    },
  ...
  [
    ...
    (d) => {
      d.nodes[0].tag =
        "This tag is far too long to sit inside one workflow node box"
    },
  ...
  [
    ...
    (d) => {
      d.groups = [
        { id: "empty", label: "Empty group", lane: "ui", fromCol: 3, toCol: 4 },
      ]
    },
  ...
  [
    ...
    (d) => {
      d.nodes[0].sublabel =
        "This supporting sentence is far too long for one data-flow node box"
    },
    ["Sublabel", "legible", "increase node.width"],
  ],
  [
    ...
    (d) => {
      d.nodes[0].tag =
        "This tag is far too long to sit inside one data-flow node box"
    },
    ["Tag", "legible", "increase node.width"],
  ],
  ...
  [
    ...
    (d) => {
      d.lanes = d.lanes.map((l) =>
        l.id === "main" ? { ...l, id: "primary" } : l,
      )
      d.states = d.states.map((s) =>
        s.lane === "main" ? { ...s, lane: "primary" } : s,
      )
    },
    ['"main"', "reserved"],
  ],
  ...
  [
    ...
    (d) => {
      d.states[0].sublabel =
        "This supporting sentence is far too long for one lifecycle state box"
    },
  ...
  [
    ...
    (d) => {
      d.states[0].tag =
        "This tag is far too long to sit inside one lifecycle state box"
    },
  ...
  [
    ...
    (d) => {
      d.components[0].tag =
        "This tag is far too long to sit inside one architecture component box"
    },
...
test("architecture: ordinary boundaries may express orthogonal overlapping memberships", () => {
  ...
  assert.equal(
    rectanglesOverlap(boundaryFrameRect(html, 0), boundaryFrameRect(html, 1)),
    true,
  )
...
test("architecture: deployment ownership requires nested membership geometry to agree", () => {
  ...
  d.boundaries.find(
    (boundary) => boundary.label === "private application network",
  ).pad = 260
  const { code, stderr } = render("architecture", d)
  ...
  d.boundaries = [
    {
      kind: "region",
      label:
        "Disaster recovery boundary Disaster recovery boundary Disaster recovery boundary",
      wraps: ["node"],
      pad: 30,
    },
  ]
...
test("architecture: boundary labels and their masks paint above relationship routes", () => {
  ...
  const firstBoundaryLabel = html.indexOf(
    'data-graph-role="structural-frame-label"',
  )
  assert.ok(lastRoute >= 0, "expected at least one rendered relationship route")
  assert.ok(
    firstBoundaryLabel >= 0,
    "expected a foreground boundary label group",
  )
  assert.ok(
...
test("architecture: boundary title masks cannot obscure connection labels", () => {
  ...
  assert.match(
    stderr,
    /Boundary label "Runtime scope" overlaps connection label "Route label"/,
  )
...
for (const [mode, mutate, preferred] of SHRINK_CASES) {
  test(`${mode}: an over-long sublabel shrinks to fit instead of overflowing`, () => {
    ...
    const sub = html.match(
      /<text data-detail="context"[^>]*font-size="([\d.]+)"[^>]*>/,
    )
    ...
    assert.ok(
      fontSize >= 6,
      `expected the sublabel to stay legible, got ${fontSize}`,
    )
  })
}
...
for (const [mode, collection, tag, preferred] of TAG_SHRINK_CASES) {
  test(`${mode}: an over-long tag shrinks to fit instead of overflowing`, () => {
    ...
    assert.ok(
      fontSize >= 6,
      `expected the tag to stay legible, got ${fontSize}`,
    )
  })
}
...
test("contract: a too-wide label is never redirected into sublabel", () => {
  ...
  for (const [mode, label] of Object.entries(LABELS)) {
    ...
    assert.notEqual(
      code,
      0,
      `${mode}: expected non-zero exit; stderr:\n${stderr}`,
    )
    assert.ok(
      stderr.includes("wider than"),
      `${mode}: expected a width message:\n${stderr}`,
    )
    assert.ok(
...
test("contract: ajv path errors are annotated with the element id", () => {
  ...
  if (!/schema validation failed/.test(stderr)) {
    return
  }
...
test("workflow: automatic routing uses one bend and avoids every node border", () => {
  ...
  assert.equal(
    points.length,
    3,
    `expected one bend, received ${JSON.stringify(points)}`,
  )
  assert.equal(
    points[0][0],
    points[1][0],
    "edge must leave the source vertically",
  )
  assert.ok(
    points[1][1] < points[0][1],
    "edge must leave the source through its top side",
  )
  assert.equal(
    points[1][1],
    points[2][1],
    "edge must enter the target horizontally",
  )
  assert.ok(
    points[2][0] > points[1][0],
    "edge must enter the target through its left side",
  )
  ...
  assert.equal(
    labelX,
    points[0][0],
    "the label should use the longer vertical segment",
  )
  assert.ok(
...
test("workflow: blocked first one-bend candidate selects the clear one-bend orientation", () => {
  ...
  const { code, stderr, outPath } = render("workflow", d)
  assert.equal(code, 0, stderr)
  const points = workflowEdgePoints(
  ...
  assert.ok(
    ...
    "edge must leave the source through its right side",
  )
  assert.equal(
    points[1][0],
    points[2][0],
    "edge must enter the target vertically",
  )
  assert.ok(
    points[2][1] < points[1][1],
    "edge must enter the target through its bottom side",
  )
})
...
test("workflow: explicit drop routing remains authoritative over the one-bend preference", () => {
  const d = {
    ...
    diagram_type: "workflow",
    meta: {
      title: "Explicit route compatibility",
  ...
  assert.equal(
    points.length,
    4,
    `explicit drop route was replaced: ${JSON.stringify(points)}`,
  )
...
test("lifecycle: legacy tangent via is rendered exactly instead of silently rewritten", () => {
  ...
  assert.equal(code, 0, stderr)
  assert.deepEqual(
    workflowEdgePoints(fs.readFileSync(outPath, "utf8"), "legacy-via"),
    [
      [248, 188],
      [320, 188],
      [320, 430],
...
test("architecture: Clean Flow Gate rejects a connection through a component", () => {
  const d = {
    ...
    connections: [
      { id: "direct", from: "left", to: "right", route: "straight" },
    ],
  }
  const { code, stderr } = render("architecture", d)
  assert.notEqual(code, 0, `expected non-zero exit; stderr:\n${stderr}`)
...
test("dataflow: showcase rejects a relationship label that hides another route", () => {
  const d = JSON.parse(
    fs.readFileSync(
      path.join(skillRoot, "examples", "event-stream.dataflow.json"),
      "utf8",
    ),
  )
  ...
  assert.match(
    stderr,
    /\[composition\/label-route-clearance\] showcase dataflow/,
  )
...
test("architecture: showcase rejects a connection label that hides another route", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/label-route-clearance\] showcase architecture/,
  )
...
test("workflow: showcase rejects an edge label that hides another route", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/label-route-clearance\] showcase workflow/,
  )
...
test("lifecycle: showcase rejects a transition label that hides another route", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/label-route-clearance\] showcase lifecycle/,
  )
...
test("sequence: showcase rejects a message label that hides an adjacent route", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/label-route-clearance\] showcase sequence/,
  )
...
test("architecture: default auto route selects a safe orthogonal candidate around an unrelated component", () => {
  ...
  assert.match(
    html,
    /data-composition-points="560,318;856,318;856,160;880,160"/,
  )
})
...
test("architecture: auto route enters explicit top and bottom ports perpendicularly", () => {
  ...
  assert.match(
    html,
    /data-composition-points="350,160;350,200;150,200;150,240"/,
  )
})
...
test("architecture: auto route preserves inferred side normals when the primary dogleg is blocked", () => {
  ...
  assert.match(
    html,
    /data-composition-points="700,130;184,130;184,330;160,330"/,
  )
  assert.doesNotMatch(
    html,
    /data-composition-points="700,130;700,230;160,230;160,330"/,
  )
})
...
test("architecture: explicit orthogonal route remains authoritative when it crosses a component", () => {
  ...
  assert.match(
    stderr,
    /connections\[0\] "api" -> "queue" crosses component "cache"/,
  )
})
...
test("architecture: showcase rejects an unrelated shared route corridor", () => {
  const { code, stderr } = render(
    "architecture",
    ambiguousCorridorDocument("showcase"),
  )
  assert.notEqual(code, 0, `expected non-zero exit; stderr:\n${stderr}`)
  assert.match(
    stderr,
    /\[composition\/ambiguous-corridor\] showcase architecture/,
  )
  assert.match(
    stderr,
...
test("architecture: standard keeps an ambiguous corridor renderable for repair", () => {
  const { code, stderr } = render(
    "architecture",
    ambiguousCorridorDocument("standard"),
  )
  assert.equal(code, 0, stderr)
})
...
test("architecture: container border run is blocking in standard and showcase", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    d.connections.find(
      (connection) => connection.id === "jwt-verification",
    ).via = [
      [620, 142],
      [620, 270],
      [735, 270],
...
test("dataflow: Clean Flow Gate rejects a flow through an unrelated node", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/edge-through-node\] dataflow flows\[0\] id "direct"/,
  )
  assert.match(stderr, /crosses node "middle"/)
...
test("lifecycle: Clean Flow Gate rejects a transition through an unrelated state", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/edge-through-node\] lifecycle transitions\[0\] id "direct"/,
  )
...
test("sequence: segment titles render as foreground badges above their borders", () => {
  ...
  assert.match(html, /data-graph-role="segment-label"[^>]*data-segment-id="0"/)
  assert.match(
    html,
    new RegExp(
      `<text x="62" y="${firstSegment.from - 9}"[^>]*>${firstSegment.label}</text>`,
    ),
  )
})
...
test("sequence: segment title badge clears a nearby first message label", () => {
  ...
  assert.equal(
    overlaps,
    false,
    "segment title badge must not cover the first message label",
  )
})
