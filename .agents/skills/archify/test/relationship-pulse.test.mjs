...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-relationship-pulse-"),
)
...
test("all typed renderers inherit one exact-edge Directional Flow Pulse", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /setAttribute\('class', 'relationship-flow-pulse'\)/,
      mode,
    )
    assert.doesNotMatch(
      canonicalSvg(html),
...
test("semantic token classification is evidence-based and fail-closed", () => {
  ...
  assert.match(
    html,
    /sourceKind === 'database' \|\| targetKind === 'database'.+return 'data'/s,
  )
  assert.match(
    html,
    /targetKind === 'waiting' \|\| targetKind === 'success'.+return 'state'/s,
  )
  assert.match(html, /return 'call';/)
  assert.doesNotMatch(
    html,
    /relationshipTokenKind[\s\S]{0,1800}data-edge-label/,
  )
})
...
test("semantic tokens use five distinct inline SVG cues on one finite timing owner", () => {
  ...
  assert.match(
    html,
    /document\.createElementNS\(svgNamespace, 'animateMotion'\)/,
  )
  assert.match(
    html,
...
test("pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe", () => {
  ...
  assert.match(
    html,
    /Archify\.motionGovernor && Archify\.motionGovernor\.isPaused\(\)/,
  )
...
test("pulse has one owner and never enters print or canonical exports", () => {
  ...
  assert.match(
    html,
    /svg\.querySelectorAll\('\[data-relationship-pulse-overlay\]'\)/,
  )
  assert.match(
    html,
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-relationship-pulse-overlay\]'\)/,
  )
  assert.match(
    html,
    /\[data-relationship-pulse-overlay\],[^']*\[data-relationship-preview\]/,
  )
  assert.doesNotMatch(
    canonicalSvg(html),
    /relationship-flow-(?:pulse|token)|data-relationship-(?:pulse|token)/,
