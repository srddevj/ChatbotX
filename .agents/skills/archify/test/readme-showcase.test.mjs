...
function inspectGif(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (marker === 0x21) {
      ...
      if (label === 0xf9) {
        ...
        assert.equal(
          buffer[offset],
          0,
          "graphic-control block missing terminator",
        )
        offset += 1
      } else {
    ...
    throw new Error(
      `unexpected GIF marker 0x${marker.toString(16)} at ${offset - 1}`,
    )
  }
...
test("README motion proof is compact, looping, and backed by current gallery artifacts", () => {
  ...
  assert.ok(
    buffer.includes(Buffer.from("NETSCAPE2.0")),
    "GIF must loop continuously",
  )
  assert.ok(
...
test("all README languages keep the product hero and retain the verified animated proof", () => {
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      proofIndex > demosIndex,
      `${filename}: animated proof must live in the demo section`,
    )
...
test("README installation tables contain a complete DeepSeek Harness row", () => {
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      row,
      `${filename}: DeepSeek Harness must be an installation table row`,
    )
    assert.equal(
      (row.match(/(?<!\\)\|/g) || []).length,
      4,
...
test("README demos use checked-in captures and live deep links below the existing hero", () => {
  ...
  for (const demo of demos) {
    const buffer = fs.readFileSync(
      path.join(repoRoot, "docs", "assets", demo.asset),
    )
    assert.equal(
    ...
    assert.equal(
      ...
      1280,
      `${demo.asset}: unexpected width`,
    )
    assert.equal(
      buffer.readUInt32BE(20),
      720,
      `${demo.asset}: unexpected height`,
    )
    assert.ok(
      buffer.byteLength < 400 * 1024,
      `${demo.asset}: capture is too large`,
    )
  }
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      heroIndex >= 0 && heroIndex < demosIndex,
      `${filename}: existing hero proof moved`,
    )
    assert.ok(
    ...
    for (const demo of demos) {
      assert.match(
        readme,
        new RegExp(`docs/assets/${demo.asset.replaceAll(".", "\\.")}`),
      )
...
test("README stays scannable without deleting the visual proof set", () => {
  ...
  assert.ok(
    wordCount <= 2085,
    `README.md is too verbose again (${wordCount} words)`,
  )
  assert.ok(
...
test("all README languages end with the self-hosted star history chart", () => {
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.ok(
      readme.includes(lightChart),
      `${filename}: missing light star history chart`,
    )
    assert.ok(
      readme.includes(darkChart),
      `${filename}: missing dark star history chart`,
    )
    assert.equal(
...
test("Star History publishing advances the data branch without a force push", () => {
  const fixture = fs.mkdtempSync(
    path.join(os.tmpdir(), "archify-star-history-"),
  )
  ...
  try {
    ...
    assert.match(
      git(
        ...
        remote,
        "show",
        `${secondCommit}:assets/star-history-light.svg`,
      ),
      /light v2/,
    )
  } finally {
