...
test("all five renderers inherit one viewer-only Story Horizon", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="guided-story-caption-next" hidden aria-hidden="true"/,
      mode,
    )
    const generatedSvg = html.match(/<svg\b[\s\S]*?<\/svg>/)?.[0] || ""
    assert.doesNotMatch(
      generatedSvg,
      /data-story-next|data-story-beat-state="next"/,
      mode,
    )
  }
})
...
test("Still, accessibility, teardown, and export preserve the product boundary", () => {
  ...
  assert.ok(
    (template.match(/svg\.removeAttribute\('data-story-next'\)/g) || [])
      .length >= 3,
  )
  assert.ok(
    (template.match(/panel\.removeAttribute\('data-story-next'\)/g) || [])
      .length >= 2,
  )
  assert.match(template, /clone\.removeAttribute\('data-story-next'\)/)
  assert.match(
    template,
    /canonicalStateClean[\s\S]*!clone\.hasAttribute\('data-story-next'\)/,
  )
})
