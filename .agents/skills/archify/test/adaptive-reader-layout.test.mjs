...
test("wide desktop diagrams use one height-budgeted reader shell instead of breakpoint jumps", () => {
  ...
  assert.doesNotMatch(
    template,
    /@media \(min-width: 1680px\)[\s\S]{0,180}\.container/,
  )
  assert.doesNotMatch(
    template,
    /@media \(min-width: 1920px\)[\s\S]{0,180}\.container/,
  )
...
test("desktop readability budget matches the minimum adaptive reader at 1440 by 900", () => {
  ...
  assert.match(
    reader,
    new RegExp(`var MIN_READER_WIDTH = ${DESKTOP_READER_MIN_WIDTH}`),
  )
  assert.match(
    template,
  ...
  assert.match(
    template,
    /\.diagram-container \{[\s\S]*?border: 1px solid var\(--panel-border\)/,
  )
})
...
test("desktop readability source floor is the inverse of the projected-size gate", () => {
  ...
  assert.ok(Math.abs(sourceFloor - 8.877_419_354_838_71) < 1e-12)
  assert.ok(
    Math.abs(
      projectedNodeTextPx(sourceFloor, 1376) - MIN_PROJECTED_NODE_TEXT_PX,
    ) < 1e-12,
  )
  assert.equal(
...
test("reader remeasures real content and reduces width before allowing desktop page overflow", () => {
  ...
  assert.match(
    skill,
    /preserve the authored SVG\/viewBox, proportions, semantic geometry/,
  )
})
