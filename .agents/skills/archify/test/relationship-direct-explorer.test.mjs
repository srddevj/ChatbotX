...
const tmp = fs.mkdtempSync(
  path.join(os.tmpdir(), "archify-relationship-direct-explorer-"),
)
...
test("one roving target represents each exact stable edge key and authored direction", () => {
  ...
  assert.match(
    template,
    /filter\(function \(record\) \{ return !record\.invalid/,
  )
  ...
  assert.match(
    template,
    /viewerText\('viewer\.passport\.relationship\.inspect'/,
  )
...
test("fine-pointer and keyboard intent preview the exact edge before activation", () => {
  ...
  assert.match(
    template,
    /if \(pinnedRelationshipKey \|\| hoveredRelationship !== target/,
  )
...
test("activation opens the existing source passport and pins its exact relationship row", () => {
  ...
  assert.match(
    template,
    /relationshipList\.querySelectorAll\('\[data-relationship-key\]'\)/,
  )
...
test("direct relationship targets support one-tap touch, yield to stronger states, and stay export-clean", () => {
  ...
  assert.match(
    template,
    /clone\.querySelectorAll\('\[data-relationship-hit-overlay\]'\)/,
  )
  assert.match(
    template,
